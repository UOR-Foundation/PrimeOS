...
const runNodeTests = async () => {
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      if (code === 0) {
        console.log(
          "Node.js storage integration tests completed successfully!",
        );
        resolve(code);
      } else {
        console.error(
          `Node.js storage integration tests failed with code ${code}`,
        );
        reject(code);
      }
    });
...
const runBrowserTests = async () => {
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      if (code === 0) {
        console.log(
          "Browser storage integration tests completed successfully!",
        );
        resolve(code);
      } else {
        console.error(
          `Browser storage integration tests failed with code ${code}`,
        );
        reject(code);
      }
    });
