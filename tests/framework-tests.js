...
const base0Tests = [
  {
    ...
    test: function() {
      ...
      assertEqual(embeddingVector.length, 5, 'Embedding should have 5 dimensions');
  ...
  },
  {
    ...
    test: function() {
      ...
      // Test properties
      assertEqual(representation.type, 'representation', 'Type should be representation');
      assertEqual(representation.transformations.length, 2, 'Should have 2 transformations');
      assertEqual(Object.keys(representation.formats).length, 2, 'Should have 2 formats');
      
      // Test transform function
      const transformed = representation.transform('hello', 'uppercase');
      assertEqual(transformed, 'HELLO', 'String should be transformed to uppercase');
      
      // Test represent function
      const represented = representation.represent({ value: 'test' }, 'json');
      assertEqual(represented, '{"value":"test"}', 'Object should be represented as JSON');
  ...
  },
  {
    ...
    test: function() {
      ...
      assert(components.representation._components, 'Representation should be connected');
...
const base1Tests = [
  ...
  },
  {
    ...
    test: function() {
      ...
      // Test properties
      assertEqual(observation.type, 'observation', 'Type should be observation');
      ...
      assert(typeof subscription.unsubscribe === 'function', 'Subscription should have unsubscribe method');
      
      // Test resolve function returns null for unknown reference
      const resolved = observation.resolve('unknown');
      assert(resolved === null, 'Resolve should return null for unknown reference');
  ...
  },
  {
    ...
    test: function() {
      ...
      const mutations = [
        { name: 'increment', apply: obj => ({ ...obj, value: obj.value + 1 }) },
        { name: 'decrement', apply: obj => ({ ...obj, value: obj.value - 1 }) }
      ];
      ...
      // Test properties
      assertEqual(interaction.type, 'interaction', 'Type should be interaction');
      ...
      // Test validate function
      assert(interaction.validate({ value: 50 }), 'Valid object should validate');
  ...
  },
  {
    ...
    test: function() {
      ...
      const renderers = [
        {
          canRender: (obj, target) => typeof obj === 'string' && target && target.innerHTML !== undefined,
          render: (obj, target) => {
      ...
      assert(presented.includes('Test'), 'Result should include object properties');
      
      // Test render function
      const target = { innerHTML: '' };
      const rendered = representation.render('Hello World', target);
      assertEqual(target.innerHTML, 'Hello World', 'Target should have rendered content');
  ...
  },
  {
    ...
    test: function() {
      ...
      // Test connections
      assert(base1.runtime.embedding === base0.embedding, 'Runtime should be connected to embedding');
      assert(base1.runtime.logic === base0.logic, 'Runtime should be connected to logic');
      assert(base1.runtime.representation === base0.representation, 'Runtime should be connected to representation');
      assert(base1.runtime.processor === base0.processor, 'Runtime should be connected to processor');
    }
  }
];
...
const base2Tests = [
  {
    ...
    test: function() {
      ...
      // Test properties
      assertEqual(resourceClient.type, 'resourceClient', 'Type should be resourceClient');
      ...
      assert(resourceClient.representation, 'Should have representation component');
      
      // Test getters
      assert(resourceClient.getRuntime() === base1.runtime, 'Should get runtime component');
      assert(resourceClient.getObservation() === base1.observation, 'Should get observation component');
      assert(resourceClient.getInteraction() === base1.interaction, 'Should get interaction component');
      assert(resourceClient.getRepresentation() === base1.representation, 'Should get representation component');
  ...
  },
  {
    ...
    test: function() {
      ...
      // Test properties
      assertEqual(applicationManager.type, 'applicationManager', 'Type should be applicationManager');
      assertEqual(applicationManager.bundles.length, 0, 'Should have 0 bundles initially');
      ...
      assertEqual(applicationManager.bundles.length, 1, 'Should have 1 bundle after loading');
      
      // Test getBundle function
      const retrievedBundle = applicationManager.getBundle('testBundle');
      assertEqual(retrievedBundle.id, 'testBundle', 'Should retrieve the correct bundle');
      
      // Test startApplication function
      const app = applicationManager.startApplication('testBundle', { appId: 'testApp' });
      assertEqual(app.id, 'testApp', 'App should have the specified ID');
      assertEqual(app.bundle.id, 'testBundle', 'App should reference the correct bundle');
      
      // Test getApplication function
      const retrievedApp = applicationManager.getApplication('testApp');
      assertEqual(retrievedApp.id, 'testApp', 'Should retrieve the correct application');
      ...
      assertEqual(Object.keys(runningApps).length, 0, 'Should have 0 running applications after stopping');
      
      // Test unloadBundle function
      applicationManager.unloadBundle('testBundle');
      assertEqual(applicationManager.bundles.length, 0, 'Should have 0 bundles after unloading');
  ...
  },
  {
    ...
    test: function() {
      ...
      // Test properties
      assertEqual(systemManager.type, 'systemManager', 'Type should be systemManager');
      ...
      const resource = systemManager.allocateResource('database', { url: 'test://db' });
      assert(resource.address, 'Should have an address');
      assertEqual(resource.type, 'database', 'Should have the correct type');
      assertEqual(resource.config.url, 'test://db', 'Should have the correct config');
  ...
  },
  {
    ...
    test: function() {
      ...
      assert(base2.resourceClient.runtime === base1.runtime, 'ResourceClient should be connected to runtime');
      assert(base2.resourceClient.observation === base1.observation, 'ResourceClient should be connected to observation');
      assert(base2.resourceClient.interaction === base1.interaction, 'ResourceClient should be connected to interaction');
      assert(base2.resourceClient.representation === base1.representation, 'ResourceClient should be connected to representation');
    }
  }
];
...
const base3Tests = [
  {
    ...
    test: function() {
      ...
      assertEqual(app.behavior.state.count, 1, 'State should be updated after dispatch');
      ...
      assertEqual(found.props.innerText, 'Clear', 'Found component should have correct properties');
      
      const removed = app.structure.removeComponent('clear');
      assertEqual(removed, true, 'Remove should return true');
      assert(!app.structure.findComponent('clear'), 'Component should be removed');
    }
  },
  {
    ...
    test: function() {
      ...
      assert(typeof base3.createApplication === 'function', 'Should have createApplication function');
...
const frameworkTests = [
  {
    ...
    test: function() {
      ...
      assertEqual(syscallResult, 'framework syscall', 'Framework syscall should work');
    }
  },
  {
    ...
    test: function() {
      ...
      assertEqual(app.behavior.state.result, 70, 'App should use kernel service correctly');
