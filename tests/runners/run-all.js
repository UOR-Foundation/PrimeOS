    ...
    exec(command, { maxBuffer: 100 * 1024 * 1024 }, (error, stdout, stderr) => {
      ...
      try {
        ...
        if (jsonStartIdx >= 0 && jsonEndIdx >= 0 && jsonEndIdx > jsonStartIdx) {
            ...
            results.testResults.forEach((testFile) => {
              if (testFile.numFailingTests > 0) {
                ...
                testFile.assertionResults
                  ...
                  .forEach((assertion) => {
                    console.log(
                      `  • ${assertion.fullName}: ${assertion.failureMessages[0]}`,
                    );
                  });
              }
            });
          ...
          if (testSummaryMatch) {
            const [, failedSuites, passedSuites, totalSuites] =
              testSummaryMatch;

            // Also extract tests count
            const testCountMatch = stdout.match(
            ...
            if (testCountMatch) {
              [, failedTests, skippedTests, passedTests, totalTests] =
                testCountMatch;
            }

            console.log(`\n${testType.name} Summary (parsed from stdout):`);
            console.log(
              `- Test Suites: ${failedSuites} failed, ${passedSuites} passed, ${totalSuites} total`,
            );
            console.log(
        ...
        if (testSummaryMatch) {
          ...
          reject(
            error || new Error("Could not parse test results: " + e.message),
          );
        }
      }
    });
...
async function runAllTests() {
  ...
  const testTypesToRun = specificTests
    ? config.testTypes.filter((type) =>
        specificTests.some((arg) =>
          type.dir.toLowerCase().includes(arg.toLowerCase()),
        ),
      )
    : config.testTypes;
  ...
  try {
    ...
    allResults.forEach((result) => {
      ...
      console.log(
        `${status} - ${result.type}: ${result.passed}/${result.total} passed, ${result.failed} failed, ${result.pending} pending`,
      );
    });
