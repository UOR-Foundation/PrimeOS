  ...
  const expectObj = {
    ...
    toBeCloseTo: (expected, precision = 2) => {
      if (typeof actual !== 'number' || typeof expected !== 'number') {
        throw new Error(`Both values must be numbers for toBeCloseTo. Got ${typeof actual} and ${typeof expected}`);
      }
...
global.matrixApproxEqual = (A, B, epsilon = 1e-10) => {
  ...
  for (let i = 0; i < A.length; i++) {
    ...
    for (let j = 0; j < A[i].length; j++) {
      ...
      const relDiff = Math.abs(A[i][j]) > 1e-10 ? diff / Math.abs(A[i][j]) : diff;
  ...
  if (!isEqual && isDebug) {
    ...
    console.log(`A[${maxDiffLocation}] = ${A[maxDiffLocation[0]][maxDiffLocation[1]]}`);
    console.log(`B[${maxDiffLocation}] = ${B[maxDiffLocation[0]][maxDiffLocation[1]]}`);
    console.log(`Max relative difference: ${maxRelDiff} at [${maxRelDiffLocation}]`);
    ...
    console.log('\nDifference matrix:');
    for (let i = 0; i < A.length; i++) {
      console.log(`[${A[i].map((v, j) => Math.abs(v - B[i][j]).toExponential(2)).join(', ')}]`);
    }
  }
