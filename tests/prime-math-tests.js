...
describe('Prime.math', () => {
  describe('Module Structure', () => {
    it('should be accessible through Prime.math namespace', () => {
      ...
      assert.equal(typeof Prime.math, 'object', 'Prime.math should be an object');
    });
    ...
    it('should provide Vector and Matrix classes', () => {
      ...
      assert.equal(typeof Prime.math.Vector, 'function', 'Vector should be a constructor');
      assert.equal(typeof Prime.math.Matrix, 'function', 'Matrix should be a constructor');
    });

    it('should provide helper functions for vector creation', () => {
      assert.ok(Prime.math.createVector, 'Prime.math.createVector should exist');
      assert.ok(Prime.math.vectorFromArray, 'Prime.math.vectorFromArray should exist');
    ...
    it('should provide helper functions for matrix creation', () => {
      assert.ok(Prime.math.createMatrix, 'Prime.math.createMatrix should exist');
      assert.ok(Prime.math.matrixFromArray, 'Prime.math.matrixFromArray should exist');
      assert.ok(Prime.math.zeroMatrix, 'Prime.math.zeroMatrix should exist');
      assert.ok(Prime.math.identityMatrix, 'Prime.math.identityMatrix should exist');
    });
  });
  ...
  describe('Vector Operations', () => {
    ...
    it('should calculate vector norm correctly', () => {
      ...
      assert.ok(Math.abs(normL2 - Math.sqrt(1*1 + 2*2 + 3*3)) < 1e-10,
      ...
      assert.ok(Math.abs(helper - Math.sqrt(14)) < 1e-10,
    ...
    it('should perform Gram-Schmidt orthogonalization correctly', () => {
      ...
      for (let i = 0; i < orthogonal.length; i++) {
        for (let j = i + 1; j < orthogonal.length; j++) {
          ...
          assert.ok(Math.abs(dot) < 1e-10, `Vectors ${i} and ${j} should be orthogonal`);
        }
      }

      // Verify unit lengths
      for (const vec of orthogonal) {
        assert.ok(Math.abs(vec.norm() - 1) < 1e-10, 'Vectors should have unit length');
      }
    });
  });
  ...
  describe('Matrix Operations', () => {
    ...
    beforeEach(() => {
      m1 = Prime.math.createMatrix([[1, 2], [3, 4]]);
      m2 = Prime.math.createMatrix([[5, 6], [7, 8]]);
      m3 = Prime.math.createMatrix([[0, 0], [0, 0]]);
    });

    it('should create matrices correctly', () => {
      assert.deepStrictEqual(m1.values, [[1, 2], [3, 4]]);
      assert.deepStrictEqual(m1.shape, [2, 2]);

      // Create with dimensions
      const zeros = Prime.math.createMatrix(2, 3);
      assert.deepStrictEqual(zeros.values, [[0, 0, 0], [0, 0, 0]]);
      assert.deepStrictEqual(zeros.shape, [2, 3]);

      // Factory methods
      const fromArray = Prime.math.matrixFromArray([[7, 8], [9, 10]]);
      assert.deepStrictEqual(fromArray.values, [[7, 8], [9, 10]]);

      const zeroMat = Prime.math.zeroMatrix(2, 2);
      assert.deepStrictEqual(zeroMat.values, [[0, 0], [0, 0]]);

      const ones = Prime.math.onesMatrix(2, 2);
      assert.deepStrictEqual(ones.values, [[1, 1], [1, 1]]);

      const identity = Prime.math.identityMatrix(3);
      assert.deepStrictEqual(identity.values, [[1, 0, 0], [0, 1, 0], [0, 0, 1]]);

      const fromFlat = Prime.math.matrixFromFlatArray([1, 2, 3, 4], 2, 2);
      assert.deepStrictEqual(fromFlat.values, [[1, 2], [3, 4]]);
    });
    ...
    it('should convert matrices to arrays correctly', () => {
      ...
      assert.deepStrictEqual(array2D, [[1, 2], [3, 4]]);
    ...
    it('should perform basic matrix operations', () => {
      // Addition
      const sum = m1.add(m2);
      assert.deepStrictEqual(sum.values, [[6, 8], [10, 12]]);

      // Subtraction
      const diff = m1.subtract(m2);
      assert.deepStrictEqual(diff.values, [[-4, -4], [-4, -4]]);

      // Scaling
      const scaled = m1.scale(2);
      assert.deepStrictEqual(scaled.values, [[2, 4], [6, 8]]);

      // Helper functions
      const sum2 = Prime.math.addMatrices(m1, m2);
      assert.deepStrictEqual(sum2.values, [[6, 8], [10, 12]]);

      const diff2 = Prime.math.subtractMatrices(m1, m2);
      assert.deepStrictEqual(diff2.values, [[-4, -4], [-4, -4]]);

      const scaled2 = Prime.math.scaleMatrix(m1, 2);
      assert.deepStrictEqual(scaled2.values, [[2, 4], [6, 8]]);
    });
    ...
    it('should multiply matrices correctly', () => {
      ...
      // [1,2] * [7,8] = 1*6 + 2*8 = 22
      // [3,4] * [7,8] = 3*6 + 4*8 = 50
      assert.deepStrictEqual(product.values, [[19, 22], [43, 50]]);

      // Helper function
      const product2 = Prime.math.multiplyMatrices(m1, m2);
      assert.deepStrictEqual(product2.values, [[19, 22], [43, 50]]);
      ...
      const m2x2 = Prime.math.createMatrix([[1, 2], [3, 4]]);
      const m2x3 = Prime.math.createMatrix([[1, 2, 3], [4, 5, 6]]);
      const m3x2 = Prime.math.createMatrix([[1, 2], [3, 4], [5, 6]]);
    ...
    it('should calculate transpose correctly', () => {
      ...
      assert.deepStrictEqual(transposed.values, [[1, 3], [2, 4]]);

      // Non-square matrix
      const m4 = Prime.math.createMatrix([[1, 2, 3], [4, 5, 6]]);
      const t4 = m4.transpose();
      assert.deepStrictEqual(t4.values, [[1, 4], [2, 5], [3, 6]]);
      assert.deepStrictEqual(t4.shape, [3, 2]);

      // Helper function
      const trans2 = Prime.math.transposeMatrix(m1);
      assert.deepStrictEqual(trans2.values, [[1, 3], [2, 4]]);
    });
    ...
    it('should calculate eigenvalues and eigenvectors correctly', () => {
      ...
      assert.ok(Math.abs(values[0] - 3) < 1e-10 || Math.abs(values[0] - 1) < 1e-10);
      assert.ok(Math.abs(values[1] - 3) < 1e-10 || Math.abs(values[1] - 1) < 1e-10);
    ...
    it('should calculate SVD correctly', () => {
      ...
      for (let i = 0; i < S.rows; i++) {
        for (let j = 0; j < S.cols; j++) {
          if (i !== j) {
            assert.ok(Math.abs(S.get(i, j)) < 1e-6,
      ...
      if (S.rows === 2 && S.cols === 2) {
        assert.ok(S.values[0][0] >= S.values[1][1], 'Singular values should be in descending order');
        assert.ok(S.values[0][0] >= 0 && S.values[1][1] >= 0, 'Singular values should be non-negative');
      }
    ...
    it('should calculate condition number correctly', () => {
      ...
      assert.ok(wellCond < cond, 'Well-conditioned matrix should have lower condition number');
      assert.ok(Math.abs(wellCond - 1) < 1e-10, 'Scalar matrix should have condition number 1');
      ...
      assert.ok(illCond > cond, 'Ill-conditioned matrix should have higher condition number');
    });

    it('should calculate matrix rank correctly', () => {
      const rank = m1.rank();
      assert.strictEqual(rank, 2, 'Full rank matrix should have rank = min(rows, cols)');
      ...
      assert.strictEqual(singularRank, 1, 'Singular matrix should have reduced rank');
    ...
    it('should check matrix equality correctly', () => {
      const m1Copy = Prime.math.createMatrix([[1, 2], [3, 4]]);
      assert.ok(m1.equals(m1Copy));
      assert.ok(!m1.equals(m2));
      
      // With epsilon tolerance
      const m1Approx = Prime.math.createMatrix([[1.000000001, 2], [3, 4]]);
    ...
    it('should apply functions to matrices with map', () => {
      ...
      assert.deepStrictEqual(doubled.values, [[2, 4], [6, 8]]);
      
      const squared = m1.map(x => x * x);
      assert.deepStrictEqual(squared.values, [[1, 4], [9, 16]]);
    });
  });
  ...
  describe('Enhanced Numerical Functions', () => {
    ...
    it('should perform numerical integration correctly', () => {
      ...
      assert.ok(Math.abs(sinResult - 2) < 1e-6,
