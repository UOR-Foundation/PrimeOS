...
const TestRunner = {
  ...
  run: async function (options = {}) {
    ...
    console.log(
      `Running ${this.tests.length} tests in ${Object.keys(this.suites).length || 1} suites...\n`,
    );
    ...
    this.results.timings.duration =
      this.results.timings.end - this.results.timings.start;
...
`;
    ...
    if (Object.keys(this.results.suites).length > 0) {
      ...
      for (const suiteName in this.results.suites) {
        ...
        console.log(
          `  ${suiteName}: ${suite.passed}/${suite.total} passed (${passRate}%)`,
        );
      }
      console.log("");
    }
    ...
    if (this.results.failures.length > 0) {
      ...
      for (const failure of this.results.failures) {
        console.log(
          `  - [${failure.suite || "default"}] ${failure.name}: ${failure.error.message}`,
        );
      }
    }
  ...
  assertEqual: function (actual, expected, message) {
    ...
    try {
      ...
      if (actual !== expected) {
        throw new Error(
          message || `Values are not equal (JSON stringify failed)`,
        );
      }
      return;
    }
  ...
  assertNotEqual: function (actual, expected, message) {
    ...
    if (actual === undefined && expected === undefined) {
      throw new Error(
        message || "Values are both undefined when they should differ",
      );
    }
    if (actual === null && expected === null) {
      throw new Error(
        message || "Values are both null when they should differ",
      );
    }
    ...
    if (actualStr === expectedStr) {
      throw new Error(
        message || `Values are equal (${expectedStr}) when they should differ`,
      );
    }
  },
  ...
  assertThrows: function (fn, errorTypeOrMessage, message) {
    ...
    try {
      ...
      if (expectedErrorType && !(error instanceof expectedErrorType)) {
        throw new Error(
          message ||
            `Expected error of type ${expectedErrorType.name}, but got ${error.constructor.name}`,
        );
      }
...
TestRunner.suite("Core", function () {
  TestRunner.test("Prime core exists and exposes essential APIs", () => {
    ...
    TestRunner.assert(
      typeof Prime.version === "string",
      "Prime version should exist",
    );
  ...
  TestRunner.test("Prime utilities handle type checking correctly", () => {
    ...
    TestRunner.assert(
      ...
      "isObject should identify objects",
    );
    TestRunner.assertFalse(
      Prime.Utils.isObject(null),
      "isObject should reject null",
    );
    TestRunner.assert(
      Prime.Utils.isFunction(() => {}),
      "isFunction should identify functions",
    );
    TestRunner.assert(
      Prime.Utils.isArray([]),
      "isArray should identify arrays",
    );
    TestRunner.assert(
      Prime.Utils.isString(""),
      "isString should identify strings",
    );
    TestRunner.assert(
      Prime.Utils.isNumber(42),
      "isNumber should identify numbers",
    );
  });
  ...
  TestRunner.test("EventBus behaves correctly", () => {
    ...
    if (callCount !== 1) {
      ...
      if (callCount !== 1) {
        throw new Error(
          "Events not working as expected - handler was not called",
        );
      }
    }
    ...
    TestRunner.assertEqual(
      payloadValue,
      42,
      "Handler should receive correct payload",
    );
    ...
    TestRunner.assertEqual(
      payloadValue,
      43,
      "Handler should receive updated payload",
    );
    ...
    if (!unsubscribeMethod) {
      throw new Error(
        "Unable to unsubscribe event - no unsubscribe method available",
      );
    }
    ...
    if (callCount === countBeforeThirdEmit) {
      ...
      if (lastPayload && lastPayload.value !== undefined) {
        TestRunner.assertEqual(
          lastPayload.value,
          43,
          "Payload should not update after unregistering",
        );
      } else if (
        lastPayload &&
        lastPayload.data &&
      ...
      TestRunner.assert(
        false,
        "Unsubscribe did not work as expected; handler was still called",
      );
    }
    ...
    if (typeof eventBus.once === "function") {
      ...
      TestRunner.assertEqual(
        callCount,
        prevCount + 1,
        "Once handler should be called exactly once",
      );
      ...
      TestRunner.assertEqual(
        callCount,
        countAfterOnce,
        "Once handler should not be called a second time",
      );
    }
  });
});
...
TestRunner.suite("Component System", function () {
  TestRunner.test("Component integration with coherence", () => {
    ...
    TestRunner.assertEqual(
      ...
      20,
      "Component count should be updated",
    );
    ...
    TestRunner.assertEqual(
      component.variant.count,
      50,
      "Component count should be updated to 50",
    );
    TestRunner.assertEqual(
      component.coherenceNorm(),
      0,
      "Component should remain coherent with multiple constraints",
    );
    ...
    try {
      ...
      TestRunner.assert(
        e instanceof Prime.CoherenceViolationError,
  ...
  TestRunner.test("Component lifecycle and events", () => {
    ...
    TestRunner.assert(
      component._events[0] === "init",
      "Init event should be first",
    );
    ...
    if (typeof component.destroy === "function") {
      ...
      TestRunner.assert(
        ...
        "Component should be marked as destroyed",
      );
      TestRunner.assert(
        component._events[1] === "destroy",
        "Destroy event should be second",
      );
    }
  });
});
...
TestRunner.suite("Mathematics", function () {
  TestRunner.test("Vector operations", () => {
    ...
    const sum = v1.add(v2);
    TestRunner.assert(
      sum.equals(new Prime.math.Vector([5, 7, 9])),
    ...
    TestRunner.assertCloseTo(
      magnitude,
      Math.sqrt(14),
      0.001,
      "Vector magnitude should work correctly",
    );
  ...
  TestRunner.test("Matrix operations", () => {
    ...
    if (typeof m1.inverse === "function") {
      ...
      TestRunner.assertCloseTo(
        ...
        1,
        0.001,
        "Matrix inverse should work correctly",
      );
      TestRunner.assertCloseTo(
        identity.get(1, 1),
        1,
...
TestRunner.suite("Framework Integration", function () {
  TestRunner.test("Complete application lifecycle", () => {
    ...
    TestRunner.assertEqual(
      ...
      2,
      "App should have 2 items",
    );
    TestRunner.assertEqual(
      app.state.items[0].id,
      "item1",
      "First item should be added",
    );
    TestRunner.assertEqual(
      app.state.items[1].id,
      "item2",
