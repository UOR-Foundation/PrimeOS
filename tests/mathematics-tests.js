...
const Test = {
  ...
  assertApproximatelyEqual: function(actual, expected, epsilon, message) {
    ...
    const relError = expected !== 0 ? Math.abs((actual - expected) / expected) : absError;
    ...
    if (absError <= effectiveEps) {
      ...
      console.error(`    Absolute error: ${absError}, Relative error: ${relError}, Epsilon: ${effectiveEps}`);
    }
  },
  ...
  assertHighPrecision: function(actual, expected, message) {
    ...
    if (Math.abs(actual - expected) <= ulpFactor) {
      ...
      console.error(`✗ FAIL (high precision): ${message} - Expected exactly ${expected}, got ${actual}`);
      console.error(`    Error: ${Math.abs(actual - expected)}, ULP factor: ${ulpFactor}`);
    }
  },
  ...
  assertExactlyEqual: function(actual, expected, message) {
    if (actual === expected) {
      ...
      console.error(`✗ FAIL (exact): ${message} - Expected ${expected} (${typeof expected}), got ${actual} (${typeof actual})`);
    }
  },
  ...
  assertArrayEqual: function(actual, expected, epsilon, message) {
    ...
    if (actual.length !== expected.length) {
      ...
      console.error(`✗ FAIL: ${message} - Array length mismatch: expected ${expected.length}, got ${actual.length}`);
      return;
    }
    ...
    if (errors.length === 0) {
      ...
      errors.forEach(e => {
        console.error(`    At index ${e.index}: expected ${e.expected}, got ${e.actual}, error: ${e.error}`);
      });
    }
  },
  ...
  assertStable: function(fn, expected, maxRelError, message) {
    try {
      ...
      if (relativeError <= maxRelError) {
        ...
        console.error(`✗ FAIL (stability): ${message} - Expected relative error ≤ ${maxRelError}, got ${relativeError}`);
      }
    } catch (e) {
      this.failed++;
      this.failureDetails.push({
...
testResults.push(Test.run("Mathematics Precision", function() {
  // Test floating point precision boundaries
  Test.assertHighPrecision(1 + Number.EPSILON, 1 + Number.EPSILON, "Can represent epsilon accurately");
  Test.assertHighPrecision(Math.sqrt(2) * Math.sqrt(2), 2, "Square root operations maintain precision");
  Test.assertHighPrecision(Math.cos(Math.PI), -1, "Cosine of PI is exactly -1");
  ...
  }, 1, 1e-10, "Large number subtraction maintains precision");
  
  // Test edge cases in trigonometric functions
  Test.assertApproximatelyEqual(Math.sin(Math.PI), 0, 1e-15, "Sin(PI) is very close to 0");
  Test.assertApproximatelyEqual(Math.sin(2*Math.PI), 0, 1e-15, "Sin(2*PI) is very close to 0");
  Test.assertApproximatelyEqual(Math.cos(Math.PI/2), 0, 1e-15, "Cos(PI/2) is very close to 0");
  ...
  for (let i = 0; i < 3; i++) {
    for (let j = 0; j < 3; j++) {
      ...
      Test.assertExactlyEqual(matMul[i][j], expected, `Matrix multiplication at [${i},${j}] is exact`);
    }
  }
  ...
  Test.assertApproximatelyEqual(Math.PI * Math.PI, 9.869604401089358, 1e-14, "PI squared calculated precisely");
  Test.assertApproximatelyEqual(Math.sqrt(2) * Math.sqrt(3), Math.sqrt(6), 1e-14, "Product of square roots is precise");
  ...
  Test.assertArrayEqual(arr1, arr3, 1e-9, "Array equality with small epsilon passes");
  
  // Test handling of special values
  Test.assertExactlyEqual(1 / 0, Infinity, "Division by zero yields Infinity");
  ...
  Test.assertApproximatelyEqual(Math.sin(Math.PI/4) * Math.sin(Math.PI/4) + Math.cos(Math.PI/4) * Math.cos(Math.PI/4), 1, 1e-15, "sin²(θ) + cos²(θ) = 1");
  Test.assertApproximatelyEqual(Math.log(Math.exp(1)), 1, 1e-15, "log(exp(1)) = 1");
  Test.assertApproximatelyEqual(Math.pow(2, Math.log2(8)), 8, 1e-14, "2^(log2(8)) = 8");
}));
...
testResults.push(Test.run("Clifford Algebra", function() {
  ...
  Test.assertExactlyEqual(Cl3.signature.length, 3, "Signature has correct length");
  ...
  const ClNonStd = Prime.Clifford.create({ dimension: 2, signature: [-1, -1] });
  Test.assertArrayEqual(ClNonStd.signature, [-1, -1], 0, "Non-standard signature is preserved");
  ...
  Test.assertExactlyEqual(scalar.components[0]["1"], 5, "scalar has correct value");
  
  // Test very small scalar value (test for epsilon handling)
  const tinyScalar = Cl3.scalar(1e-15);
  Test.assertHighPrecision(tinyScalar.components[0]["1"], 1e-15, "tiny scalar value is preserved");
  
  const vector = Cl3.vector([1, 2, 3]);
  Test.assert(Prime.Clifford.isMultivector(vector), "vector creates a multivector");
  Test.assertExactlyEqual(vector.components[1]["e1"], 1, "vector has correct x component");
  Test.assertExactlyEqual(vector.components[1]["e2"], 2, "vector has correct y component");
  Test.assertExactlyEqual(vector.components[1]["e3"], 3, "vector has correct z component");
  ...
  Test.assertExactlyEqual(bivectorMatrix.components[2]["e1e2"], 1, "bivector has correct e1^e2 component");
  Test.assertExactlyEqual(bivectorMatrix.components[2]["e1e3"], 2, "bivector has correct e1^e3 component");
  Test.assertExactlyEqual(bivectorMatrix.components[2]["e2e3"], 3, "bivector has correct e2^e3 component");
  ...
  Test.assertExactlyEqual(general.components[0]["1"], 1, "multivector has correct scalar component");
  Test.assertExactlyEqual(general.components[1]["e1"], 2, "multivector has correct e1 component");
  Test.assertExactlyEqual(general.components[1]["e3"], 3, "multivector has correct e3 component");
  Test.assertExactlyEqual(general.components[2]["e1e2"], 4, "multivector has correct e1^e2 component");
  ...
  Test.assert(Prime.Clifford.isMultivector(preciseArray), "fromArray creates a multivector with irrational values");
  Test.assertHighPrecision(preciseArray.components[1]["e1"], Math.PI, "fromArray preserves PI precisely");
  Test.assertHighPrecision(preciseArray.components[1]["e2"], Math.E, "fromArray preserves E precisely");
  Test.assertHighPrecision(preciseArray.components[1]["e3"], Math.sqrt(2), "fromArray preserves sqrt(2) precisely");
  ...
  Test.assertExactlyEqual(fromArray.components[1]["e1"], 4, "fromArray has correct x component");
  Test.assertExactlyEqual(fromArray.components[1]["e2"], 5, "fromArray has correct y component");
  Test.assertExactlyEqual(fromArray.components[1]["e3"], 6, "fromArray has correct z component");
  ...
  Test.assertHighPrecision(safeBivector.components[2]["e1e2"], safeValues[0], 
    "Large value preserved in bivector");
  Test.assertHighPrecision(safeBivector.components[2]["e1e3"], safeValues[1], 
    "Small value preserved in bivector");
  Test.assertHighPrecision(safeBivector.components[2]["e2e3"], safeValues[2], 
  ...
  Test.assert(!zeroHandlingVector.components[1].hasOwnProperty("e1"), "Zero x component is eliminated");
  Test.assert(!zeroHandlingVector.components[1].hasOwnProperty("e3"), "Zero z component is eliminated");
  Test.assert(zeroHandlingVector.components[1].hasOwnProperty("e2"), "Non-zero y component is preserved");
...
testResults.push(Test.run("SO(3) Rotations", function() {
  ...
  Test.assertApproximatelyEqual(rotatedLength, originalLength, 1e-6, 
  ...
  Test.assertApproximatelyEqual(rotatedTwice[0], v[0], 1e-6, "Arbitrary rotation twice is identity (x)");
  Test.assertApproximatelyEqual(rotatedTwice[1], v[1], 1e-6, "Arbitrary rotation twice is identity (y)");
  Test.assertApproximatelyEqual(rotatedTwice[2], v[2], 1e-6, "Arbitrary rotation twice is identity (z)");
...
testResults.forEach(result => {
  ...
  if (result.failureDetails && result.failureDetails.length > 0) {
    ...
    result.failureDetails.forEach(detail => {
      ...
      else if (detail.absoluteError !== undefined) precisionStats.approximateTests++;
...
if (overallFailed > 0) {
  ...
  if (precisionFailures.length > 0) {
    ...
    precisionFailures.forEach((detail, index) => {
      console.error(`${index + 1}. Suite: ${detail.suite}, Test: ${detail.message}`);
      if (detail.expected !== undefined) console.error(`   Expected: ${detail.expected}`);
      if (detail.actual !== undefined) console.error(`   Actual: ${detail.actual}`);
      if (detail.absoluteError !== undefined) console.error(`   Absolute Error: ${detail.absoluteError}`);
      if (detail.relativeError !== undefined) console.error(`   Relative Error: ${detail.relativeError}`);
      if (detail.ulpFactor !== undefined) console.error(`   ULP Factor: ${detail.ulpFactor}`);
      if (detail.epsilon !== undefined) console.error(`   Epsilon: ${detail.epsilon}`);
      if (detail.maxRelError !== undefined) console.error(`   Max Relative Error: ${detail.maxRelError}`);
    });
  }
  ...
  if (precisionFailures.length > 0) {
    ...
    console.error(`Approximate equality failures: ${precisionStats.approximateTests}`);
