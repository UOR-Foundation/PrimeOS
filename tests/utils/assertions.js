...
const Assertions = {
  ...
  assertThrows: (fn, errorType, errorMessage, message = "") => {
    ...
    const checkPromise = (result) => result && typeof result.then === 'function';
    ...
    try {
      ...
      if (checkPromise(result)) {
        return result
          .then(() => {
            assert.fail(`${message || "Expected to throw"}${errorType ? ': ' + errorType.name : ''}`);
          })
          .catch(handleError);
      }
   ...
   */
  assertAsyncErrorHandling: async (asyncOperation, errorCases, errorClassifier, message = "") => {
    const results = {};
   ...
   */
  assertErrorBoundaries: (operation, validInputs, boundaryInputs, invalidInputs, message = "") => {
    const results = {
      valid: [],
      boundary: [],
      invalid: []
