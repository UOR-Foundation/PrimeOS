...
const Fixtures = {
  ...
  createVector: (pattern, dimension, options = {}) => {
    switch (pattern) {
      case 'identity':
        return Array(dimension).fill(0).map((_, i) => i === options.index ? 1 : 0);
      case 'random':
        return Array(dimension).fill(0).map(() => Math.random());
      case 'sequence':
        return Array(dimension).fill(0).map((_, i) => i + (options.start || 0));
      case 'zeros':
      ...
      case 'extreme':
        for (let i = 0; i < rows; i++) {
          for (let j = 0; j < cols; j++) {
            ...
            if (index % 5 === 0) matrix[i][j] = 1e-15; // Extremely small
            else if (index % 5 === 1) matrix[i][j] = 1e15; // Extremely large
            else if (index % 5 === 2) matrix[i][j] = 0; // Zero
            else if (index % 5 === 3) matrix[i][j] = 1; // Regular value
  ...
  createCoherenceTestData: (options = {}) => {
    ...
    // Create state vector
    const state = Array(dimension).fill(0).map(() => Math.floor(Math.random() * 100));
