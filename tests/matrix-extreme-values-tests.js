...
describe("Matrix Extreme Values", () => {
  describe("LU Decomposition with Extreme Values", () => {
    test("should handle matrices with large magnitude differences", () => {
      ...
      expect(
        Math.abs(reconstructed[0][0] / matrixWithExtremes[0][0] - 1),
      ).toBeLessThan(1e-10);
      expect(
        Math.abs(reconstructed[0][1] / matrixWithExtremes[0][1] - 1),
      ).toBeLessThan(1e-6);
      expect(
        Math.abs(reconstructed[1][0] / matrixWithExtremes[1][0] - 1),
      ).toBeLessThan(1e-6);
      expect(
        Math.abs(reconstructed[1][1] / matrixWithExtremes[1][1] - 1),
      ).toBeLessThan(1e-10);
    });
      ...
      for (let i = 0; i < matrixWithExtremes.length; i++) {
        for (let j = 0; j < matrixWithExtremes[0].length; j++) {
          if (Math.abs(matrixWithExtremes[i][j]) > 1e-6) {
            expect(
              Math.abs(reconstructed[i][j] / matrixWithExtremes[i][j] - 1),
            ).toBeLessThan(1e-6);
          } else {
            expect(
              Math.abs(reconstructed[i][j] - matrixWithExtremes[i][j]),
            ).toBeLessThan(1e-6);
          }
        }
      }
  ...
  describe("Eigenvalues with Extreme Values", () => {
    test("should handle symmetric matrices with extreme values", () => {
      ...
      // Compute eigenvalues
      const { eigenvalues } = Prime.Math.Matrix.eigenvalues(symmetricExtreme, {
        numEigenvalues: 2,
      });
  ...
  describe("SVD with Extreme Values", () => {
    ...
    test("should handle very small matrices", () => {
      ...
      expect(result.S.values[0][0]).toBeGreaterThanOrEqual(
        result.S.values[1][1],
      );
