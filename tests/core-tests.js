...
// Test Utilities
testResults.push(Test.run("Utilities", function() {
  // isObject tests
  Test.assert(Prime.Utils.isObject({}), "isObject returns true for empty object");
  Test.assert(Prime.Utils.isObject({ a: 1 }), "isObject returns true for object literal");
  ...
  Test.assert(!Prime.Utils.isObject("string"), "isObject returns false for string");
  Test.assert(!Prime.Utils.isObject(123), "isObject returns false for number");
  Test.assert(!Prime.Utils.isObject(true), "isObject returns false for boolean");
  Test.assert(!Prime.Utils.isObject(undefined), "isObject returns false for undefined");
  Test.assert(!Prime.Utils.isObject(function() {}), "isObject returns false for function");
  
  // isFunction tests
  Test.assert(Prime.Utils.isFunction(function() {}), "isFunction returns true for function declaration");
  Test.assert(Prime.Utils.isFunction(() => {}), "isFunction returns true for arrow function");
  Test.assert(!Prime.Utils.isFunction({}), "isFunction returns false for object");
  Test.assert(!Prime.Utils.isFunction([]), "isFunction returns false for array");
  
  // isArray tests
  Test.assert(Prime.Utils.isArray([]), "isArray returns true for empty array");
  Test.assert(Prime.Utils.isArray([1, 2, 3]), "isArray returns true for array with elements");
  Test.assert(!Prime.Utils.isArray({}), "isArray returns false for object");
  Test.assert(!Prime.Utils.isArray("string"), "isArray returns false for string");
  ...
  Test.assert(!Prime.Utils.isNumber("123"), "isNumber returns false for numeric string");
  
  // isString tests
  Test.assert(Prime.Utils.isString(""), "isString returns true for empty string");
  Test.assert(Prime.Utils.isString("hello"), "isString returns true for non-empty string");
  Test.assert(!Prime.Utils.isString(123), "isString returns false for number");
  
  // isBoolean tests
  Test.assert(Prime.Utils.isBoolean(true), "isBoolean returns true for true");
  Test.assert(Prime.Utils.isBoolean(false), "isBoolean returns true for false");
  ...
  Test.assert(!Prime.Utils.isBoolean("true"), "isBoolean returns false for string 'true'");
  ...
  Test.assert(clone !== original, "deepClone creates a different object reference");
  Test.assert(clone.b !== original.b, "deepClone creates deep object copies");
  Test.assert(clone.b.d !== original.b.d, "deepClone creates deep array copies");
  Test.assert(clone.b.d[2] !== original.b.d[2], "deepClone creates deep nested object copies");
  Test.assert(clone.f.getTime() === original.f.getTime(), "deepClone preserves Date values");
  Test.assert(clone.g.source === original.g.source && clone.g.flags === original.g.flags, "deepClone preserves RegExp properties");
  Test.assert(clone.h.get('key') === original.h.get('key'), "deepClone preserves Map entries");
  Test.assert(clone.i.has(1) && clone.i.has(2) && clone.i.has(3), "deepClone preserves Set values");
  ...
  Test.assert(circularClone !== circular, "deepClone handles circular references (different reference)");
  Test.assert(circularClone.self === circularClone, "deepClone preserves circular structure");
  ...
  Test.assert(result3 === 5, "memoize returns correct result for different args");
  Test.assert(callCount === 2, "memoize calls original function only when needed");
  
  // get tests
  const obj = { a: { b: { c: 42 } } };
  Test.assert(Prime.Utils.get(obj, 'a.b.c') === 42, "get retrieves nested property with string path");
  Test.assert(Prime.Utils.get(obj, ['a', 'b', 'c']) === 42, "get retrieves nested property with array path");
  Test.assert(Prime.Utils.get(obj, 'a.x.y', 'default') === 'default', "get returns default for missing property");
  Test.assert(Prime.Utils.get(null, 'a.b', 'default') === 'default', "get handles null object");
  ...
  Test.assert(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(uuid1), 
    "uuid has correct format (RFC4122 version 4)");
}));

// Test Error Classes
testResults.push(Test.run("Error Classes", function() {
  // PrimeError tests
  const error1 = new Prime.PrimeError("Test error");
  Test.assert(error1 instanceof Error, "PrimeError is an instance of Error");
  Test.assert(error1 instanceof Prime.PrimeError, "PrimeError is an instance of PrimeError");
  Test.assert(error1.name === "PrimeError", "PrimeError has correct name");
  Test.assert(error1.message === "Test error", "PrimeError has correct message");
  ...
  const error2 = new Prime.PrimeError("Custom error", { code: "CUSTOM_CODE", context: { key: "value" } });
  Test.assert(error2.code === "CUSTOM_CODE", "PrimeError accepts custom code");
  Test.assert(error2.context.key === "value", "PrimeError stores context data");
  
  // CoherenceViolationError tests
  const constraint = { name: "TestConstraint", check: () => false };
  const coherenceError = new Prime.CoherenceViolationError("Coherence violation", constraint, 0.75);
  Test.assert(coherenceError instanceof Prime.PrimeError, "CoherenceViolationError is a PrimeError");
  Test.assert(coherenceError.constraint === constraint, "CoherenceViolationError stores constraint");
  Test.assert(coherenceError.magnitude === 0.75, "CoherenceViolationError stores magnitude");
  
  // MathematicalError tests
  const mathError = new Prime.MathematicalError("Math error");
  Test.assert(mathError instanceof Prime.PrimeError, "MathematicalError is a PrimeError");
  Test.assert(mathError.name === "MathematicalError", "MathematicalError has correct name");
  
  // InvalidOperationError tests
  const opError = new Prime.InvalidOperationError("Invalid operation");
  Test.assert(opError instanceof Prime.PrimeError, "InvalidOperationError is a PrimeError");
  Test.assert(opError.name === "InvalidOperationError", "InvalidOperationError has correct name");
  
  // ConfigurationError tests
  const configError = new Prime.ConfigurationError("Config error");
  Test.assert(configError instanceof Prime.PrimeError, "ConfigurationError is a PrimeError");
  Test.assert(configError.name === "ConfigurationError", "ConfigurationError has correct name");
  
  // ValidationError tests
  const validationError = new Prime.ValidationError("Validation error");
  Test.assert(validationError instanceof Prime.PrimeError, "ValidationError is a PrimeError");
  Test.assert(validationError.name === "ValidationError", "ValidationError has correct name");
}));

// Test EventBus
testResults.push(Test.run("EventBus", function() {
  ...
  Test.assert(typeof unsubscribe === 'function', "subscribe returns unsubscribe function");
  
  // Test publishing
  Prime.EventBus.publish('test-event', { value: 42 });
  Test.assert(callCount === 1, "Event handler was called once");
  Test.assert(lastData && lastData.value === 42, "Event handler received correct data");
...
// Test Logger
testResults.push(Test.run("Logger", function() {
...
testResults.push(Test.run("Version Management", function() {
  // Test version property exists
  Test.assert(typeof Prime.version === 'string', "version is a string");
  Test.assert(/^\d+\.\d+\.\d+$/.test(Prime.version), "version follows semantic versioning format");
  
  // Test validateVersion
  Test.assert(Prime.validateVersion('0.0.1'), "Validates higher version");
  Test.assert(Prime.validateVersion('0.9.0'), "Validates higher minor version");
  Test.assert(Prime.validateVersion('0.9.9'), "Validates higher minor and patch versions");
  Test.assert(Prime.validateVersion(Prime.version), "Validates same version");
  Test.assert(!Prime.validateVersion('99.0.0'), "Rejects higher major version");
  
  // Test isCompatible
  Test.assert(Prime.isCompatible({
    minVersion: '0.0.1',
    features: []
  ...
  Test.assert(result, "isCompatible works with valid version and no features");
}));

// Test Testing Utilities
testResults.push(Test.run("Testing Utilities", function() {
  // Test assert
  ...
  Test.assert(typeof mock.calls === 'object', "Mock has calls tracking object");
  Test.assert(typeof mock.results === 'object', "Mock has results object");
  
  // Test call tracking
  mock.method1(1, 2);
  Test.assert(Array.isArray(mock.calls.method1), "Calls are tracked in an array");
...
// Test ModuleLoader
testResults.push(Test.run("ModuleLoader", function() {
  ...
  Test.assert(Prime.testModule === mockModule, "Module is registered on Prime object");
