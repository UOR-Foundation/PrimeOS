...
function addDeprecationNotice(filePath, replacementPath) {
  ...
  try {
    ...
    const replacements = replacementPath.split(',').map(p => 
      `* - ${p.trim()}`
    ).join('\n');
    
    // Add deprecation notice
    const newContent = deprecationNotice.replace('{replacementPaths}', replacements) + content;
...
`;
  ...
  for (const [oldFile, newFiles] of Object.entries(migrationMap)) {
    const replacements = newFiles.split(',').map(p => p.trim()).join('<br>');
