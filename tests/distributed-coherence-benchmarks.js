...
const runCoherenceBenchmarks = async () => {
  ...
  benchmarkSuite.register(
    ...
    async (context) => {
      ...
      const instabilityLevel =
        instabilityLevels[
          Math.floor(context.iteration / matrixSizes.length) %
            instabilityLevels.length
        ];
  ...
  benchmarkSuite.register(
    ...
    async (context) => {
      ...
      const layer = {
        ...
        biases: introduceNumericalInstability(
  ...
  benchmarkSuite.register(
    ...
    async (context) => {
      ...
      for (
        let i = Math.ceil(nodeCount * 0.7);
        i < Math.ceil(nodeCount * 0.9);
        i++
      ) {
        nodeResults.push({
          nodeId: `node_${i}`,
  ...
  benchmarkSuite.register(
    ...
    async (context) => {
      ...
      if (result.violations) {
        syncViolation = result.violations.find(
          (v) =>
            v.type ===
  ...
  const scalingResult = await benchmarkSuite.measureScaling(
    "tensor_coherence",
    {
      nodeCounts: [1, 2, 4, 8],
      iterations: 10,
