...
describe("Consciousness Coherence Stability", () => {
  ...
  describe("Memory Capacity Limits", () => {
    test("should handle memory overflow gracefully", () => {
      ...
      expect(memory.shortTermMemory.length).toBeLessThanOrEqual(
        memory.shortTermCapacity,
      );
      expect(memory.longTermMemory.length).toBeLessThanOrEqual(
        memory.longTermCapacity,
      );
  ...
  describe("Temporal Stability", () => {
    test("should maintain stability with extreme time steps", () => {
      ...
      expect(Number.isFinite(smallStepModule.currentState.coherence)).toBe(
        true,
      );
      expect(Number.isFinite(largeStepModule.currentState.coherence)).toBe(
        true,
      );

      // Values should remain in valid range
      expect(
        smallStepModule.currentState.vector.every((v) => v >= 0 && v <= 1),
      ).toBe(true);
      expect(
        largeStepModule.currentState.vector.every((v) => v >= 0 && v <= 1),
      ).toBe(true);
    });
