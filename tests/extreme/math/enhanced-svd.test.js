...
describe("Enhanced SVD Implementation", () => {
  ...
  describe("Special Cases", () => {
    ...
    test("handles ill-conditioned matrices", () => {
      ...
      for (let i = 0; i < 5; i++) {
        for (let j = 0; j < 5; j++) {
          expect(Math.abs(hilbert[i][j] - reconstructed[i][j])).toBeLessThan(
            1e-10,
          );
        }
      }
      ...
      for (const matrix of testMatrices) {
        ...
        for (let i = 0; i < rows; i++) {
          for (let j = 0; j < cols; j++) {
            expect(Math.abs(matrix[i][j] - reconstructed[i][j])).toBeLessThan(
              1e-10,
            );
          }
        }
