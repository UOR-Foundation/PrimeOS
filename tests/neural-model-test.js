...
const runCoherenceModelTests = () => {
  ...
  // Test model creation with coherence configuration
  {
    ...
    assert(model instanceof Prime.Neural.Model.NeuralModel, "Model can be instantiated with coherence config");
    assert(model.coherenceConfig.enabled === true, "Coherence enabled flag set correctly");
    assert(model.coherenceConfig.minThreshold === 0.8, "Coherence threshold set correctly");
    assert(model.coherenceConfig.checkFrequency === 50, "Coherence check frequency set correctly");
    assert(model.coherenceConfig.autoCorrect === true, "Coherence auto-correct setting is correct");
    assert(model.coherenceConfig.strictMode === false, "Coherence strict mode setting is correct");
  }
  ...
  log("\n--- Layer Coherence Tests ---");
  {
    ...
    assert(typeof model.layers[0].getMetrics().coherence === 'number', "Layer provides coherence metric");
  ...
  log("\n--- Coherence Optimizer Tests ---");
  {
    ...
    assert(sgdOptimizer instanceof Prime.Neural.Optimization.CoherenceOptimizer, "SGD is a coherence optimizer");
    assert(sgdOptimizer.learningRate === 0.01, "Learning rate set correctly");
    assert(sgdOptimizer.minCoherence === 0.7, "Minimum coherence threshold set");
    ...
    assert(adamOptimizer instanceof Prime.Neural.Optimization.CoherenceOptimizer, "Adam is a coherence optimizer");
    assert(adamOptimizer.minCoherence === 0.8, "Minimum coherence threshold set in Adam");
  }
  ...
  log("\n--- Neural Architecture Search Coherence Tests ---");
  {
    ...
    assert(search instanceof Prime.Neural.Model.NeuralArchitectureSearch, "Architecture search can be instantiated");
    assert(search.coherenceConfig.minCoherence === 0.8, "Coherence threshold set in search");
    assert(search.coherenceConfig.coherenceWeight === 0.5, "Coherence weight properly set");
    assert(search.coherenceConfig.enforceCoherence === true, "Coherence enforcement correctly set");
    ...
    assert(coherenceEstimate >= 0 && coherenceEstimate <= 1, "Coherence score is in valid range");
    
    const enhancedArchitecture = search._enhanceArchitectureCoherence(architecture, coherenceEstimate);
    assert(typeof enhancedArchitecture === 'object', "Architecture enhancement works");
    assert(enhancedArchitecture.coherence.enabled === true, "Enhanced architecture has coherence enabled");
    assert(enhancedArchitecture.coherence.minThreshold === search.coherenceConfig.minCoherence, 
  ...
  log("\n--- Model Update Coherence Tests ---");
  {
    ...
    assert(typeof model.metrics.learningRate === 'number', "Model tracks learning rate");
    ...
    assert(typeof model.metrics.coherenceScore === 'number', "Model tracks coherence score");
    assert(Array.isArray(model.metrics.layerCoherenceScores), "Model tracks layer coherence scores");
  }
  ...
  log("\n--- Coherence Visualization Tests ---");
  {
    ...
    assert(typeof vizData.metadata === 'object', "Visualization data has metadata");
    assert(typeof vizData.metadata.coherenceScore === 'number', "Visualization includes coherence score");
    
    // Check node coherence values
    for (const node of vizData.nodes) {
      if (node.id !== 'input') {
        assert(typeof node.coherence === 'number', `Node ${node.id} has coherence value`);
      }
    }
  }
