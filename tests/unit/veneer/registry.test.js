...
describe('Veneer Registry Component', () => {
  ...
  describe('Initialization', () => {
    ...
    it('should initialize with custom configuration', () => {
      ...
      assert.deepStrictEqual(customRegistry.options.appDirectories, ['/custom/dir1', '/custom/dir2']);
    });
  });
  ...
  describe('App Registration', () => {
    ...
    it('should register apps', () => {
      ...
      assert.deepStrictEqual(registry.apps.get('test-app-1').manifest, manifest1);
      assert.deepStrictEqual(registry.apps.get('test-app-2').manifest, manifest2);
    });
  ...
  describe('App Management', () => {
    ...
    it('should unregister specific app version', () => {
      ...
      registry.registerApp(appManifestV2, '/tmp/prime-apps/test-app-v2', { force: true });
  ...
  describe('Version Management', () => {
    it('should handle version-specific app registration and retrieval', () => {
      ...
      registry.registerApp(appV1Manifest, '/tmp/prime-apps/app-v1', { force: true });
      registry.registerApp(appV2Manifest, '/tmp/prime-apps/app-v2', { force: true });
      
      // Verify both versions are tracked
      assert.ok(registry.versions.has('versioned-app'), 'Should have versioned-app');
      const versions = registry.versions.get('versioned-app');
      assert.strictEqual(versions.length, 2, 'Should have 2 versions');
      assert.strictEqual(versions[0], '2.0.0', 'Latest version should be first'); 
      assert.strictEqual(versions[1], '1.0.0', 'Older version should be second');
  ...
  describe('App Discovery and Querying', () => {
    beforeEach(() => {
      ...
      appManifests.forEach((manifest, index) => {
        registry.registerApp(manifest, `/tmp/prime-apps/app${index+1}`, { force: true });
      });
    });
    ...
    it('should find interface providers', () => {
      ...
      appManifests.forEach((manifest, index) => {
        registry.registerApp(manifest, `/tmp/prime-apps/interface-app${index}`, { force: true });
      });
    ...
    it('should resolve dependencies', () => {
      ...
      appManifests.forEach((manifest, index) => {
        registry.registerApp(manifest, `/tmp/prime-apps/dep-app${index}`, { force: true });
      });
