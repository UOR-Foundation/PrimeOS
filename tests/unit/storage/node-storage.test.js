...
describe('Node.js Storage Provider', () => {
  ...
  beforeAll(() => {
    if (!isNode) {
      console.warn('Skipping Node.js Storage Provider tests in non-Node environment');
      return;
    }
  ...
  describe('Swap Space Management', () => {
    ...
    test('should offload data to swap when memory limit is reached', async () => {
      ...
      const largeData = Array(100000).fill(0).map((_, i) => ({
        ...
        values: Array(20).fill(0).map(() => Math.random()),
        timestamp: Date.now(),
        metadata: {
          category: i % 5,
      ...
      const swapFiles = fs.readdirSync(testDir).filter(file => 
        file.startsWith('swap-') || file.includes('swap')
      );
      ...
      expect(retrieved[50000].metadata.tags).toEqual(largeData[50000].metadata.tags);
    });
    ...
    test('should automatically clean up swap files when data is deleted', async () => {
      ...
      if (manager.swapSpace && typeof manager.swapSpace.flushToDisk === 'function') {
