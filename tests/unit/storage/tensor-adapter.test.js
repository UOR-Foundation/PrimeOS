...
describe("Swappable Tensor", () => {
  ...
  beforeEach(async () => {
    ...
    swappableTensor = await Prime.Storage.createSwappableTensor(
      storageManager,
      id,
      {
  ...
  test("should support tensor contraction", async () => {
    ...
    const smallTensor = await swappableTensor.getSubtensor([
      [0, 3],
      [0, 3],
      [0, 3],
    ]);
  ...
  test("should transpose tensor dimensions", async () => {
    ...
    const asymSwappableTensor = await Prime.Storage.createSwappableTensor(
      storageManager,
      asymId,
    );
  ...
  test("should calculate matrix multiplication along last dimensions", async () => {
    ...
    const swappableA = await Prime.Storage.createSwappableTensor(
      storageManager,
      idA,
    );
    const swappableB = await Prime.Storage.createSwappableTensor(
      storageManager,
      idB,
    );
