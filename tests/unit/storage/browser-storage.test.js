...
describe("Browser Storage Provider", () => {
  ...
  beforeAll(() => {
    if (!isBrowser) {
      console.warn(
        "Skipping Browser Storage Provider tests in non-browser environment",
      );
    ...
    test("should handle storage limits gracefully", async () => {
      ...
      if (!isBrowser) {
        ...
        mockLocalStorage.setItem = function (key, value) {
          ...
          if (
            Object.keys(this.store).length >= 3 &&
      ...
      try {
        ...
        for (let i = 0; i < 3; i++) {
          try {
            ...
            console.log(
              `Item limit-test-${i} not found, may be due to storage limit`,
            );
          }
        }
      } finally {
  ...
  describe("Storage Utilities", () => {
    test("should provide browser storage utility functions", () => {
      ...
      expect(typeof Prime.Storage.browser.utils.getStorageUsage).toBe(
        "function",
      );
