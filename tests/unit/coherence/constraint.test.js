...
describe('Coherence Constraints', () => {
  describe('Constraint Creation', () => {
    ...
    test('should create hard constraints', () => {
      const hardConstraint = Prime.coherence.createConstraint(
        (x) => x >= 0,
        { name: "nonNegative", type: "hard" }
      );
  ...
  describe('Constrained State', () => {
    ...
    test('should calculate coherence norm', () => {
      ...
      const constraints = [
        Prime.coherence.createConstraint(
          (state) => state.count >= 0,
          { name: "nonNegativeCount", weight: 2, type: "soft" }
        ),
        Prime.coherence.createConstraint(
          (state) => state.count <= 100,
          { name: "maxCount", weight: 1, type: "soft" }
        ),
      ];
  ...
  describe('Constraint-based Optimization', () => {
    ...
    test('should respect maximum iterations', () => {
      ...
      expect(gradientCalls).toBeLessThanOrEqual(optimizationConfig.maxIterations);
