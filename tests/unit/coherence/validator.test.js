...
describe('Coherence Validator', () => {
  ...
  describe('Validation Options', () => {
    ...
    test('should use the specified tolerance for numeric validations', () => {
      const tolerantValidator = new CoherenceValidator({ numericalTolerance: 1e-2 });
      const strictValidator = new CoherenceValidator({ numericalTolerance: 1e-12 });
      
      // A large number - using the validators to check if it's "reasonably sized"
      const largeNumber = 1e8;
      
      expect(tolerantValidator.validate('numeric', largeNumber).valid).toBe(true);
...
describe('Coherence Constraints', () => {
  describe('Numeric Constraints', () => {
    test('should validate finiteness', () => {
      ...
      expect(CoherenceConstraints.numeric.finiteness.validator(Infinity)).toBe(false);
      expect(CoherenceConstraints.numeric.finiteness.validator(NaN)).toBe(false);
    });
  ...
  describe('Vector Constraints', () => {
    test('should validate array elements', () => {
      expect(CoherenceConstraints.vectorSpace.arrayElements.validator([1, 2, 3])).toBe(true);
      expect(CoherenceConstraints.vectorSpace.arrayElements.validator([1, 'string', 3])).toBe(false);
    });
  ...
  describe('Matrix Constraints', () => {
    test('should validate matrix structure', () => {
      expect(CoherenceConstraints.matrixAlgebra.matrixStructure.validator([
        [1, 2],
        [3, 4],
      ])).toBe(true);
    ...
    test('should validate numeric values in matrix', () => {
      ...
      const result = CoherenceConstraints.matrixAlgebra.matrixStructure.validator([
        [1, 2],
        [3, NaN], // Contains NaN
      ]);
