...
describe('Clifford Algebra', () => {
  describe('Algebra Creation', () => {
    ...
    test('throws for negative dimension', () => {
      expect(() => Prime.Clifford.create({ dimension: -1 }))
        .toThrow(Prime.ValidationError);
    });

    test('throws for mismatched signature length', () => {
      expect(() => Prime.Clifford.create({ dimension: 3, signature: [1, 1] }))
        .toThrow(Prime.ValidationError);
    });
  ...
  describe('Multivector Creation', () => {
    ...
    test('preserves irrational values in fromArray', () => {
      ...
      Assertions.assertExtremePrecision(preciseArray.components[1]["e1"], Math.PI);
      Assertions.assertExtremePrecision(preciseArray.components[1]["e2"], Math.E);
      Assertions.assertExtremePrecision(preciseArray.components[1]["e3"], Math.sqrt(2));
    });
    ...
    test('safely handles extreme values', () => {
      ...
      Assertions.assertExtremePrecision(safeBivector.components[2]["e1e2"], safeValues[0]);
      Assertions.assertExtremePrecision(safeBivector.components[2]["e1e3"], safeValues[1]);
      Assertions.assertExtremePrecision(safeBivector.components[2]["e2e3"], safeValues[2]);
    });
  ...
  describe('Multivector Operations', () => {
    ...
    describe('Norm and Utility Methods', () => {
      ...
      test('scalarValue throws for non-scalar', () => {
        expect(() => Cl3.vector([1, 2, 3]).scalarValue())
          .toThrow(Prime.MathematicalError);
      });
    });
