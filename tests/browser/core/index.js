...
function loadCoreTests() {
  ...
  TestRunner.suite("Core Module", function () {
    TestRunner.test("Prime core module is loaded and accessible", function () {
      TestRunner.assert(
        ...
        "Prime global object should exist",
      );
      TestRunner.assert(
        typeof Prime.version === "string",
        "Prime.version should be a string",
      );
      ...
      coreComponents.forEach((component) => {
        TestRunner.assert(
          typeof Prime[component] !== "undefined",
          `Prime.${component} should exist`,
        );
      });
    });
  });
  ...
  TestRunner.suite("Core EventBus", function () {
    TestRunner.test("EventBus exists and has core functions", function () {
      TestRunner.assert(
        typeof Prime.EventBus === "object",
        "EventBus should be an object",
      );
      TestRunner.assert(
        typeof Prime.EventBus.subscribe === "function",
        "EventBus should have subscribe function",
      );
      TestRunner.assert(
        typeof Prime.EventBus.publish === "function",
        "EventBus should have publish function",
      );
    });
    ...
    TestRunner.test("publishes events to subscribed handlers", function () {
      ...
      TestRunner.assertEqual(
        passedData.value,
        42,
        "Handler should receive event data",
      );
    });
  });
  ...
  TestRunner.suite("Core ModuleLoader", function () {
    TestRunner.test("ModuleLoader exists and detects environment", function () {
      TestRunner.assert(
        typeof Prime.ModuleLoader === "object",
        "ModuleLoader should be an object",
      );
