...
TestRunner.suite("Core EventBus", function () {
  ...
  TestRunner.test("only calls handlers for the published event", function () {
    ...
    TestRunner.assert(
      !handler2Called,
      "Handler for event2 should not be called",
    );
  });

  TestRunner.test(
    "unsubscribe stops handler from receiving events",
    function () {
      beforeTest();
  ...
  TestRunner.test("unsubscribe returns true if handler was found", function () {
    ...
    TestRunner.assertEqual(
      result,
      true,
      "unsubscribe should return true when handler is found",
    );
  });

  TestRunner.test(
    "unsubscribe returns false if handler was not found",
    function () {
      beforeTest();
      ...
      TestRunner.assertEqual(
        result,
        false,
        "unsubscribe should return false when handler is not found",
      );
    },
  );
  ...
  TestRunner.test("clear removes all handlers for specific event", function () {
    ...
    TestRunner.assert(
      !handler1Called,
      "Handler for cleared event should not be called",
    );
    TestRunner.assert(
      handler2Called,
      "Handler for other event should still be called",
    );
  });
  ...
  TestRunner.test(
    "errors in handlers do not affect other handlers",
    function () {
      beforeTest();
