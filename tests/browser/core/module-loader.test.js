...
TestRunner.suite('Core ModuleLoader', function() {
  ...
  function afterTests() {
    ...
    if (Prime && Prime.ModuleLoader) {
      testModules.forEach(name => {
        if (Prime[name] !== undefined && typeof Prime.ModuleLoader.unregister === 'function') {
  ...
  TestRunner.test('registers a module successfully', function() {
    ...
    TestRunner.assert(result === true, "Register should return true on success");
    TestRunner.assert(Prime[moduleName] === mockModule, "Module should be attached to Prime");
  });
  ...
  TestRunner.test('validates module name', function() {
    ...
    try {
      ...
      errorThrown = true;
      TestRunner.assert(error.message.includes("Module name"), "Error should mention module name");
    }
    
    TestRunner.assert(errorThrown, "Should throw error for invalid module name");
  ...
  TestRunner.test('validates module object', function() {
    ...
    try {
      ...
      errorThrown = true;
      TestRunner.assert(error.message.includes("Module"), "Error should mention module");
    }
  ...
  TestRunner.test('prevents overwriting existing modules', function() {
    ...
    TestRunner.assertEqual(result, false, "Should return false when module exists");
    
    // Module should still have original value
    TestRunner.assert(Prime[moduleName].first === true, "Module should not be overwritten");
    TestRunner.assert(Prime[moduleName].second === undefined, "Module should not be overwritten");
  });
  ...
  if (Prime.ModuleLoader.require) {
    TestRunner.test('loads an existing module with require', function() {
      ...
      TestRunner.assertEqual(module.value, 42, "Required module should have correct value");
    });
    ...
    TestRunner.test('throws for non-existent module with require', function() {
      ...
      try {
        ...
        errorThrown = true;
        TestRunner.assert(error.message.includes(nonExistentName), "Error should mention module name");
      }
      
      TestRunner.assert(errorThrown, "require should throw for non-existent module");
    });
  } else {
  ...
  if (typeof Prime.ModuleLoader.unregister === 'function') {
    TestRunner.test('unregisters an existing module', function() {
      ...
      TestRunner.assertEqual(result, true, "unregister should return true on success");
      TestRunner.assert(Prime[moduleName] === undefined, "Module should be removed from Prime");
    });
    
    TestRunner.test('returns false for non-existent module with unregister', function() {
      const nonExistentName = 'nonExistentModule' + Date.now();
      
      const result = Prime.ModuleLoader.unregister(nonExistentName);
      TestRunner.assertEqual(result, false, "unregister should return false for non-existent module");
    });
  } else {
    TestRunner.test('ModuleLoader.unregister implementation check', function() {
      TestRunner.log('unregister method not implemented - skipping related tests');
      TestRunner.assert(true); // Auto-pass
    });
  }
  ...
  if (typeof Prime.ModuleLoader.getModules === 'function') {
    TestRunner.test('returns list of registered modules', function() {
      ...
      TestRunner.assert(Array.isArray(modules), "getModules should return an array");
      TestRunner.assert(modules.includes(moduleName1), "Module list should include first test module");
      TestRunner.assert(modules.includes(moduleName2), "Module list should include second test module");
    });
  } else {
    TestRunner.test('ModuleLoader.getModules implementation check', function() {
      TestRunner.log('getModules method not implemented - skipping related tests');
      TestRunner.assert(true); // Auto-pass
    });
  }
