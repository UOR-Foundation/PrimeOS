...
function testMemoryOptimization() {
  ...
  passed += runTest("In-place operation memory efficiency", () => {
    ...
    for (let i = 0; i < 100; i++) {
      // Just check first 100 for speed
      if (result[i] !== 3) {
...
function runAllTests() {
  ...
  console.log(
    `- New functionality: ${newFunctionalityWorks ? "✅ PASS" : "❌ FAIL"}`,
  );
  console.log(
    `- Memory optimization: ${memoryOptimized ? "✅ PASS" : "❌ FAIL"}`,
  );

  const allPassed =
    backwardCompatible && newFunctionalityWorks && memoryOptimized;
  console.log(
