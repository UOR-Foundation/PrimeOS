    ...
    fn debug_resonance_class_count() {
        ...
        for klein_base in 0u8..64 {
            ...
            for &member in &members {
                let word = BitWord::from_u8(member);
                let r = word.r(&alpha);
                if r < min_res {
                    min_res = r;
                    min_member = member;
        ...
        println!("\n=== Analysis ===");
        println!(
            "We have {} unique resonance values",
            sorted_resonances.len()
        );
        println!(
            "Average Klein groups per resonance: {}",
            64.0 / sorted_resonances.len() as f64
        );
        ...
        for (resonance, klein_groups) in sorted_resonances.iter().take(5) {
            ...
            for &kg in klein_groups {
                let members = [kg, kg | 0b01000000, kg | 0b10000000, kg | 0b11000000];
                for m in &members {
                    let r = m.r(&alpha);
                    println!("  {:08b} -> {:.6}", m, r);
                }
