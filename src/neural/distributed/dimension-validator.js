...
Prime.Neural.Distributed.DimensionValidator = {
  ...
  validateModelConfig(config) {
    ...
    if (!Number.isInteger(config.inputSize) || config.inputSize <= 0) {
      throw new Prime.ValidationError(
        `Invalid input size: ${config.inputSize}. Must be a positive integer.`,
      );
    }
  ...
  validateLayerConfig(layerConfig, index) {
    if (!layerConfig) {
      throw new Prime.ValidationError(
        `Layer configuration at index ${index} is undefined`,
      );
    }

    // Validate outputSize (required)
    if (
      !Number.isInteger(layerConfig.outputSize) ||
      layerConfig.outputSize <= 0
    ) {
      throw new Prime.ValidationError(
    ...
    if (
      layerConfig.inputSize !== undefined &&
    ...
    for (let i = 0; i < model.layers.length; i++) {
      ...
      if (!Number.isInteger(layer.inputSize) || layer.inputSize <= 0) {
        if (Prime.Logger && Prime.Logger.error) {
          Prime.Logger.error(
            `Layer ${i} has invalid inputSize: ${layer.inputSize}`,
          );
        }
        return false;
      }
      ...
      if (!Number.isInteger(layer.outputSize) || layer.outputSize <= 0) {
        if (Prime.Logger && Prime.Logger.error) {
          Prime.Logger.error(
            `Layer ${i} has invalid outputSize: ${layer.outputSize}`,
          );
        }
        return false;
      }
      ...
      if (layer.weights) {
        if (layer.weights.length !== layer.outputSize) {
          if (Prime.Logger && Prime.Logger.error) {
            Prime.Logger.error(
              `Layer ${i} weights rows (${layer.weights.length}) don't match outputSize (${layer.outputSize})`,
            );
          }
          return false;
        }
        ...
        if (layer.weights[0].length !== layer.inputSize) {
          if (Prime.Logger && Prime.Logger.error) {
            Prime.Logger.error(
              `Layer ${i} weights columns (${layer.weights[0].length}) don't match inputSize (${layer.inputSize})`,
            );
          }
          return false;
        }
      ...
      if (layer.biases && layer.biases.length !== layer.outputSize) {
        if (Prime.Logger && Prime.Logger.error) {
          Prime.Logger.error(
            `Layer ${i} biases length (${layer.biases.length}) doesn't match outputSize (${layer.outputSize})`,
          );
        }
        return false;
      }
      ...
      if (i < model.layers.length - 1) {
        ...
        if (layer.outputSize !== nextLayer.inputSize) {
          if (Prime.Logger && Prime.Logger.error) {
            Prime.Logger.error(
              `Dimension mismatch between layers ${i} and ${i + 1}: ${layer.outputSize} != ${nextLayer.inputSize}`,
            );
          }
          return false;
        }
