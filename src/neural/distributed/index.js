...
(function () {
  ...
  class DistributedNeuralModel extends Prime.Neural.Model.NeuralModel {
    ...
    _distributedForward(input, options = {}) {
      ...
      switch (this.partitionScheme.type) {
        case Prime.Distributed.Partition.PartitionType.DATA_PARALLEL:
          ...
          return this._dataParallelForward(input, layerActivations, layerCaches, isTraining);
          
        case Prime.Distributed.Partition.PartitionType.LAYER_WISE:
          // In layer-wise mode, each layer runs on a different node
          return this._layerWiseForward(input, layerActivations, layerCaches, isTraining);
          
        case Prime.Distributed.Partition.PartitionType.INTRA_LAYER:
          // In intra-layer mode, layers are split internally across nodes
          return this._intraLayerForward(input, layerActivations, layerCaches, isTraining);
    ...
    _dataParallelForward(input, layerActivations, layerCaches, isTraining) {
      ...
      if (!clusterManager || !input) {
        ...
        return this._localForward(input, layerActivations, layerCaches, isTraining);
      }
      ...
      try {
        ...
        Prime.Logger.debug(`Data-parallel forward: distributing batch of ${batchSize} across ${nodesCount} nodes`, {
          samplesPerNode,
          training: isTraining
        });
        ...
        for (let nodeIndex = 0; nodeIndex < nodesCount; nodeIndex++) {
          ...
          const task = {
            ...
            data: {
              modelConfig: {
                layers: this.layers.map(layer => this._serializeLayerConfig(layer)),
                isTraining
              },
              input: nodeInput,
          ...
          tasks.push(
            clusterManager.submitTask(task, [nodeId])
              .then(result => {
              ...
              .catch(error => {
                ...
                Prime.Logger.error(`Data-parallel forward task failed on node ${nodeId}`, { 
                  error: error.message 
                });
        ...
        return Promise.all(tasks)
          .then(() => {
            ...
            return this._combineDataParallelResults(validResults, layerActivations, layerCaches, batchSize);
          })
          .catch(error => {
            // Handle all tasks failing
            Prime.Logger.error("Data-parallel forward distribution failed, falling back to local", {
              error: error.message
            });
     ...
     */
    _combineDataParallelResults(nodeResults, layerActivations, layerCaches, batchSize) {
      ...
      for (const { result, dataRange } of nodeResults) {
        ...
        if (result.caches && Array.isArray(result.caches)) {
          for (let layerIdx = 0; layerIdx < Math.min(result.caches.length, layerCaches.length); layerIdx++) {
      ...
      if (this.coherenceConfig.enabled) {
        this._verifyDistributedCoherence(finalActivation, "data_parallel_forward");
      }
    ...
    _verifyDistributedCoherence(result, operation) {
      ...
      if (Array.isArray(result)) {
        ...
        if (hasInvalidValues) {
          Prime.Logger.warn(`Coherence violation: NaN or Infinity values in ${operation} result`);
    ...
    _layerWiseForward(input, layerActivations, layerCaches, isTraining) {
      ...
      try {
        ...
        Prime.Logger.debug(`Layer-wise forward: distributing ${this.layers.length} layers across ${activeNodes.length} nodes`, {
        ...
        for (let layerIndex = 0; layerIndex < this.layers.length; layerIndex++) {
          ...
          currentPromise = currentPromise.then(() => {
            ...
            // Submit task to cluster
            return clusterManager.submitTask(task, [nodeId])
              .then(result => {
                ...
                // Verify coherence if enabled
                if (this.coherenceConfig.enabled) {
                  this._verifyDistributedCoherence(result.activation, `layer_wise_forward_${i}`);
                }
              ...
              .catch(error => {
                // Track task failure
                this.distributedState.failedTasks++;
                Prime.Logger.error(`Layer-wise forward task failed for layer ${i} on node ${nodeId}`, {
                  error: error.message
                });
                
                // Fall back to local computation for this layer
                Prime.Logger.warn(`Falling back to local computation for layer ${i}`);
                const result = layer.forward(layerActivations[i]);
    ...
    _intraLayerForward(input, layerActivations, layerCaches, isTraining) {
      ...
      try {
        ...
        Prime.Logger.debug(`Intra-layer forward: distributing layer computations across ${activeNodes.length} nodes`, {
          training: isTraining
        });
     ...
     */
    _processLayerWithIntraParallelism(layer, input, activeNodes, isTraining, layerIndex) {
      const clusterManager = this.distributedConfig.clusterManager;
      ...
      return Promise.all(tasks)
        .then(results => {
          ...
          // Combine results from all nodes
          return this._combineIntraLayerResults(validResults, outputSize, layer);
        })
        .catch(error => {
          // Handle all tasks failing
          Prime.Logger.error("All intra-layer tasks failed, falling back to local", {
            error: error.message
          });
    ...
    _combineIntraLayerResults(results, outputSize, layer) {
      ...
      const isBatch = results[0].activation && Array.isArray(results[0].activation[0]);
      const isMultiDimensional = layer.outputShape && layer.outputShape.length > 1;
      ...
      if (isMultiDimensional) {
        ...
        combinedActivation = this._combineConvLayerActivations(results, layer.outputShape);
    ...
    _combineConvLayerActivations(results, outputShape) {
      ...
      const batchSize = Array.isArray(results[0].activation) ? results[0].activation.length : 1;
      let combined;
    ...
    _dataParallelBackward(dY, forwardResult) {
      ...
      try {
        ...
        const hasDistributedCaches = layerCaches && layerCaches.some(cache => 
        ...
        if (hasDistributedCaches) {
          ...
          for (let layerIndex = this.layers.length - 1; layerIndex >= 0; layerIndex--) {
            ...
            if (!layerCache || !layerCache.dataRanges) {
              ...
              const localBackwardPromise = Promise.resolve().then(() => {
                const layer = this.layers[i];
                const prevGrad = (i === this.layers.length - 1) ? dY : layerGradients[i + 1].dX;
            ...
            for (let partitionIndex = 0; partitionIndex < layerCache.dataRanges.length; partitionIndex++) {
              ...
              if (i === this.layers.length - 1) {
                ...
                nodeGradient = layerGradients[i + 1].dX.slice(dataRange.start, dataRange.end);
              }
              ...
              // Submit task to cluster
              const nodePromise = clusterManager.submitTask(task, [nodeId])
                .then(result => {
                  // Track task completion
                  this.distributedState.completedTasks++;
                ...
                .catch(error => {
                  // Track task failure
                  this.distributedState.failedTasks++;
                  Prime.Logger.error(`Data-parallel backward task failed for layer ${i}`, {
                    error: error.message
                  });
            ...
            const layerPromise = Promise.all(nodePromises)
              .then(() => {
                // Filter out failed results
                const validWeightResults = nodeWeightGradients.filter(r => r && r.dW);
                ...
                const dX = this._combineInputGradients(nodeInputGradients, batchSize);
              ...
              .catch(error => {
                ...
                Prime.Logger.error(`Failed to aggregate gradients for layer ${i}`, {
                  error: error.message
                });
                ...
                try {
                  ...
                  Prime.Logger.error(`Local fallback also failed for layer ${i}`, {
                  ...
                  const inputSize = layer.inputSize || (layer.inputShape ? 
                    layer.inputShape.reduce((a, b) => a * b, 1) : layerActivations[i][0].length);
                    
                  const zeroDX = new Array(batchSize).map(() => new Array(inputSize).fill(0));
          ...
          return Promise.all(layerPromises)
            .then(() => {
              ...
              if (this.coherenceConfig.enabled) {
                this._verifyGradientCoherence(layerGradients, "data_parallel_backward");
              }
    ...
    _layerWiseBackward(dY, forwardResult) {
      ...
      try {
        ...
        Prime.Logger.debug(`Layer-wise backward: distributing ${this.layers.length} layers across ${activeNodes.length} nodes`, {
          usePartialDistribution
        });
            ...
            return clusterManager.submitTask(task, [nodeId])
              .then(result => {
                ...
                if (this.coherenceConfig.enabled) {
                  ...
                  this._verifyGradientCoherence(layerGrads, `layer_wise_backward_${i}`);
                }
              ...
              .catch(error => {
                // Track task failure
                this.distributedState.failedTasks++;
                Prime.Logger.error(`Layer-wise backward task failed for layer ${i} on node ${nodeId}`, {
                  error: error.message
                });
                ...
                const localPrevGrad = (i === this.layers.length - 1) ? dY : layerGradients[i + 1].dX;
    ...
    _intraLayerBackward(dY, forwardResult) {
      ...
      try {
        ...
        Prime.Logger.debug(`Intra-layer backward: distributing gradients across ${activeNodes.length} nodes`);
        ...
        for (let layerIndex = this.layers.length - 1; layerIndex >= 0; layerIndex--) {
          ...
          const prevGradient = (i === this.layers.length - 1) ? dY : layerGradients[i + 1].dX;
          ...
          const layerPromise = (async () => {
            ...
            for (let nodeIndex = 0; nodeIndex < layerCache.nodeResults.length; nodeIndex++) {
              ...
              if (Array.isArray(prevGradient[0])) {
                ...
                nodeGradient = prevGradient.map(row => row.slice(outputStart, outputEnd));
              } else {
              ...
              const nodePromise = clusterManager.submitTask(task, [nodeId])
                ...
                .catch(error => {
                  // Track task failure
                  this.distributedState.failedTasks++;
                  Prime.Logger.error(`Intra-layer backward task failed for layer ${i} on node ${nodeId}`, {
                    error: error.message
                  });
            ...
            const validWeightResults = nodeWeightGradients.filter(r => r && r.dW);
            const validBiasResults = nodeBiasGradients.filter(r => r && r.dB);
            const validInputResults = nodeInputGradients.filter(r => r && r.dX);
            
            if (validWeightResults.length === 0) {
              throw new Error(`All intra-layer backward tasks failed for layer ${i}`);
            }
            ...
            // Verify coherence if enabled
            if (this.coherenceConfig.enabled) {
              this._verifyGradientCoherence([layerGradients[i]], `intra_layer_backward_${i}`);
            }
    ...
    _aggregateParameterGradients(gradients, paramShape) {
      ...
      if (Array.isArray(paramShape[0])) {
        ...
        const result = Array.isArray(paramShape) ? 
          ...
          new Array(gradients[0].length).map(() => new Array(gradients[0][0].length).fill(0));
    ...
    _verifyGradientCoherence(gradients, operation) {
      ...
      gradients.forEach((layerGrad, i) => {
        ...
        if (dW && this._checkForInvalidValues(dW)) {
          Prime.Logger.warn(`Coherence violation: Invalid values in ${operation} weight gradients for layer ${i}`);
          isCoherent = false;
        ...
        if (dB && this._checkForInvalidValues(dB)) {
          Prime.Logger.warn(`Coherence violation: Invalid values in ${operation} bias gradients for layer ${i}`);
          isCoherent = false;
    ...
    _localBackward(dY, forwardResult) {
      ...
      if (!forwardResult) {
        Prime.Logger.warn("Local backward pass received null or undefined forward result");
        const emptyGradients = new Array(this.layers.length).fill(null).map(() => ({
          dWeights: null,
          dBiases: null
        }));
      ...
      if (!dY) {
        Prime.Logger.warn("Local backward pass received null or undefined gradient");
    ...
    async _updateModel(layerGradients) {
      ...
      if (
        ...
        this.metrics.iteration - this.distributedState.lastCoherenceCheck >= this.distributedConfig.coherenceCheckFrequency
      ) {
        try {
    ...
    _distributedUpdateModel(layerGradients) {
      ...
      if (!layerGradients || !Array.isArray(layerGradients)) {
        ...
        if (Prime.Logger && Prime.Logger.warn) {
          Prime.Logger.warn("Missing or invalid layer gradients in distributed update");
        }
      ...
      // Update each layer
      for (let i = 0; i < this.layers.length && i < layerGradients.length; i++) {
        const layer = this.layers[i];
    ...
    async _synchronizeParameters() {
      ...
      try {
        ...
        if (!this.cluster || !this.cluster.manager) {
          Prime.Logger.warn("Cluster manager not available for parameter synchronization");
          return true;
        }
        ...
        for (const nodeId of this.distributedState.activeNodes) {
          ...
          syncPromises.push(
            this.cluster.manager.submitTask(syncTask, [nodeId])
              .catch(error => {
                Prime.Logger.error(`Failed to get parameters from node ${nodeId}`, {
                  error: error.message,
                  iteration: this.metrics.iteration
                });
        ...
        if (validResults.length === 0) {
          Prime.Logger.warn("No valid parameters received from nodes, skipping synchronization");
          return false;
        }
        
        // Perform parameter synchronization based on strategy
        const syncStrategy = this.distributedConfig.synchronizationStrategy || "average";
        let synchronizedParameters;
        
        switch (syncStrategy) {
          case "average":
            synchronizedParameters = this._averageParameters(localParameters, validResults);
            break;
            
          case "weighted_average":
            synchronizedParameters = this._weightedAverageParameters(localParameters, validResults);
            break;
            
          case "coherence_weighted":
            synchronizedParameters = this._coherenceWeightedParameters(localParameters, validResults);
            break;
            
          default:
            Prime.Logger.warn(`Unknown synchronization strategy: ${syncStrategy}, using average`);
            synchronizedParameters = this._averageParameters(localParameters, validResults);
        }
        
        // Verify synchronized parameters for coherence
        const isCoherent = this._verifyParameterCoherence(synchronizedParameters);
        
        if (!isCoherent) {
          Prime.Logger.error("Synchronized parameters failed coherence verification", {
            iteration: this.metrics.iteration,
          ...
          return this._recoverFromSynchronizationFailure(localParameters, validResults);
        }
    ...
    _extractModelParameters() {
      ...
      for (let i = 0; i < this.layers.length; i++) {
        ...
        parameters.weights.push(layer.weights ? JSON.parse(JSON.stringify(layer.weights)) : null);
        parameters.biases.push(layer.biases ? JSON.parse(JSON.stringify(layer.biases)) : null);
    ...
    _applyParameters(parameters) {
      ...
      // Sanity check
      if (parameters.weights.length !== this.layers.length || 
    ...
    _averageParameters(localParameters, remoteParameters) {
      ...
      const result = {
        weights: localParameters.weights.map(w => w ? JSON.parse(JSON.stringify(w)) : null),
        biases: localParameters.biases.map(b => b ? JSON.parse(JSON.stringify(b)) : null),
    ...
    _addParametersToSum(sumParams, paramsToAdd) {
      ...
      for (let l = 0; l < sumParams.weights.length && l < paramsToAdd.weights.length; l++) {
        // Skip if either is null
        if (!sumParams.weights[l] || !paramsToAdd.weights[l]) continue;
        
        // Handle 2D weights (matrices)
        if (Array.isArray(sumParams.weights[l][0]) && Array.isArray(paramsToAdd.weights[l][0])) {
          for (let i = 0; i < sumParams.weights[l].length && i < paramsToAdd.weights[l].length; i++) {
            for (let j = 0; j < sumParams.weights[l][i].length && j < paramsToAdd.weights[l][i].length; j++) {
              sumParams.weights[l][i][j] += paramsToAdd.weights[l][i][j];
            }
          }
        } 
        // Handle 1D weights (vectors)
        else if (Array.isArray(sumParams.weights[l]) && Array.isArray(paramsToAdd.weights[l])) {
          for (let i = 0; i < sumParams.weights[l].length && i < paramsToAdd.weights[l].length; i++) {
        ...
        if (sumParams.biases[l] && paramsToAdd.biases[l]) {
          for (let i = 0; i < sumParams.biases[l].length && i < paramsToAdd.biases[l].length; i++) {
    ...
    _weightedAverageParameters(localParameters, remoteParameters) {
      ...
      for (const params of remoteParameters) {
        ...
        if (typeof metrics.coherenceViolations === 'number' && metrics.coherenceViolations > 0) {
        ...
        if (typeof metrics.processingTime === 'number' && metrics.averageProcessingTime > 0) {
          // Faster nodes get slightly higher weight (small effect)
          const timeRatio = metrics.processingTime / metrics.averageProcessingTime;
      ...
      for (let i = 0; i < remoteParameters.length; i++) {
        this._addWeightedParametersToSum(result, remoteParameters[i], nodeWeights[i + 1]);
      }
    ...
    _addWeightedParametersToSum(sumParams, paramsToAdd, weight) {
      ...
      for (let l = 0; l < sumParams.weights.length && l < paramsToAdd.weights.length; l++) {
        ...
        if (Array.isArray(sumParams.weights[l][0]) && Array.isArray(paramsToAdd.weights[l][0])) {
          for (let i = 0; i < sumParams.weights[l].length && i < paramsToAdd.weights[l].length; i++) {
            for (let j = 0; j < sumParams.weights[l][i].length && j < paramsToAdd.weights[l][i].length; j++) {
              sumParams.weights[l][i][j] += paramsToAdd.weights[l][i][j] * weight;
            }
          }
        } 
        // Handle 1D weights (vectors)
    ...
    _coherenceWeightedParameters(localParameters, remoteParameters) {
      ...
      for (const params of remoteParameters) {
        ...
        if (coherenceMetrics.layerScores && Array.isArray(coherenceMetrics.layerScores)) {
          // Average layer coherence scores
          const layerAverage = coherenceMetrics.layerScores.reduce((sum, score) => sum + score, 0) / 
    ...
    _verifyParameterCoherence(parameters) {
          ...
          for (let i = 0; i < parameters.weights[l].length; i++) {
            for (let j = 0; j < parameters.weights[l][i].length; j++) {
              if (!Number.isFinite(parameters.weights[l][i][j])) {
                Prime.Logger.warn(`Invalid weight value at layer ${l}, position [${i},${j}]`);
                return false;
              }
            }
          }
        ...
        // Check 1D weights
          ...
          for (let i = 0; i < parameters.weights[l].length; i++) {
            if (!Number.isFinite(parameters.weights[l][i])) {
              Prime.Logger.warn(`Invalid weight value at layer ${l}, position [${i}]`);
              return false;
            }
          }
        }
        ...
        if (parameters.biases[l]) {
          for (let i = 0; i < parameters.biases[l].length; i++) {
            if (!Number.isFinite(parameters.biases[l][i])) {
              Prime.Logger.warn(`Invalid bias value at layer ${l}, position [${i}]`);
              return false;
            }
          }
        }
        ...
        if (Array.isArray(parameters.weights[l][0])) {
          for (let i = 0; i < parameters.weights[l].length; i++) {
            for (let j = 0; j < parameters.weights[l][i].length; j++) {
              if (Math.abs(parameters.weights[l][i][j]) > maxAllowedValue) {
                Prime.Logger.warn(`Excessively large weight at layer ${l}, position [${i},${j}]`);
                return false;
              }
            }
          }
          ...
          for (let i = 0; i < parameters.weights[l].length; i++) {
            if (Math.abs(parameters.weights[l][i]) > maxAllowedValue) {
              Prime.Logger.warn(`Excessively large weight at layer ${l}, position [${i}]`);
              return false;
            }
          }
        }
        ...
        if (parameters.biases[l]) {
          for (let i = 0; i < parameters.biases[l].length; i++) {
            if (Math.abs(parameters.biases[l][i]) > maxAllowedValue) {
              Prime.Logger.warn(`Excessively large bias at layer ${l}, position [${i}]`);
              return false;
            }
          }
        }
      ...
      for (let l = 0; l < parameters.weights.length && l < localParameters.weights.length; l++) {
        if (!parameters.weights[l] || !localParameters.weights[l]) continue;
        
        // Check 2D weights
        if (Array.isArray(parameters.weights[l][0]) && Array.isArray(localParameters.weights[l][0])) {
          for (let i = 0; i < parameters.weights[l].length && i < localParameters.weights[l].length; i++) {
            for (let j = 0; j < parameters.weights[l][i].length && j < localParameters.weights[l][i].length; j++) {
              const localVal = localParameters.weights[l][i][j];
              const syncVal = parameters.weights[l][i][j];
              
              if (localVal !== 0 && Math.abs(syncVal / localVal) > maxDivergenceFactor) {
                Prime.Logger.warn(`Excessive parameter divergence at layer ${l}, weight [${i},${j}]`);
                return false;
              }
            }
          }
        ...
        // Check 1D weights
        else if (Array.isArray(parameters.weights[l]) && Array.isArray(localParameters.weights[l])) {
          for (let i = 0; i < parameters.weights[l].length && i < localParameters.weights[l].length; i++) {
            const localVal = localParameters.weights[l][i];
            const syncVal = parameters.weights[l][i];
            
            if (localVal !== 0 && Math.abs(syncVal / localVal) > maxDivergenceFactor) {
              Prime.Logger.warn(`Excessive parameter divergence at layer ${l}, weight [${i}]`);
              return false;
            }
          }
        }
     ...
     */
    async _recoverFromSynchronizationFailure(localParameters, remoteParameters) {
      // Get recovery strategy from config
      const recoveryStrategy = this.distributedConfig.syncRecoveryStrategy || "local_fallback";
      
      Prime.Logger.warn(`Recovering from synchronization failure using ${recoveryStrategy} strategy`, {
        iteration: this.metrics.iteration
      });
      ...
      switch (recoveryStrategy) {
        case "local_fallback":
          ...
          Prime.Logger.info("Using local parameters as fallback after synchronization failure");
          return true;
          
        case "most_coherent":
          // Find the most coherent node and use its parameters
          return await this._recoverWithMostCoherentNode(localParameters, remoteParameters);
          
        case "conservative_merge":
          // Keep local parameters for unstable layers, use synchronized for stable ones
          return this._recoverWithConservativeMerge(localParameters, remoteParameters);
          
        default:
          Prime.Logger.warn(`Unknown recovery strategy: ${recoveryStrategy}, using local fallback`);
          return true;
      }
    }
    ...
    async _recoverWithMostCoherentNode(localParameters, remoteParameters) {
      ...
      if (bestParams && bestCoherence > 0.5) {
        Prime.Logger.info(`Recovered synchronization using parameters with coherence ${bestCoherence.toFixed(3)}`);
        this._applyParameters(bestParams);
        return true;
      }
      
      // No suitable parameters found
      Prime.Logger.warn("Could not find coherent parameters for recovery, using local parameters");
      return false;
    }
    ...
    _recoverWithConservativeMerge(localParameters, remoteParameters) {
      try {
        ...
        const conservativeParams = this._averageParameters(localParameters, remoteParameters);
        ...
        for (let l = 0; l < conservativeParams.weights.length; l++) {
          if (!this._checkLayerCoherence(conservativeParams, l)) {
            ...
            Prime.Logger.info(`Using local parameters for layer ${l} due to coherence issues`);
            conservativeParams.weights[l] = localParameters.weights[l] ? 
              JSON.parse(JSON.stringify(localParameters.weights[l])) : null;
            conservativeParams.biases[l] = localParameters.biases[l] ? 
              JSON.parse(JSON.stringify(localParameters.biases[l])) : null;
          }
        }
        ...
        Prime.Logger.info("Applied conservative merge of parameters for recovery");
        return true;
    ...
    _checkLayerCoherence(parameters, layerIndex) {
      if (!parameters || !parameters.weights || !parameters.biases || 
          layerIndex < 0 || layerIndex >= parameters.weights.length) {
        return false;
      }
      ...
      if (Array.isArray(weights[0])) {
        ...
        for (let i = 0; i < weights.length; i++) {
          for (let j = 0; j < weights[i].length; j++) {
            if (!Number.isFinite(weights[i][j]) || Math.abs(weights[i][j]) > 1e6) {
              return false;
            }
          }
        }
      ...
      try {
        ...
        for (const nodeId of this.distributedState.activeNodes) {
          ...
          broadcastPromises.push(
            this.cluster.manager.submitTask(broadcastTask, [nodeId])
              .catch(error => {
                Prime.Logger.error(`Failed to send parameters to node ${nodeId}`, {
                  error: error.message,
                  iteration: this.metrics.iteration
                });
        ...
        const isSuccessful = successCount >= Math.ceil(this.distributedState.activeNodes.length / 2);
        
        if (isSuccessful) {
          Prime.Logger.info(`Successfully synchronized parameters to ${successCount} of ${this.distributedState.activeNodes.length} nodes`);
        } else {
          Prime.Logger.warn(`Parameter synchronization partially failed, only ${successCount} of ${this.distributedState.activeNodes.length} nodes updated`);
        }
    ...
    async _distributedCoherenceCheck() {
      ...
      if (!this.cluster || !this.cluster.manager || this.distributedState.activeNodes.length === 0) {
      ...
      try {
        ...
        for (const nodeId of this.distributedState.activeNodes) {
          ...
          // Submit task to node and collect promise
          checkPromises.push(
            this.cluster.manager.submitTask(checkTask, [nodeId])
              .catch(error => {
                Prime.Logger.error(`Failed to perform coherence check on node ${nodeId}`, {
                  error: error.message,
                  iteration: this.metrics.iteration
                });
        ...
        if (validResults.length === 0) {
          Prime.Logger.warn("No valid coherence check results received from nodes");
          return localCoherence;
        }
          ...
          if (result.violations && Array.isArray(result.violations)) {
            ...
            for (const violation of result.violations) {
              if (violation.severity === 'high') {
                Prime.Logger.warn(`High severity coherence violation on node: ${violation.message || violation.type}`);
        ...
        const averageCoherence = validResults.length > 0 ? 
          globalCoherence / validResults.length : 0;
        
        // Update global coherence metrics
        this.metrics.globalCoherence = (this.metrics.globalCoherence || 1.0) * 0.7 + averageCoherence * 0.3;
    ...
    getDistributedStatus() {
      return {
        ...
        synchronizedIterations: this.distributedState.synchronizedIterations || 0,
        ...
        failedSynchronizations: this.distributedState.failedSynchronizations || 0,
        globalCoherence: this.metrics.globalCoherence || 1.0,
        coherenceViolations: this.coherenceViolations ? this.coherenceViolations.length : 0,
        lastCoherenceCheck: this.distributedState.lastCoherenceCheck || 0,
        partitionScheme: this.partitionScheme ? this.partitionScheme.type : null,
        networkPartitions: this.distributedState.networkPartitions || 0,
        syncStrategy: this.distributedConfig.synchronizationStrategy || "average",
        recoveryStrategy: this.distributedConfig.syncRecoveryStrategy || "local_fallback"
      };
    }
  }
  ...
  class DistributedOptimizer {
    ...
    update(parameters, gradients, coherenceFn) {
      ...
      const updatedParameters = this.baseOptimizer.update(parameters, gradients, coherenceFn);
    ...
    synchronize(localParameters, remoteParameters) {
      ...
      switch (this.config.synchronizationStrategy) {
        ...
        case "weighted_average":
          return this._weightedAverageParameters(localParameters, remoteParameters);
        
        case "coherence_weighted":
          return this._coherenceWeightedParameters(localParameters, remoteParameters);
  ...
  class DistributedLayer {
    ...
    constructor(baseLayer, config = {}) {
      ...
      Object.defineProperty(this, "weights", {
        ...
        set: (value) => { this.baseLayer.weights = value; }
      });
      
      Object.defineProperty(this, "biases", {
        get: () => this.baseLayer.biases,
        set: (value) => { this.baseLayer.biases = value; }
      });
      
      // Other properties to forward from base layer
      ["inputSize", "outputSize", "activation", "returnSequences", 
       "hiddenSize", "sequenceLength", "inputShape", "outputShape", 
       "filters", "kernelSize", "strides", "padding"].forEach(prop => {
        if (prop in baseLayer) {
          Object.defineProperty(this, prop, {
            get: () => this.baseLayer[prop],
            set: (value) => { this.baseLayer[prop] = value; }
          });
        }
      });
