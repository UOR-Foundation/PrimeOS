...
(function () {
  ...
  class TrainingLoop {
    ...
    async train(x, y, options = {}) {
      ...
      if (config.verbose) {
        console.log(
          `Training on ${trainX.length} samples, batch size: ${config.batchSize}`,
        );
        if (valX) {
      ...
      for (let epoch = 0; epoch < config.epochs; epoch++) {
        ...
        for (
          ...
          if (batchResult.metric !== null) {
            epochMetric =
              epochMetric === null
                ? batchResult.metric * (batchEnd - batchStart)
                : epochMetric + batchResult.metric * (batchEnd - batchStart);
          }
    ...
    _shuffleData(x, y) {
      ...
      const shuffledX = Array.isArray(x)
        ? []
        : x instanceof Float32Array
          ? new Float32Array(x.length)
          : new Float64Array(x.length);
      const shuffledY = Array.isArray(y)
        ? []
    ...
    _shouldEarlyStop() {
      ...
      return (
        this.state.earlyStoppingPatience >= this.earlyStoppingConfig.patience
      );
    }
      ...
      switch (type) {
        ...
        case "plateau":
          ...
          this.lrScheduler = (epoch, currentLR, metrics) => {
            ...
            if (
              monitor <
              this.plateauConfig.bestValue - this.plateauConfig.minDelta
            ) {
              this.plateauConfig.bestValue = monitor;
              ...
              if (this.plateauConfig.wait >= this.plateauConfig.patience) {
                const newLR = Math.max(
                  currentLR * this.plateauConfig.factor,
                  this.plateauConfig.minLR,
                );
                this.plateauConfig.wait = 0;
                this.plateauConfig.cooldownCounter =
                  this.plateauConfig.cooldown;
                return newLR;
              }
            }
