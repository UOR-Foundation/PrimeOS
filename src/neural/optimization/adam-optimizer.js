...
(function () {
  ...
  class AdamOptimizer {
        ...
        if (Array.isArray(paramValue)) {
          if (Array.isArray(paramValue[0])) {
            ...
            if (this.useTypedArrays && typeof Float32Array !== 'undefined') {
              ...
              Object.defineProperty(this.m[paramName], '_flatArray', { value: flatM });
              Object.defineProperty(this.v[paramName], '_flatArray', { value: flatV });
              if (this.amsgrad) {
                Object.defineProperty(this.vMax[paramName], '_flatArray', { value: flatVMax });
              }
            } else {
              // Standard arrays
              this.m[paramName] = new Array(rows);
    ...
    _applyWeightDecay(gradients, parameters) {
      ...
      for (const paramName in gradients) {
        if (gradients.hasOwnProperty(paramName) && parameters.hasOwnProperty(paramName)) {
          const grad = gradients[paramName];
    ...
    update(parameters, gradients, options = {}) {
      ...
      this.metrics.meanGradientNorm = 0.9 * this.metrics.meanGradientNorm + 0.1 * gradientNorm;
      ...
      const updatedParams = inPlace ? parameters : JSON.parse(JSON.stringify(parameters));
      
      // Update each parameter
      for (const paramName in gradients) {
        if (gradients.hasOwnProperty(paramName) && updatedParams.hasOwnProperty(paramName)) {
          const grad = gradients[paramName];
          ...
          if (Array.isArray(grad)) {
            ...
            // Compute bias-corrected second raw moment estimate
            const vHat = (this.amsgrad ? this.vMax[paramName] : this.v[paramName]) / bias2;
    ...
    calculateCoherence() {
      ...
      const normalizedGradientNorm = Math.min(1.0, this.metrics.meanGradientNorm / 10.0);
      ...
      const normalizedSecondMoment = Math.min(1.0, this.metrics.maxSecondMoment / 100.0);
