      ...
      try {
        ...
        if (options.strict && modelData.serializationInfo) {
          ...
          if (currentVersion !== serializedVersion) {
            console.warn(
              `Model was serialized with version ${serializedVersion}, ` +
    ...
    static applyWeights(model, weights, validateStructure = true) {
      ...
      if (validateStructure) {
        if (weights.layers.length !== model.layers.length) {
          throw new Error(
            `Weights structure doesn't match model: expected ${model.layers.length} layers, got ${weights.layers.length}`,
          );
        }
        ...
        for (let i = 0; i < model.layers.length; i++) {
          ...
          if (Array.isArray(modelLayer.weights[0])) {
            if (
              modelLayer.weights.length !== weightLayer.weights.length ||
      ...
      for (
        let i = 0;
        i < Math.min(model.layers.length, weights.layers.length);
        i++
      ) {
        const modelLayer = model.layers[i];
        const weightLayer = weights.layers[i];
      ...
      for (
        ...
        if (Array.isArray(layerResult.weights[0])) {
          ...
          for (let i = 0; i < layerResult.weights.length; i++) {
            for (let j = 0; j < layerResult.weights[i].length; j++) {
              ...
              for (
                let modelIndex = 0;
                modelIndex < weightsArray.length;
                modelIndex++
              ) {
                const modelWeights =
                  weightsArray[modelIndex].layers[layerIndex].weights;
        ...
        for (let i = 0; i < layerResult.biases.length; i++) {
          ...
          for (
            ...
            const modelBiases =
              weightsArray[modelIndex].layers[layerIndex].biases;
            sum += modelBiases[i];
          }
        ...
        if (layer.weights && layer.weights.data && layer.weights.shape) {
          if (layer.weights.shape.length > 1) {
            ...
            for (let i = 0; i < rows; i++) {
              ...
              for (let j = 0; j < cols; j++) {
                row.push(
                  webModel.layers[layer.index].weights.data[i * cols + j],
                );
              }
              weights.push(row);
            }
            ...
            layer.weights = Array.from(
              webModel.layers[layer.index].weights.data,
            );
          }
        }
