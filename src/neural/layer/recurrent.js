...
(function () {
  ...
  class RecurrentLayer {
    ...
    _initializeParameters(params) {
      const scale = params.scale || Math.sqrt(2 / (this.inputSize + this.hiddenSize));
      ...
      if (this.cellType === "lstm") {
        ...
        this.weights.Wi = this._initializeMatrix(this.hiddenSize, this.inputSize, scale, distribution);
        this.weights.Wf = this._initializeMatrix(this.hiddenSize, this.inputSize, scale, distribution);
        this.weights.Wc = this._initializeMatrix(this.hiddenSize, this.inputSize, scale, distribution);
        this.weights.Wo = this._initializeMatrix(this.hiddenSize, this.inputSize, scale, distribution);
        
        // Recurrent weights (applied to previous hidden state)
        this.weights.Ui = this._initializeMatrix(this.hiddenSize, this.hiddenSize, scale, distribution);
        this.weights.Uf = this._initializeMatrix(this.hiddenSize, this.hiddenSize, scale, distribution);
        this.weights.Uc = this._initializeMatrix(this.hiddenSize, this.hiddenSize, scale, distribution);
        this.weights.Uo = this._initializeMatrix(this.hiddenSize, this.hiddenSize, scale, distribution);
        ...
        // Input weights
        this.weights.Wz = this._initializeMatrix(this.hiddenSize, this.inputSize, scale, distribution);
        this.weights.Wr = this._initializeMatrix(this.hiddenSize, this.inputSize, scale, distribution);
        this.weights.Wh = this._initializeMatrix(this.hiddenSize, this.inputSize, scale, distribution);
        
        // Recurrent weights
        this.weights.Uz = this._initializeMatrix(this.hiddenSize, this.hiddenSize, scale, distribution);
        this.weights.Ur = this._initializeMatrix(this.hiddenSize, this.hiddenSize, scale, distribution);
        this.weights.Uh = this._initializeMatrix(this.hiddenSize, this.hiddenSize, scale, distribution);
        ...
        // Simple RNN
        this.weights.Wx = this._initializeMatrix(this.hiddenSize, this.inputSize, scale, distribution);
        this.weights.Wh = this._initializeMatrix(this.hiddenSize, this.hiddenSize, scale, distribution);
    ...
    _forwardGRU(x, prevH) {
      ...
      const hiddenState = this._vectorAdd(
        ...
        this._vectorMultiply(
          this._vectorSubtract(new Array(updateGate.length).fill(1), updateGate),
          candidateH
        )
      );
    ...
    forward(input) {
      ...
      for (let i = 0; i < sequence.length; i++) {
        if (!Array.isArray(sequence[i]) || sequence[i].length !== this.inputSize) {
          throw new Prime.ValidationError(
      ...
      // Initialize states
      let hiddenStates = new Array(sequence.length + 1);
      let cellStates = this.cellType === "lstm" ? new Array(sequence.length + 1) : null;
      let gateStates = new Array(sequence.length);
    ...
    _backwardLSTM(dh, dc, x, states) {
      ...
      let dCellState = this._vectorAdd(dTanhCDerivative, dc);
      ...
      const dX = this._vectorAdd(
        this._vectorAdd(
          this._matrixVectorProduct(this._transpose(this.weights.Wi), dInputGatePre),
          this._matrixVectorProduct(this._transpose(this.weights.Wf), dForgetGatePre)
        ),
        this._vectorAdd(
          this._matrixVectorProduct(this._transpose(this.weights.Wc), dCellCandidatePre),
          this._matrixVectorProduct(this._transpose(this.weights.Wo), dOutputGatePre)
        )
      );
      ...
      const dPrevH = this._vectorAdd(
        this._vectorAdd(
          this._matrixVectorProduct(this._transpose(this.weights.Ui), dInputGatePre),
          this._matrixVectorProduct(this._transpose(this.weights.Uf), dForgetGatePre)
        ),
        this._vectorAdd(
          this._matrixVectorProduct(this._transpose(this.weights.Uc), dCellCandidatePre),
          this._matrixVectorProduct(this._transpose(this.weights.Uo), dOutputGatePre)
        )
      );
    ...
    _backwardGRU(dh, x, states) {
      ...
      const dX = this._vectorAdd(
        this._vectorAdd(
          this._matrixVectorProduct(this._transpose(this.weights.Wz), dUpdateGatePre),
          this._matrixVectorProduct(this._transpose(this.weights.Wr), dResetGatePre)
        ),
        this._matrixVectorProduct(this._transpose(this.weights.Wh), dCandidateHPre)
      );
      ...
      const dPrevHFromReset = this._vectorMultiply(
        dResetHidden,
        resetGate
      );
    ...
    _backwardSimpleRNN(dh, x, states) {
      ...
      const dPreActivation = this._vectorMultiply(
        dh,
        this._tanhDerivative(h)
      );
    ...
    backward(dY, cache) {
      ...
      if (this.returnSequences) {
        ...
        if (!Array.isArray(dY)) {
          throw new Prime.ValidationError(
            "Output gradient must be a vector"
          );
        }
      }
    ...
    _clipGradientsIfNeeded(gradients) {
      ...
      if (this.metrics.gradientNorm > this.validationResults.highestGradientNorm) {
    ...
    _updateGateActivationStatistics(gates) {
      for (const gateType in gates) {
        if (gates[gateType] && this.usagePatterns.gateActivationStatistics[gateType]) {
     ...
     */
    _monitorCellStateCoherence(prevCellState, currentCellState, inputGate, forgetGate) {
      ...
      const avgAbsCellValue = currentCellState.reduce((sum, val) => sum + Math.abs(val), 0) / currentCellState.length;
      ...
      const avgForgetGate = forgetGate.reduce((sum, val) => sum + val, 0) / forgetGate.length; 
      ...
      const avgInputGate = inputGate.reduce((sum, val) => sum + val, 0) / inputGate.length;
    ...
    _calculateCoherenceScore() {
      ...
      const hiddenStateDistribution = this.usagePatterns.hiddenStateDistribution;
      ...
      if (this.cellType === "lstm") {
        ...
        const forgetGateStats = this.usagePatterns.gateActivationStatistics.forget;
        const avgForgetGate = forgetGateStats.reduce((a, b) => a + b, 0) / forgetGateStats.length;
        ...
        const inputGateStats = this.usagePatterns.gateActivationStatistics.input;
        const avgInputGate = inputGateStats.reduce((a, b) => a + b, 0) / inputGateStats.length;
        ...
        const resetGateStats = this.usagePatterns.gateActivationStatistics.reset;
        const avgResetGate = resetGateStats.reduce((a, b) => a + b, 0) / resetGateStats.length;
        ...
        const updateGateStats = this.usagePatterns.gateActivationStatistics.update;
        const avgUpdateGate = updateGateStats.reduce((a, b) => a + b, 0) / updateGateStats.length;
      ...
      const gradientHealth = Math.max(
        0,
        1 - Math.abs(this.metrics.gradientNorm - idealGradientNorm) / maxGradientNorm
      );
    ...
    getUsagePatterns() {
      return {
        hiddenStateDistribution: [...this.usagePatterns.hiddenStateDistribution],
        gateActivationStatistics: JSON.parse(JSON.stringify(this.usagePatterns.gateActivationStatistics)),
