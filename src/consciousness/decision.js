...
class DecisionMaking {
  ...
  decide(alternatives, state, context = {}) {
    ...
    const requiresDeliberation = this._requiresDeliberation(alternatives, state, context);
  ...
  completeDeliberation() {
    ...
    // Record decision
    this._recordDecision(decision, this._activeDeliberation.state, this._activeDeliberation.context);
  ...
  _deliberationIteration(deliberation) {
    ...
    const perspectiveIdx = Math.floor(Math.random() * this._perspectiveModels.length);
    ...
    const candidateIdxs = alternativeIdxs.slice(0, Math.ceil(deliberation.alternatives.length / 2));
    
    // Randomly choose from candidates
    const alternativeIdx = candidateIdxs[Math.floor(Math.random() * candidateIdxs.length)];
    ...
    evaluationItem.meanCoherence = evaluationItem.coherenceScores.reduce((sum, val) => sum + val, 0) / 
    ...
    if (evaluationItem.coherenceScores.length > 1) {
      ...
      const variance = evaluationItem.coherenceScores.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) /
                      evaluationItem.coherenceScores.length;
  ...
  _decideFromDeliberation(deliberation) {
    ...
    const sortedEvals = [...deliberation.evaluations]
    ...
    if (topEval.meanCoherence < this.coherenceThreshold && 
      ...
      const viableAlts = sortedEvals
        ...
        .map(e => {
          ...
          return {
            ...
            novelty: e.noveltyScores.reduce((sum, val) => sum + val, 0) / e.noveltyScores.length,
            idx
          };
        })
    ...
    const selectedEval = deliberation.evaluations.find(e => 
      deliberation.alternatives[deliberation.evaluations.indexOf(e)] === selectedAlt);
  ...
  _makeFastDecision(alternatives, state, context) {
    ...
    // If top evaluation has low coherence and we have exploration tendency, 
    // potentially choose a different alternative
    if (topChoice.evaluation.coherence < this.coherenceThreshold && 
        Math.random() < this.explorationRate) {
    ...
    // Calculate certainty
    const scoreDiff = withScores.length > 1 
      ? selectedChoice.score - withScores[1].score
      : 1.0;
    
    const certainty = Math.min(1.0, Math.max(0.1, 
  ...
  _requiresDeliberation(alternatives, state, context) {
    ...
    const coherenceLevel = state.coherence || 
                          (state.vector ? state.vector[2] : 0.5);
  ...
  _evaluateFromPerspective(alternative, state, context, perspective) {
    ...
    if (altType === 'object' && alternative) {
      if (typeof alternative.coherence === 'number') {
        ...
        coherence = this._calculateCoherenceWithPerspective(alternative, perspective);
      }
    } else {
    ...
    if (altType === 'object' && alternative) {
      if (typeof alternative.utility === 'number') {
        ...
        utility = this._estimateUtilityFromPerspective(alternative, perspective);
      }
    } else {
  ...
  _estimateUtilityFromPerspective(alternative, perspective) {
    ...
    if (typeof alternative.benefit === 'number') {
      ...
      const benefitProps = ['positive', 'gain', 'benefit', 'reward', 'advantage'];
    ...
    if (typeof alternative.cost === 'number') {
      ...
      const costProps = ['negative', 'cost', 'effort', 'difficulty', 'disadvantage'];
    ...
    return benefitUtility * benefitWeight + costUtility * costWeight + riskUtility * riskWeight;
  }
  ...
  _extractValuesFromState(state) {
    ...
    if (state.vector && Array.isArray(state.vector)) {
      ...
      if (typeof state.attention === 'number') values.attention = state.attention;
      if (typeof state.awareness === 'number') values.awareness = state.awareness;
      if (typeof state.coherence === 'number') values.coherence = state.coherence;
      if (typeof state.integration === 'number') values.integration = state.integration;
      if (typeof state.differentiation === 'number') values.differentiation = state.differentiation;
      if (typeof state.selfReference === 'number') values.selfReference = state.selfReference;
      if (typeof state.temporalBinding === 'number') values.temporalBinding = state.temporalBinding;
    }
  ...
  _shouldUpdatePerspectives(state) {
    ...
    const stateCoherence = state.coherence || 
                          (state.vector ? state.vector[2] : 0.5);
   ...
   */
  _computeOverallScore(coherenceScores, utilityScores, confidenceScores, noveltyScores) {
    // Calculate means for each dimension
    const meanCoherence = coherenceScores.reduce((sum, val) => sum + val, 0) / coherenceScores.length;
    const meanUtility = utilityScores.reduce((sum, val) => sum + val, 0) / utilityScores.length;
    const meanConfidence = confidenceScores.reduce((sum, val) => sum + val, 0) / confidenceScores.length;
    const meanNovelty = noveltyScores.reduce((sum, val) => sum + val, 0) / noveltyScores.length;
    
    // Combine using weighted sum
    return meanCoherence * this.valueWeights.coherence +
  ...
  _calculateCertainty(selectedEval, allEvals) {
    ...
    if (allEvals.length <= 1) {
      return selectedEval.confidenceScores.reduce((sum, val) => sum + val, 0) / 
    ...
    const coherenceVariability = selectedEval.coherenceScores.length > 1
    ...
    return Math.min(1, Math.max(0.1, certaintySources.reduce((sum, val) => sum + val, 0)));
  }
  ...
  _generateExplanation(evaluation, isExploration) {
    // Calculate means
    const meanCoherence = evaluation.coherenceScores.reduce((sum, val) => sum + val, 0) / 
                          evaluation.coherenceScores.length;
    
    const meanUtility = evaluation.utilityScores.reduce((sum, val) => sum + val, 0) / 
                        evaluation.utilityScores.length;
    
    const meanConfidence = evaluation.confidenceScores.reduce((sum, val) => sum + val, 0) / 
                           evaluation.confidenceScores.length;
    
    const meanNovelty = evaluation.noveltyScores.reduce((sum, val) => sum + val, 0) / 
  ...
  recordOutcome(selected, outcome) {
    ...
    const decision = this.decisionHistory.find(d => 
      d.selected === selected || 
      (typeof d.selected === 'object' && typeof selected === 'object' && 
  ...
  _updatePerspectivesFromOutcome(decision, outcome) {
    ...
    for (const influence of perspectiveInfluence) {
      const perspective = this._perspectiveModels.find(p => p.id === influence.id);
      ...
      if (perspective) {
        ...
        perspective.certainty = Math.min(1, Math.max(0.5, perspective.certainty + certAdjustment));
        
        // Adjust bias slightly
        const biasAdjustment = (success - 0.5) * 0.05 * influence.weight;
        perspective.bias = Math.min(0.9, Math.max(0.1, perspective.bias + biasAdjustment));
        
        // Adjust risk tolerance based on outcome and exploration
        if (decision.exploration) {
          const riskAdjustment = (success - 0.5) * 0.15 * influence.weight;
          perspective.riskTolerance = Math.min(0.9, Math.max(0.1, perspective.riskTolerance + riskAdjustment));
        }
      }
    }
  }
   ...
   */
  getStats() {
    const averageDecisionTime = this._stats.decisionsCount > 0
      ? this._stats.totalDecisionTime / this._stats.decisionsCount
      : 0;
    
    // Calculate decision distribution
    const explorationRate = this._stats.decisionsCount > 0
      ? this._stats.explorationDecisions / this._stats.decisionsCount
      : 0;
    
    const coherenceRate = this._stats.decisionsCount > 0
