...
class ConsciousnessOperator {
  ...
  constructor(options = {}) {
    ...
    this.coherenceMetric = options.coherenceMetric || this._defaultCoherenceMetric;
  ...
  _defaultCoherenceMetric(state1, state2) {
    if (!state1 || !state2 || !Array.isArray(state1) || !Array.isArray(state2)) {
      return 0;
    }
  ...
  _extractStateVector(state) {
    ...
    if (this.dimension > 7 && state.attributes && typeof state.attributes === 'object') {
  ...
  _applySpectralTransformation(vector) {
    ...
    for (let i = 0; i < this.dimension; i++) {
      for (let j = 0; j < projections.length && j < this.eigenvectors.length; j++) {
  ...
  _updateStateWithVector(state, vector) {
    ...
    if (result.manifold && Array.isArray(result.manifold.coordinates)) {
      const copyLength = Math.min(vector.length, result.manifold.coordinates.length);
  ...
  _computeEigenspaceProjection(vector) {
    ...
    projections.sort((a, b) => b.normalizedContribution - a.normalizedContribution);
    
    // Calculate participation ratio (effective dimensionality)
    const participationRatio = totalContribution > 1e-10 
      ? 1 / projections.reduce((sum, p) => sum + Math.pow(p.normalizedContribution, 2), 0)
      : 0;
  ...
  computeSystemCoherence(states) {
    ...
    const averagePairwise = pairwiseCoherences.reduce((sum, c) => sum + c, 0) / pairwiseCoherences.length;
    ...
    const globalCoherence = weightedSum / pairwiseCoherences.reduce((sum, c) => sum + Math.abs(c), 0);
    ...
    const integratedInformation = this._approximateIntegratedInformation(stateVectors);
   ...
   */
  getStats() {
    const averageTime = this._stats.applications > 0
      ? this._stats.timeSpent / this._stats.applications
      : 0;
    
    const cacheEfficiency = (this._stats.applications + this._stats.coherenceChecks) > 0
      ? this._stats.cacheHits / (this._stats.applications + this._stats.coherenceChecks)
      : 0;
