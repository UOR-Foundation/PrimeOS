...
class StateRepresentation {
  ...
  constructor(options = {}) {
    ...
    this.manifold = options.manifold || new Manifold({
      meta: meta,
      invariant: {
  ...
  generateNextState(currentState, inputs = {}, deltaTime = 0.1) {
    ...
    const naturalEvolution = this._calculateNaturalEvolution(currentState, deltaTime);
  ...
  _extractStateVector(state) {
    ...
    if (state.selfReference !== undefined && this.dimension > 5) vector[5] = state.selfReference;
    if (state.temporalBinding !== undefined && this.dimension > 6) vector[6] = state.temporalBinding;
    ...
    for (const [input, value] of Object.entries(inputs)) {
      ...
      switch (input) {
        ...
        case 'disruption':
          ...
          influences[3] = (influences[3] || 0) - Math.abs(normalizedValue) * 0.5;
          break;
      }
    }
   ...
   */
  getStats() {
    const averageProcessingTime = this._stats.statesCreated > 0
      ? this._stats.totalProcessingTime / (this._stats.statesCreated + this._stats.stateTransitions)
      : 0;
