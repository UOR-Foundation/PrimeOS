...
class MemoryStructure {
  ...
  constructor(options = {}) {
    ...
    this.similarityFunction = options.similarityFunction || this._defaultSimilarityFunction;
  ...
  store(state, context = {}) {
    ...
    if (this.shortTermMemory.length > this.shortTermCapacity) {
      ...
      const oldestMemories = this.shortTermMemory.splice(0, 
  ...
  retrieve(state, options = {}) {
    ...
    for (const memory of candidates) {
      ...
      if ((!includeShortTerm && memory.storage === 'short-term') ||
  ...
  _indexMemory(memory) {
    ...
    for (let i = 0; i < Math.min(this.indexDimension, memory.vector.length); i++) {
  ...
  _calculateSignificance(state, vector) {
    ...
    return (0.3 * coherence + 0.3 * attention + 0.2 * selfReference + 0.2 * magnitude);
  }
  ...
  _consolidateMemories(memories) {
    ...
    for (const memory of memories) {
      ...
      if (memory.significance >= this.consolidationThreshold || 
  ...
  _pruneLongTermMemory() {
    ...
    const withScore = this.longTermMemory.map(memory => {
      ...
      if (memory.lastRetrieved) {
        const daysSinceRetrieval = (Date.now() - memory.lastRetrieved) / 86400000;
    ...
    this.longTermMemory = withScore.slice(0, this.longTermCapacity)
      .map(item => item.memory);
    
    // Move pruned memories to episodic (they might be forgotten)
    const pruned = withScore.slice(this.longTermCapacity)
      .map(item => item.memory);
  ...
  _calculatePatternConfidence(pattern, memories) {
    ...
    for (const memory of memories) {
      ...
      if (preconditionMatch) {
        ...
        if (typeof outcomeDim === 'number' && memory.vector[outcomeDim] >= 0.5) {
          matchingOutcomes++;
        }
      }
    }
  ...
  getSignificantMemories(count = 10) {
    ...
    // Sort by significance
    const sorted = [...allMemories].sort((a, b) => b.significance - a.significance);
   ...
   */
  getStats() {
    const averageProcessingTime = this._stats.memoriesStored > 0
    ...
    return {
      ...
      totalMemories: this.shortTermMemory.length + this.longTermMemory.length + this.episodicMemory.length,
