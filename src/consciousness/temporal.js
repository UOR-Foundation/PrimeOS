...
class TemporalIntegration {
  ...
  integrate(newState) {
    ...
    const shouldIntegrate = this.integrationWindow.length > 1 && 
  ...
  _updateBuffers(state, isSignificant) {
    ...
    const longTermSamplingRate = Math.max(1, Math.floor(this.shortTermCapacity / 20));
    if (isSignificant || this.shortTermBuffer.length % longTermSamplingRate === 0) {
  ...
  _isSignificantState(state) {
    ...
    const changeMagnitude = this._calculateChangeMagnitude(previousState, state);
  ...
  _temporallyIntegrate(currentState) {
    ...
    const combinedVector = this._combineTimeScaleVectors(currentVector, timeScaleVectors);
    ...
    integrated.temporalBinding = this._calculateTemporalBinding(timeScaleVectors);
  ...
  _calculateTemporalBinding(timeScaleVectors) {
    ...
    for (let i = 0; i < vectors.length; i++) {
      for (let j = i + 1; j < vectors.length; j++) {
        totalSimilarity += this._calculateVectorSimilarity(vectors[i], vectors[j]);
        comparisons++;
      }
    }
  ...
  _blendWithCurrentIntegration(state) {
    ...
    const integrationVector = this._extractStateVector(this._currentIntegration);
  ...
  _calculateWindowCoherence() {
    ...
    const vectors = this.integrationWindow.map(
      state => this._extractStateVector(state)
    );
  ...
  findSimilarStates(targetState, count = 5) {
    ...
    const searchBuffer = [
      ...this.shortTermBuffer,
      ...this.significantStateBuffer.filter(s => !this.shortTermBuffer.includes(s))
    ];
    
    // Calculate similarity for each state
    const withSimilarity = searchBuffer.map(state => {
      const stateVector = this._extractStateVector(state);
      const similarity = this._calculateVectorSimilarity(targetVector, stateVector);
   ...
   */
  getStats() {
    const averageIntegrationTime = this._stats.integrations > 0
      ? this._stats.totalIntegrationTime / this._stats.integrations
      : 0;
    
    const integrationRate = (this._stats.integrations + this._stats.skippedIntegrations) > 0
      ? this._stats.integrations / (this._stats.integrations + this._stats.skippedIntegrations)
      : 0;
