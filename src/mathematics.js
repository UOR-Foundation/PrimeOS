...
(function(Prime) {
  ...
  class Multivector {
    ...
    /**
     * Calculate the norm of this multivector
    ...
    norm() {
      ...
      return Math.sqrt(Math.abs(scalarPart.components[0]["1"]));
    }
  ...
  class CliffordAlgebra {
          ...
          for (let k = 1; k <= this.dimension; k++) {
            if (k !== i && k !== j) {
              ...
              if (k < i) {
                ...
                // e_k between e_i and e_j (should not happen in this loop)
                throw new Prime.MathematicalError(`Invalid case: ${eij} * ${ek}`);
              }
    ...
    /**
     * Get the result of multiplying two basis elements
    ...
    getMultiplicationResult(a, b) {
      ...
      if (a !== "1" && !a.match(/^e\d+$/)) {
        // Simple case: try to split a into first vector and rest
        const firstVector = a.match(/^e\d+/)[0];
      ...
      if (b !== "1" && !b.match(/^e\d+$/)) {
        // Simple case: try to split b into first vector and rest
        const firstVector = b.match(/^e\d+/)[0];
  ...
  class LieGroupElement {
    ...
    _computeExponentialMatrix() {
      ...
      // For simple cases like rotations in SO(2) or SO(3), we can use direct formulas
      // For a general matrix exponential, we would use a series approximation
      
      // This is a placeholder for a proper implementation
      throw new Prime.MathematicalError(
        `Matrix exponential calculation not implemented for ${this.group.name}`
      );
    }
