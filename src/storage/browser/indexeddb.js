...
class IndexedDBProvider extends StorageProvider {
  ...
  openDatabase() {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = (event) => {
        reject(
          new Error(`IndexedDB open error: ${event.target.error.message}`),
        );
      };
  ...
  async store(data, id) {
    ...
    await this.transaction("readwrite", (store, resolve, reject) => {
      ...
      request.onerror = (event) =>
        reject(new Error(`Store error: ${event.target.error.message}`));
    });
  ...
  async delete(id) {
    return new Promise((resolve, reject) => {
      this.transaction("readwrite", (store, txResolve, txReject) => {
        ...
        request.onerror = (event) =>
          reject(new Error(`Delete error: ${event.target.error.message}`));
      });
    });
  }
  ...
  async clear() {
    return this.transaction("readwrite", (store, resolve, reject) => {
      ...
      request.onerror = (event) =>
        reject(new Error(`Clear error: ${event.target.error.message}`));
    });
  }
