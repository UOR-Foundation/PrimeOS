...
class FilesystemProvider extends StorageProvider {
  ...
  constructor(options = {}) {
    ...
    this.options.storagePath =
      this.options.storagePath || path.join(os.tmpdir(), "primeos-storage");
  ...
  async ensureDirectory(dir) {
    return new Promise((resolve, reject) => {
      fs.mkdir(dir, { recursive: true }, (err) => {
        if (err) {
          reject(
            new Error(`Failed to create directory ${dir}: ${err.message}`),
          );
        } else {
          resolve();
        }
    ...
    try {
      ...
      const deletePromises = files.map((file) => {
        ...
        return this.deleteFile(filePath).catch((err) => {
          ...
          Prime.Logger.warn(
            `Failed to delete file during clear: ${err.message}`,
            {
              path: filePath,
            },
          );
