...
class StorageManager {
  ...
  async init() {
    ...
    switch (this.options.provider) {
      ...
      case 'auto':
        ...
        break;
      default:
        throw new StorageError(`Unknown storage provider: ${this.options.provider}`);
    }
  ...
  async storeModel(model, key) {
    ...
    const modelKey = key || `model_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    
    // Extract model weights
    const weights = [];
    const numLayers = model.getNumLayers ? model.getNumLayers() : model.layers.length;
  ...
  async loadModelWeights(model, key) {
    ...
    for (const layerData of modelData.weights) {
      const layer = model.getLayer ? model.getLayer(layerData.layerIndex) : model.layers[layerData.layerIndex];
  ...
  async storeComponentState(component, key) {
    ...
    // If no key is provided, generate one
    const componentKey = key || `component_${component.name || 'unnamed'}_${Date.now()}`;
...
async function createSwappableMatrix(storageManager, id, options = {}) {
  ...
  const swappableMatrix = new SwappableMatrix(storageManager, id, matrix, options);
...
class SwapSpaceManager {
  ...
  async free(id) {
    ...
    const chunkKeys = allKeys.filter(key => key.startsWith(`swap_chunk_${id}_`));
  ...
  async read(id, offset, length) {
    ...
    for (let i = startChunk; i <= endChunk; i++) {
      try {
        const chunkData = await this.storageManager.load(`swap_chunk_${id}_${i}`);
        const chunkOffset = i * chunkSize;
        const startIndex = Math.max(0, offset - chunkOffset);
        const endIndex = Math.min(chunkData.length, offset + length - chunkOffset);
        
        result.push(...chunkData.slice(startIndex, endIndex));
      } catch (error) {
        // If chunk not found, fill with zeros
        const zeros = new Array(Math.min(chunkSize, offset + length - (i * chunkSize))).fill(0);
        result.push(...zeros);
      }
    }
...
class FilesystemStorageProvider {
  ...
  async init() {
    ...
    try {
      await util.promisify(fs.mkdir)(this.options.storagePath, { recursive: true });
      this.initialized = true;
      return true;
    } catch (error) {
      throw new StorageError(`Failed to initialize filesystem storage: ${error.message}`, 
        { path: this.options.storagePath }, 'STORAGE_INIT_FAILED', error);
    }
  }
  ...
  async store(data, key) {
    ...
    try {
      ...
      throw new StorageError(`Failed to store data: ${error.message}`, 
        { key, path: filePath }, 'STORAGE_STORE_FAILED', error);
    }
  }
  ...
  async load(key) {
    ...
    try {
      ...
      if (entry.type === 'Buffer' && entry.data && entry.data.type === 'Buffer' && Array.isArray(entry.data.data)) {
      ...
      throw new StorageError(`Failed to load data: ${error.message}`, 
        { key, path: filePath }, 'STORAGE_LOAD_FAILED', error);
    }
  }
  ...
  async delete(key) {
    ...
    try {
      ...
      throw new StorageError(`Failed to delete data: ${error.message}`, 
        { key, path: filePath }, 'STORAGE_DELETE_FAILED', error);
    }
  }
  ...
  async getAllKeys() {
    ...
    try {
      ...
      throw new StorageError(`Failed to get all keys: ${error.message}`, 
        { path: this.options.storagePath }, 'STORAGE_GET_KEYS_FAILED', error);
    }
  }
  ...
  async clear() {
    ...
    try {
      ...
      throw new StorageError(`Failed to clear storage: ${error.message}`, 
        { path: this.options.storagePath }, 'STORAGE_CLEAR_FAILED', error);
    }
  }
  ...
  async getStorageInfo() {
    ...
    try {
      // Get disk space info
      const stats = await util.promisify(fs.statfs || fs.statvfs)(this.options.storagePath);
