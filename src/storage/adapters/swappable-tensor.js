...
class SwappableTensor {
  ...
  constructor(storageManager, id, tensor, options = {}) {
    ...
    this.totalBlocks = this.blockCounts.reduce(
    ...
    this.fullTensor =
      totalElements <= this.options.blockSize * this.options.maxCachedBlocks
        ? tensor
        : null;
  ...
  _extractBlock(tensor, blockIndices) {
    ...
    const startIndices = blockIndices.map(
      (blockIdx, dim) => blockIdx * this.options.blockSize,
    );
  ...
  _getBlockIndices(indices) {
    ...
    const blockIndices = indices.map((idx) =>
      Math.floor(idx / this.options.blockSize),
    );
    const indicesInBlock = indices.map(
      (idx, dim) => idx % this.options.blockSize,
    );
  ...
  async set(indices, value) {
    ...
    // Get block indices
    const { blockId, blockIndices, indicesInBlock } =
      this._getBlockIndices(indices);
  ...
  async _fillBinaryOperation(result, other, operation, dimension, indices) {
    ...
    for (let i = 0; i < this.shape[dimension]; i++) {
      indices.push(i);
      await this._fillBinaryOperation(
        result,
        other,
        operation,
        dimension + 1,
    ...
    const contractionInfo = {
      ...
      nonContractionMapping: this.shape.map((_, i) =>
        dims.includes(i)
          ? -1
          : resultShape.length -
            this.shape.filter((_, j) => !dims.includes(j) && j >= i).length,
      ),
    };
  ...
  async _fillMatmul(
    ...
    for (let i = 0; i < resultShape[dimension]; i++) {
      ...
      const thisIdx =
        dimension < thisShape.length - 2
          ? thisShape[dimension] === 1
            ? 0
            : i
          : 0;

      const otherIdx =
        dimension < otherShape.length - 2
          ? otherShape[dimension] === 1
            ? 0
            : i
          : 0;
  ...
  async _calculateBatchTraces(batchShape, dimension, indices, addTrace) {
    ...
    for (let i = 0; i < batchShape[dimension]; i++) {
      indices.push(i);
      await this._calculateBatchTraces(
        batchShape,
        dimension + 1,
        indices,
        addTrace,
