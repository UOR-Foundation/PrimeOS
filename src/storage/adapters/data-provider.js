...
class DataProvider {
  ...
  async nextBatch() {
    ...
    try {
      ...
      const inputData = this.cachedInputData || await this.storageManager.load(this.options.inputId);
      const outputData = this.options.outputId ? 
        (this.cachedOutputData || await this.storageManager.load(this.options.outputId)) : 
        null;
  ...
  async getRandomBatch() {
    ...
    const outputBatch = outputData ? selectedIndices.map(index => {
      let output = outputData[index];
      if (this.options.preprocessOutput) {
        output = this.options.preprocessOutput(output);
      }
