...
class ChunkManager {
  ...
  splitArrayData(data, id, chunkSize) {
    ...
    const estimatedBytesPerElement = Math.max(
      1,
      Math.ceil(estimatedByteSize / size),
    );

    // Calculate chunks based on estimated byte size
    let actualChunkSize = chunkSize;
    if (!actualChunkSize) {
      const optimalByteChunkSize =
        this.calculateOptimalChunkSize(estimatedByteSize);
      actualChunkSize = Math.max(
        1,
        Math.floor(optimalByteChunkSize / estimatedBytesPerElement),
      );
    }
  ...
  reassembleChunks(chunks) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (
        this.options.validateChecksums &&
        chunks[i].metadata &&
        chunks[i].metadata.checksum
      ) {
