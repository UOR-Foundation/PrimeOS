...
const ManifoldVisualization = {
  ...
  createVisualization: function(manifold, options = {}) {
    ...
    if (format === "json") {
      ...
      return {
        ...
        type: manifold.getType(),
        name: meta.name || "Unnamed Manifold",
        coordinates: this._generateVisualCoordinates(manifold, dimensions, method),
  ...
  createMultiManifoldVisualization: function(manifolds, options = {}) {
    if (!Array.isArray(manifolds) || !manifolds.every(m => m instanceof Manifold)) {
      throw new Prime.ValidationError("First argument must be an array of manifolds");
    }
    ...
    if (format === "graph") {
      ...
      return manifolds.map(manifold => {
        ...
        let coordinates;
        try {
          coordinates = this.generateVisualCoordinates(manifold, dimensions, options.method || "pca");
        } catch (e) {
    ...
    throw new Prime.InvalidOperationError(`Visualization format ${format} not supported for multiple manifolds`);
  },
  ...
  generateVisualCoordinates: function(manifold, dimensions, method) {
    ...
    if (method === "pca") {
      ...
      while (result.length < dimensions && result.length < numericValues.length) {
  ...
  createHeatmap: function(manifold, options = {}) {
    ...
    if (propertyKey && variant[propertyKey]) {
      ...
      const arrayProps = Object.entries(variant)
        .filter(([_, val]) => Array.isArray(val) && val.every(v => typeof v === 'number'));
