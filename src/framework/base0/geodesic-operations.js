...
const GeodesicOperations = {
  ...
  parallelTransport: function(source, target, vector, options = {}) {
    ...
    for (let i = 0; i < steps; i++) {
      ...
      const dotProduct = transportedVector.reduce((sum, val, idx) => sum + val * normalizedTangent[idx], 0);
      ...
      const currentNorm = Math.sqrt(transportedVector.reduce((sum, val) => sum + val * val, 0));
      transportedVector = transportedVector.map(val => val * (originalNorm / currentNorm));
    }
  ...
  sectionalCurvature: function(manifold, vector1, vector2, options = {}) {
    ...
    const projection = vector2.reduce((sum, val, idx) => sum + val * v1Normalized[idx], 0);
    
    // Subtract projection to get orthogonal component
    const v2Orthogonal = vector2.map((val, idx) => val - projection * v1Normalized[idx]);
    
    // Normalize orthogonal component
    const v2Norm = Math.sqrt(v2Orthogonal.reduce((sum, val) => sum + val * val, 0));
    ...
    const dotProduct = v1Normalized.reduce((sum, val, idx) => sum + val * v2Normalized[idx], 0);
