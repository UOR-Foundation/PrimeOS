...
function createComponent(options = {}) {
  ...
  const props = Prime.Utils.isObject(options.props) ? Prime.Utils.deepClone(options.props) : {};
  ...
  const component = {
            ...
            if (options.propTypes[propName]) {
              ...
              if (Prime.Utils.isFunction(validator)) {
                ...
                if (validationResult !== true) {
                  throw new Prime.ValidationError(`Invalid prop '${propName}'`, {
                    context: {
                      componentId: this.id,
                      propName,
                      value,
                      validationMessage: validationResult || 'Failed type validation'
                    }
                  });
                }
              }
    ...
    _applyPreciseUpdate: function(current, updates) {
      ...
      for (const key in updates) {
        if (Object.prototype.hasOwnProperty.call(updates, key)) {
          ...
          else if (Prime.Utils.isObject(currentValue) && Prime.Utils.isObject(updateValue)) {
          ...
          else if (Array.isArray(currentValue) && Array.isArray(updateValue) && 
    ...
    addChild: function(child, index) {
      if (!child || !child.id) {
        throw new Prime.ValidationError('Child must be a valid component with an id', {
          context: { child }
        });
      }
      ...
      if (Prime.Utils.isNumber(index) && index >= 0 && index <= this.children.length) {
    ...
    removeChild: function(childId) {
      const id = typeof childId === 'string' ? childId : 
                (childId && childId.id ? childId.id : null);
    ...
    on: function(eventType, handler) {
      if (!eventType || !Prime.Utils.isFunction(handler)) {
        throw new Prime.ValidationError('Event type and handler function are required', {
    ...
    trigger: function(eventType, eventData = {}) {
      ...
      for (const handler of handlers) {
        try {
          ...
          if (Prime.Utils.isFunction(hooks.errorCaught)) {
            hooks.errorCaught.call(this, error, 'eventHandler', { eventType, event });
          } else {
            Prime.Logger.error(`Error in ${this.id} '${eventType}' event handler:`, error);
          }
          results.push(null);
        }
      }
    ...
    render: function(options = {}) {
      if (!Prime.Utils.isFunction(options.renderer) && !Prime.Utils.isFunction(this.renderer)) {
        throw new Prime.InvalidOperationError(`No renderer available for component ${this.id}`);
      }
      ...
      try {
        ...
        metrics.totalRenderTime = Prime.KahanSum(metrics.totalRenderTime, duration);
