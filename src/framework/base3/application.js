...
function createApplication(options = {}) {
  ...
  const app = {
    ...
    _applyPreciseUpdate: function (prevState, nextState) {
      ...
      if (
        !Prime.Utils.isObject(prevState) ||
      ...
      for (const key of allKeys) {
        if (key in nextState) {
          if (key in prevState) {
            result[key] = this._applyPreciseUpdate(
              prevState[key],
              nextState[key],
            );
          } else {
    ...
    _notifySubscribers: function (action, prevState, newState) {
      ...
      changeInfo.changedPaths = this._calculateChangedPaths(
        prevState,
        newState,
      );
    ...
    _calculateChangedPaths: function (prevState, newState, basePath = "") {
      ...
      for (const key of allKeys) {
        ...
        if (!(key in prevState)) {
          ...
          Prime.Utils.isObject(prevState[key]) &&
          Prime.Utils.isObject(newState[key])
        ) {
