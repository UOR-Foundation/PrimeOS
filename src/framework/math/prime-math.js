...
const PrimeMath = {
  ...
  sqrt: function(x) {
    if (!isNumber(x) || x < 0) {
      throw new Prime.MathematicalError('Square root requires a non-negative number');
    }
  ...
  cos: function(x) {
    ...
    if (Math.abs(normalized - Math.PI/2) < 1e-5 || 
  ...
  integrate: function(f, a, b, options = {}) {
    ...
    const adaptiveIntegrate = (a, b, fa, fb, depth) => {
      ...
      return adaptiveIntegrate(a, c, fa, fc, depth + 1) + 
  ...
  solveNewton: function(f, df, x0, options = {}) {
    if (typeof f !== 'function' || typeof df !== 'function') {
      throw new Prime.ValidationError('Newton method requires function and derivative');
    }
    ...
    for (let i = 0; i < maxIterations; i++) {
      ...
      if (Math.abs(dfx) < 1e-14) {
        throw new Prime.MathematicalError('Derivative near zero, Newton method failed');
      }
  ...
  minimizeGradient: function(f, initialGuess, options = {}) {
    if (Prime.ExtremePrecision && Prime.ExtremePrecision.gradientDescent) {
      return Prime.ExtremePrecision.gradientDescent(f, 
    ...
    const x = initialGuess instanceof Vector ? 
      initialGuess.values.slice() : initialGuess.slice();
  ...
  approxEqual: function(a, b, epsilon = Number.EPSILON) {
    if (!isNumber(a) || !isNumber(b)) {
      throw new Prime.ValidationError('Approximate equality requires numeric values');
    }
  ...
  randomNormal: function(mean = 0, stdDev = 1) {
    if (!isNumber(mean) || !isNumber(stdDev)) {
      throw new Prime.ValidationError('Normal distribution requires numeric parameters');
    }
