...
try {
  ...
  mathUtils = {
    optimization: {
      gradientDescent: () => { throw new Error('Math utilities not available'); }
    }
  };
}
...
class CliffordAlgebraFiber {
  ...
  _initializeMetric() {
    ...
    const metric = Array(n).fill().map(() => Array(n).fill(0));
    ...
    for (let i = 1; i < n; i++) {
      for (let j = 0; j < i; j++) {
        ...
        const correlation = 0.1 * Math.cos(Math.PI * this.position.reduce((sum, v) => sum + v, 0) * (i + j) / n);
...
class CoherenceGradientDescent {
  ...
  _initializeSymmetryGenerators() {
    ...
    for (let size = 2; size <= maxCluster; size++) {
      for (let start = 0; start <= this.dimension - size; start++) {
        const indices = Array(size).fill().map((_, i) => start + i);
        generators.push({
          type: 'cluster_flip',
          indices,
  ...
  encodeProblem(problem) {
    ...
    const encoded = {
      ...
      initialState: problem.initialState || Array(n).fill().map(() => Math.random() < 0.5 ? 0 : 1),
      bestState: null,
  ...
  _createFiberEncodings(problem) {
    ...
    for (let i = 0; i < this.fibers.length; i++) {
      ...
      for (let j = 0; j < encoding.constraintWeights.length; j++) {
        const weight = 1.0 + 0.5 * Math.sin(Math.PI * pos[0] * j / encoding.constraintWeights.length);
        encoding.constraintWeights[j] *= weight;
      }
      
      // Vary variable weights
      for (let j = 0; j < encoding.variableWeights.length; j++) {
        const weight = 1.0 + 0.3 * Math.cos(Math.PI * pos[1] * j / encoding.variableWeights.length);
  ...
  _evaluateConstraint(constraint, state) {
    ...
    if (typeof constraint === 'function') {
      ...
      return typeof result === 'boolean' ? (result ? 1 : 0) : Math.min(1, Math.max(0, result));
    }
  ...
  _computeSpectralCoherence(problem, state, satisfaction) {
    ...
    for (let i = 0; i < problem.constraints.length; i++) {
      ...
      const involved = this._identifyInvolvedVariables(problem.constraints[i], problem.n_vars);
    ...
    for (const clusterId in clusterSatisfaction) {
      if (Object.hasOwnProperty.call(clusterSatisfaction, clusterId)) {
        ...
        if (satisfactions.length > 0) {
          ...
          const satisfactionRate = satisfactions.filter(Boolean).length / satisfactions.length;
    ...
    for (let i = 0; i < problem.n_vars; i++) {
      ...
      if (importanceScores[i] > 0.5) {
        ...
        for (let j = 0; j < problem.constraints.length; j++) {
          const involved = this._identifyInvolvedVariables(problem.constraints[j], problem.n_vars);
        ...
        if (involvedConstraints.length > 0) {
          importantVarCoherence += importanceScores[i] * involvedConstraints.length;
        }
      }
    }
    
    // Normalize
    importantVarCoherence = Math.min(1.0, importantVarCoherence / problem.n_vars);
    
    // Combine components
    const spectralCoherence = 0.5 * clusterCoherence + 0.5 * importantVarCoherence;
  ...
  _computeFiberCoherence(problem, state, satisfaction) {
    ...
    return fiberCoherences.length > 0
      ? fiberCoherences.reduce((sum, val) => sum + val, 0) / fiberCoherences.length
      : 0;
  }
  ...
  computeCoherenceGradient(problem, state) {
    ...
    for (let i = 0; i < this.symmetryGenerators.length; i++) {
      ...
      if (generator.type === 'bit_flip') {
        ...
        isPriority = importantIndices.has(generator.indices[0]) || 
      ...
      const { coherence: newCoherence } = this.computeStateCoherence(problem, newState);
  ...
  diversifySearch(problem, state, previousStates, iteration) {
    ...
    if (previousStates && previousStates.length > 0) {
      const similarities = previousStates.map(prevState => {
        return state.reduce((count, val, i) => count + (val === prevState[i] ? 1 : 0), 0) / state.length;
      });
    ...
    if (strategy === 0) {
      ...
      if (uniqueClusters.length > 0) {
        const targetCluster = uniqueClusters[Math.floor(Math.random() * uniqueClusters.length)];
      ...
      const generatorIdx = Math.floor(Math.random() * this.symmetryGenerators.length);
    ...
    for (let restart = 0; restart < restartCount; restart++) {
      ...
      if (restart === 0) {
        ...
        if (this.useSpectral && problem.spectralData) {
          ...
          currentState = Array(problem.n_vars).fill().map(() => Math.random() < 0.5 ? 0 : 1);
          
          for (let i = 0; i < currentState.length; i++) {
            if (i < importanceScores.length && Math.random() < importanceScores[i]) {
      ...
      for (let iter = 0; iter < restartIterations; iter++) {
        ...
        if (filteredGradient.length === 0) {
          ...
          currentState = this.diversifySearch(
            problem, currentState, previousStates, iter
          );
          
          const { coherence, satisfaction } = this.computeStateCoherence(problem, currentState);
        ...
        if (iter % 50 === 0 && coherence > 0) {
          ...
          const { coherence: divCoherence } = this.computeStateCoherence(problem, diversifiedState);
        ...
        history.satisfiedConstraints.push(satisfaction.reduce((sum, val) => sum + (val > 0.5 ? 1 : 0), 0));
