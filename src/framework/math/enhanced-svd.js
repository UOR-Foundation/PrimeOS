...
function computeSVD(matrix, options = {}) {
  ...
  if (isExtremelySmall) {
    console.log("Matrix contains extremely small values, using specialized handling");
    return handleExtremelySmallMatrix(matrix);
  }

  // Analyze matrix for extreme values and condition number
  const { maxAbs, minNonZero, dynamicRange, needsScaling, scaleFactor } = analyzeMatrix(matrix, useScaling);

  // Apply scaling if needed
  let workingMatrix = matrix;
  if (needsScaling) {
    console.log(`Scaling matrix for SVD with factor ${scaleFactor.toExponential(2)}`);
  ...
  try {
    switch (selectedAlgorithm) {
      ...
      case 'powerIteration':
        ...
        break;
      default:
        throw new Prime.ValidationError(`Unknown SVD algorithm: ${selectedAlgorithm}`);
    }
  } catch (error) {
    // If the primary algorithm fails, try fallback methods
    console.error(`SVD ${selectedAlgorithm} algorithm failed: ${error.message}. Trying fallbacks.`);
    
    try {
      result = jacobiSVD(workingMatrix, { maxIterations: maxIterations * 2, tolerance: tolerance * 10 });
    } catch (fallbackError) {
      try {
        result = powerIterationSVD(workingMatrix, { maxIterations: maxIterations * 2, tolerance: tolerance * 10 });
        ...
        return createEmergencyFallbackSVD(matrix);
      }
    }
  }
...
function tridiagonalize(matrix, V) {
  ...
  for (let k = 0; k < n - 2; k++) {
    ...
    for (let j = k + 1; j < n; j++) {
      ...
      for (let i = k + 1; i < n; i++) {
        for (let j = k + 1; j <= i; j++) {
          matrix[i][j] -= matrix[i][k] * temp[j - k - 1] + matrix[j][k] * temp[i - k - 1];
