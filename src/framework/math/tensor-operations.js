...
const TensorOperations = {
  ...
  create: function(shape, initialValue = 0) {
    ...
    for (let i = 0; i < firstDim; i++) {
      if (restDims.length === 0) {
        // For leaf elements
        result[i] = typeof initialValue === 'function' ? initialValue(i) : initialValue;
      } else {
        // For nested arrays, pass the function with adjusted index if it's a function
        result[i] = typeof initialValue === 'function'
          ? this.create(restDims, (j) => initialValue(i * elementsPerItem + j))
  ...
  findScalingFactor: function(tensor) {
    ...
    // Handle mixed scales with large dynamic range
      ...
      if (dynamicRange > 1e200) {
        ...
        const logMean = Math.exp((Math.log(maxAbsValue) + Math.log(minAbsNonZeroValue)) / 2);
  ...
  add: function(a, b, options = {}) {
    ...
    const addArrays = (arrA, arrB, compensation = null) => {
      if (!Array.isArray(arrA) || !Array.isArray(arrB)) {
        ...
        const { result, compensation: newComp } = addElements(scalarA, scalarB, comp);
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        const elemComp = compensation && i < compensation.length ? compensation[i] : 0;
        ...
        if (Array.isArray(elemA) || Array.isArray(elemB)) {
          ...
          const { result: subResult, compensation: subComp } = addElements(elemA, elemB, elemComp);
          result[i] = subResult;
          if (newCompensation) newCompensation[i] = subComp;
        }
      }
    ...
    if (scaleFactorA !== 1 || scaleFactorB !== 1) {
      ...
      const resultScale = scaleFactorA !== 1 && scaleFactorB !== 1
        ? 1 / Math.max(scaleFactorA, scaleFactorB)
        : (scaleFactorA !== 1 ? 1 / scaleFactorA : 1 / scaleFactorB);
  ...
  matmul: function(tensor, matrix, options = {}) {
    ...
    if (tensorShape.length < 2 || matrixShape.length < 2) {
      throw new Error('Input tensors must have at least 2 dimensions for matmul');
    }
  ...
  _dotVectors: function(a, b, options = {}) {
    ...
    // Apply scaling if needed
    const scaleA = needsScaling && maxAbsValue > 1e100 ? 1e-100 : (needsScaling && maxAbsValue < 1e-100 ? 1e100 : 1);
  ...
  _matrixVectorDot: function(matrix, vector, options = {}) {
    ...
    // Validate inputs
    if (!Array.isArray(matrix) || !Array.isArray(matrix[0]) || !Array.isArray(vector)) {
    ...
    // Apply scaling if needed
    const scaledMatrix = needsScaling ? matrix.map(row => row.map(x => x * scaleFactor)) : matrix;
    const scaledVector = needsScaling ? vector.map(x => x * scaleFactor) : vector;
  ...
  innerProduct: function(a, b, options = {}) {
    ...
    if (flatA.length !== flatB.length) {
      throw new Error('Tensors must have the same number of elements for inner product');
    }
  ...
  softmax: function(tensor, axis = -1, options = {}) {
    ...
    throw new Error('Softmax for tensors with more than 2 dimensions not implemented');
  },
