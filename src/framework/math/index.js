...
function adaptiveEpsilon(magnitude, context = 'general') {
  ...
  const baseTolerance = CONSTANTS.TOLERANCES[context] || CONSTANTS.EPSILON_GENERAL;
...
function kahanSum(values) {
  ...
  return {
    ...
    precision: Math.log10(1 / (CONSTANTS.EPSILON * conditionNumber || CONSTANTS.EPSILON))
  };
}
...
const vector = {
  ...
  distance: function(a, b, options = {}) {
    ...
    const relativeError = distance > 0 ? 
  ...
  cosineSimilarity: function(a, b, options = {}) {
    ...
    const dotProductRelError = dotProduct !== 0 ? 
      CONSTANTS.EPSILON * dotResult.conditionNumber / Math.abs(dotProduct) : 0;
    const normRelError = CONSTANTS.EPSILON * 
      Math.max(aNormResult.conditionNumber, bNormResult.conditionNumber) / 2;
  ...
  manhattanDistance: function(a, b) {
    ...
    return {
      ...
      absoluteError: CONSTANTS.EPSILON * sumResult.conditionNumber * sumResult.sum,
      conditionNumber: sumResult.conditionNumber
    };
  },
  ...
  normalize: function(v, options = {}) {
    ...
    const relativeError = CONSTANTS.EPSILON * (1 + sumResult.conditionNumber / 2);
...
const matrix = {
  ...
  multiply: function(a, b) {
    ...
    if (!Array.isArray(a) || !a.every(Array.isArray) || 
        !Array.isArray(b) || !b.every(Array.isArray)) {
    ...
    if (aCols !== bRows) {
      throw new Error(`Matrix dimensions mismatch: ${aRows}x${aCols} * ${bRows}x${bCols}`);
    }
    
    // Initialize result matrix
    const result = Array(aRows).fill().map(() => Array(bCols).fill(0));
    const maxErrors = Array(aRows).fill().map(() => Array(bCols).fill(0));
    ...
    for (let i = 0; i < aRows; i++) {
      for (let j = 0; j < bCols; j++) {
        ...
        maxConditionNumber = Math.max(maxConditionNumber, sumResult.conditionNumber);
        
        // Calculate error bound for this element
        maxErrors[i][j] = CONSTANTS.EPSILON * sumResult.conditionNumber * Math.abs(sumResult.sum);
      }
    }
  ...
  determinant: function(matrix) {
    ...
    try {
      ...
      return {
        ...
        warning: 'Used less numerically stable method, result may have larger errors'
      };
    }
  },
  ...
  luDecomposition: function(matrix) {
    ...
    const pivots = Array(n).fill().map((_, i) => i);
    let pivotSign = 1;
...
const optimization = {
  ...
  gradientDescent: function(f, initialPoint, options = {}) {
    ...
    const maxIterations = options.maxIterations || CONSTANTS.MAX_ITERATIONS.optimization;
  ...
  _computeGradient: function(f, x, options = {}) {
    ...
    const epsilon = options.epsilon || 
      adaptiveEpsilon(Math.abs(f(x)), 'optimization');
...
const integration = {
  ...
  adaptiveQuadrature: function(f, a, b, options = {}) {
    ...
    const gkPoints = [
      ...
      0.7415311855993944, -0.7415311855993944,
    ...
    return {
      integral: result,
      error,
      relativeError: Math.abs(result) > 1e-10 ? error / Math.abs(result) : error,
      evaluations
    };
  }
};
