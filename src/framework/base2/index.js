  ...
  syscall: function(name, ...args) {
    ...
    try {
      ...
      if (executionTime > 100) {
        Prime.Logger.warn(`Slow syscall ${name} detected: ${executionTime.toFixed(2)}ms`);
      }
      ...
      throw new Prime.InvalidOperationError(`Syscall ${name} execution failed: ${error.message}`, {
        context: { 
          syscall: name,
          syscallArgs: args,
