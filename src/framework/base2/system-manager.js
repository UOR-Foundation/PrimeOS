  ...
  create: function(config = {}) {
    return {
      ...
      allocateMemory: function(size, options = {}) {
        ...
        if (!Number.isFinite(size)) {
          throw new Prime.ValidationError('Size must be a finite number', {
            context: { providedSize: size, isNaN: Number.isNaN(size), isInfinite: !Number.isFinite(size) }
          });
        }
        ...
        if (options.enforceLimits !== false && this.memory.limits) {
          const { totalLimit, processLimit, allocationLimit } = this.memory.limits;

          // Check if total memory limit would be exceeded
          if (totalLimit && memoryStats.totalAllocated + size > totalLimit) {
            throw new Prime.ResourceExhaustionError('Memory allocation would exceed total memory limit', {
              context: {
                requested: size,
                totalAllocated: memoryStats.totalAllocated,
                totalLimit,
                available: Math.max(0, totalLimit - memoryStats.totalAllocated)
              }
            });
          }

          // Check process memory limit
          if (processLimit && memoryStats.processAllocations[options.processId] &&
              memoryStats.processAllocations[options.processId] + size > processLimit) {
            throw new Prime.ResourceExhaustionError('Memory allocation would exceed process memory limit', {
              context: {
                processId: options.processId,
                requested: size,
                processAllocated: memoryStats.processAllocations[options.processId],
                processLimit,
                available: Math.max(0, processLimit - memoryStats.processAllocations[options.processId])
              }
            });
          }
          ...
          if (allocationLimit && size > allocationLimit) {
            throw new Prime.ResourceExhaustionError('Memory allocation size exceeds individual allocation limit', {
              context: {
                requested: size,
                allocationLimit,
            ...
            setTimeout(() => {
              try {
                ...
                if (this._resources[address]) {
                  ...
                  if (Prime.Logger && this.memory.logging?.expirations !== false) {
                    Prime.Logger.debug(`Memory at address ${address} automatically freed after TTL expiration`);
                  }
                }
              } catch (error) {
                // Log error but don't crash
                if (Prime.Logger) {
                  Prime.Logger.error(`Error freeing expired memory at address ${address}:`, error);
                }
              }
            }, options.ttl);
          } else {
        ...
        this._updateMemoryStats(address, size, 'allocate', options.processId, allocationType);
      ...
      _extendMemoryTTL: function(address, ttl) {
        if (!this._resources[address] || !Prime.Utils.isNumber(ttl) || ttl <= 0) {
          return false;
        }
      ...
      _getMemoryStats: function() {
        ...
        for (const address in this._resources) {
          ...
          if (resource.type === 'memory') {
            ...
            stats.processAllocations[processId] = (stats.processAllocations[processId] || 0) + resource.size;

            // Update priority statistics
            const priority = resource.priority || 'normal';
            stats.byPriority[priority] = (stats.byPriority[priority] || 0) + resource.size;

            // Update purpose statistics
            const purpose = resource.purpose || 'general';
            stats.byPurpose[purpose] = (stats.byPurpose[purpose] || 0) + resource.size;
            
            // Update allocation type statistics
            const allocationType = resource.metadata.allocationType || 'general';
            stats.byAllocationType[allocationType] = (stats.byAllocationType[allocationType] || 0) + resource.size;
       ...
       */
      _updateMemoryStats: function(address, size, operation, processId, allocationType = 'general') {
        // Initialize allocationsByType if needed
        this._memoryTracking.allocationsByType[allocationType] = 
          this._memoryTracking.allocationsByType[allocationType] || {
            count: 0,
            totalBytes: 0,
            peakBytes: 0
        ...
        if (operation === 'allocate') {
          ...
          this._memoryTracking.allocationsByType[allocationType].totalBytes += size;
          
          // Update peak usage by type
          if (this._memoryTracking.allocationsByType[allocationType].totalBytes > 
      ...
      _checkAndPerformGarbageCollection: function() {
        ...
        if (stats.totalAllocated > 1024 * 1024 * 1024) { // > 1GB
          // Use logarithmic scale for large memory usage
          const baseThreshold = this.memory.gcThreshold;
          const logFactor = Math.log10(stats.totalAllocated / (1024 * 1024)) / 3; // log10(MB)/3
        ...
        const timeSinceLastGc = this._memoryTracking.lastGcTime ? 
          Date.now() - this._memoryTracking.lastGcTime : Infinity;
        
        // More aggressive GC if we haven't run in a while
        if (timeSinceLastGc > 3600000) { // > 1 hour
        ...
        if (stats.totalAllocated > adaptiveThreshold) {
          ...
          if (Prime.Logger && this.memory.logging?.gc !== false) {
            Prime.Logger.info(`Memory threshold exceeded (${stats.totalAllocated} bytes), running garbage collection`);
          }
      ...
      _garbageCollect: function(options = {}) {
        ...
        const cleanupOrder = [
          ...
          resource => resource.type === 'memory' && resource.expiresAt && resource.expiresAt < Date.now(),

          // Target unused resources first
          resource => resource.type === 'memory' && 
          ...
          resource => resource.type === 'memory' && resource.priority === 'low' &&
                    (Date.now() - resource.lastAccessed) > (this.memory.idleTimeout || 300000),

          // Finally, clean normal priority resources with very old access times
          resource => resource.type === 'memory' && resource.priority === 'normal' &&
                    (Date.now() - resource.lastAccessed) > (this.memory.extendedIdleTimeout || 1800000)
        ];
        ...
        for (const rule of cleanupOrder) {
          ...
          candidateAddresses.sort((a, b) => {
            ...
            const timeSort = this._resources[a].lastAccessed - this._resources[b].lastAccessed;
            if (timeSort !== 0) return timeSort;
            
            // If tied, sort by access count (less used first)
            return (this._resources[a].accessCount || 0) - (this._resources[b].accessCount || 0);
          });
          ...
          for (const address of candidateAddresses) {
            ...
            try {
              ...
              stats.byType[resourceType] = (stats.byType[resourceType] || 0) + resourceSize;

              // Stop if we've freed enough memory
              if (options.targetBytes && stats.bytesReclaimed >= options.targetBytes) {
                break;
              }
            } catch (error) {
              // Log error but continue with other resources
              if (Prime.Logger) {
                Prime.Logger.warn(`Error during garbage collection for address ${address}:`, error);
              }
            }
          }
        ...
        this._memoryTracking.gcStatistics.bytesReclaimed += stats.actualBytesReclaimed;
      ...
      freeMemory: function(address, options = {}) {
        ...
        if (!this._resources[address]) {
          ...
          throw new Prime.InvalidOperationError(`Unknown memory address: ${address}`, {
            context: { availableAddresses: Object.keys(this._resources).length }
          });
        }
        ...
        if (this._resources[address].type !== 'memory') {
          ...
          throw new Prime.InvalidOperationError(`Address ${address} is not a memory resource`, {
        ...
        const allocationType = this._resources[address].metadata.allocationType || 'general';
        ...
        if (options.cleanup && typeof options.cleanup === 'function') {
          try {
            ...
            if (Prime.Logger) {
              Prime.Logger.warn(`Error in memory cleanup callback for address ${address}:`, error);
            }
          }
        }
        ...
        this._updateMemoryStats(address, resourceSize, 'free', processId, allocationType);

        // Log memory free if enabled
        if (Prime.Logger && this.memory.logging?.frees !== false) {
          Prime.Logger.debug(`Memory at address ${address} freed (${resourceSize} bytes)`, {
        ...
        if (this.security.policy && this.security.policy[operation]) {
          ...
          if (typeof policy === 'function') {
            try {
              ...
              Prime.Logger.error(`Error evaluating security policy for ${operation}:`, error);
              return false;
            }
