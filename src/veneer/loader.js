...
class PrimeAppLoader {
  ...
  async loadFromDirectory(directory, options = {}) {
    try {
      ...
      try {
        ...
        throw new PrimeAppLoaderError('Manifest file not found', {
          directory,
          manifestPath
        }, 'MANIFEST_NOT_FOUND');
      }
      ...
      try {
        ...
        throw new PrimeAppLoaderError('Invalid manifest JSON', {
          directory,
          manifestPath,
          error: error.message
      ...
      try {
        ...
        throw new PrimeAppLoaderError('Entry point not found', {
          directory,
          entryPath,
      ...
      try {
        ...
        throw new PrimeAppLoaderError('Failed to load entry module', {
          directory,
          entryPath,
          error: error.message
      ...
      if (!AppClass.prototype instanceof PrimeApplication) {
        throw new PrimeAppLoaderError('Entry module does not export a PrimeApplication class', {
          directory,
          entryPath
        }, 'INVALID_APP_CLASS');
      }
      ...
      if (this.registry && !options.skipRegistration) {
        this.registry.registerApp(manifest, directory, { force: options.force });
      }
      ...
      throw new PrimeAppLoaderError('Failed to load PrimeApp', {
        directory,
        originalError: error
      }, 'LOAD_FAILED');
    }
  ...
  async loadFromZip(zipFile, options = {}) {
    try {
      ...
      try {
        ...
        throw new PrimeAppLoaderError('ZIP file not found', {
          zipFile
        }, 'ZIP_NOT_FOUND');
      }
      ...
      if (fileExt !== '.primeapp' && fileExt !== '.zip') {
        throw new PrimeAppLoaderError('Invalid ZIP file extension', {
          zipFile,
          extension: fileExt
      ...
      try {
        ...
        throw new PrimeAppLoaderError('Failed to extract ZIP file', {
          zipFile,
          tempDir,
          error: error.message
      ...
      if (!extractionSuccessful) {
        await this._cleanupTempDirectory(tempDir);
        throw new PrimeAppLoaderError('ZIP extraction did not complete successfully', {
          zipFile,
          tempDir
      ...
      throw new PrimeAppLoaderError('Failed to load PrimeApp from ZIP', {
        zipFile,
        originalError: error
      }, 'ZIP_LOAD_FAILED');
    }
  ...
  async loadFromNpm(packageName, options = {}) {
    try {
      ...
      try {
        ...
        throw new PrimeAppLoaderError('NPM package not found', {
          packageName,
          error: error.message
      ...
      if (!packageJson.primeos || packageJson.primeos.type !== 'app') {
        throw new PrimeAppLoaderError('Not a valid PrimeOS app package', {
          packageName,
          packageJson
      ...
      throw new PrimeAppLoaderError('Failed to load PrimeApp from NPM', {
        packageName,
        originalError: error
      }, 'NPM_LOAD_FAILED');
    }
  ...
  _validateManifest(manifest) {
    ...
    for (const field of requiredFields) {
      if (!manifest[field]) {
        throw new PrimeAppLoaderError(`Missing required field in manifest: ${field}`, {
          manifest
        }, 'INVALID_MANIFEST');
      }
    }
    ...
    if (!/^[a-z0-9_\-]+$/.test(manifest.name)) {
      throw new PrimeAppLoaderError('Invalid app name format', {
        name: manifest.name
      }, 'INVALID_APP_NAME');
    }
    
    // Check version format (semver)
    if (!/^\d+\.\d+\.\d+(?:-[a-z0-9\-]+(?:\.[a-z0-9\-]+)*)?(?:\+[a-z0-9\-]+(?:\.[a-z0-9\-]+)*)?$/.test(manifest.version)) {
      throw new PrimeAppLoaderError('Invalid version format', {
  ...
  async _loadDependencies(manifest, directory, options) {
    ...
    if (this.registry) {
      const dependencyResolution = this.registry.resolveDependencies(manifest.name);
      ...
      for (const [depId, version] of Object.entries(dependencyResolution.resolved)) {
          ...
          if (depManifest) {
            ...
            if (appInfo && appInfo.path) {
              try {
                ...
                Prime.Logger.error(`Failed to load dependency: ${depId}@${version}`, {
                  error: error.message
                });
              }
            }
