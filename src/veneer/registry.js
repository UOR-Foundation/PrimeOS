...
class PrimeAppRegistry {
  ...
  constructor(options = {}) {
    ...
    Prime.Logger.info('PrimeApp Registry initialized', { options: this.options });
  }
  ...
  registerApp(manifest, appPath, options = {}) {
    if (!manifest.name) {
      throw new PrimeAppRegistryError('Invalid manifest: missing name', {
        manifest
      }, 'INVALID_MANIFEST');
    }
    
    if (!manifest.version) {
      throw new PrimeAppRegistryError('Invalid manifest: missing version', {
        manifest
      }, 'INVALID_MANIFEST');
    }
    ...
    if (this.apps.has(appId) && 
        ...
        !options.force) {
      throw new PrimeAppRegistryError('App version already registered', {
        appId,
        version: appVersion
      }, 'APP_VERSION_EXISTS');
    }
    ...
    if (manifest.interfaces) {
      ...
      if (manifest.interfaces.provides && Array.isArray(manifest.interfaces.provides)) {
        for (const interfaceName of manifest.interfaces.provides) {
          ...
          if (!providers.some(p => p.appId === appId && p.version === appVersion)) {
            providers.push({
              appId,
              version: appVersion,
      ...
      if (manifest.interfaces.consumes && Array.isArray(manifest.interfaces.consumes)) {
        for (const interfaceName of manifest.interfaces.consumes) {
          ...
          if (!consumers.some(c => c.appId === appId && c.version === appVersion)) {
            consumers.push({
              appId,
              version: appVersion
            });
  ...
  resolveDependencies(appId, options = {}) {
    if (!this.apps.has(appId)) {
      throw new PrimeAppRegistryError('App not found', { appId }, 'APP_NOT_FOUND');
    }
  ...
  _cleanupAppInterfaces(appId) {
    ...
    for (const [interfaceName, providers] of this.interfaces.providers.entries()) {
    ...
    for (const [interfaceName, consumers] of this.interfaces.consumers.entries()) {
