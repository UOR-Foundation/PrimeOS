...
} = require("../framework/base0/manifold-validator");
const {
  CoherenceValidator,
} = require("../framework/base0/coherence-validation");
...
class CoherenceManager extends EventEmitter {
  ...
  async checkAppCoherence(appId) {
    ...
    if (app.instance && typeof app.instance.calculateCoherence === "function") {
      try {
        ...
        Prime.Logger.warn(`Error getting coherence from app ${appId}`, {
          error,
        });
      }
    }
    ...
    try {
      ...
      Prime.Logger.warn(`Error checking lifecycle coherence for ${appId}`, {
        error,
      });
    }
    ...
    try {
      ...
      Prime.Logger.warn(`Error checking resource coherence for ${appId}`, {
        error,
      });
    }
  ...
  async resolveViolation(violationOrId) {
    ...
    try {
      ...
      if (this.resolutionHistory.length > this.options.maxResolutionHistory) {
        this.resolutionHistory = this.resolutionHistory.slice(
          -this.options.maxResolutionHistory,
        );
      }
      ...
      Prime.Logger.info(
        `Coherence resolution ${success ? "succeeded" : "failed"}: ${violation.message}`,
        {
          strategy,
          type: violation.type,
          severity: violation.severity,
  ...
  getStats() {
    ...
    const avgResolutionTime =
      this.stats.resolutionsAttempted > 0
  ...
  _calculateSystemCoherence(results) {
    ...
    if (appScores.length > 0) {
      appCoherence =
        appScores.reduce((sum, score) => sum + score, 0) / appScores.length;
    } else {
  ...
  async _checkResourceCoherence() {
    ...
    for (const type of resourceTypes) {
      ...
      if (resource.allocations && resource.allocations instanceof Map) {
        const utilization =
          resource.allocations.size / Math.max(1, this.veneer.options.maxApps);
    ...
    if (Object.keys(resourceUtilization).length > 0) {
      ...
      const avgUtilization =
        totalUtilization / Object.keys(resourceUtilization).length;
  ...
  _checkInterfaceCoherence(app1, app2) {
    ...
    const getInterfaces = (app) => {
      ...
      if (
        ...
        app.instance.metadata.interfaces
      ) {
  ...
  _registerDefaultConstraints() {
    ...
    this.validator.registerConstraint("app", {
      ...
      validator: (app, context) => {
        ...
        const requested =
          app.resources && app.resources.requested
            ? Object.keys(app.resources.requested)
            : [];
        const allocated =
          app.resources && app.resources.allocated
  ...
  async _determineResolutionStrategy(violation) {
    ...
    const defaultStrategyMap = {
      ...
      [ViolationType.NUMERICAL_INSTABILITY]:
        ResolutionStrategy.NUMERIC_STABILIZATION,
    ...
    if (violation.severity === ViolationSeverity.CRITICAL) {
      if (
        violation.type === ViolationType.LOGICAL_INCONSISTENCY ||
    ...
    const strategy =
      defaultStrategyMap[violation.type] || ResolutionStrategy.LOG_ONLY;
  ...
  async _applyResolution(resolution) {
    ...
    try {
      ...
      switch (strategy) {
        ...
        case ResolutionStrategy.RESOURCE_ADJUSTMENT: {
          const resourceType =
            actions.resourceType || violation.source.resourceType;
          ...
          if (actions.reduceUsage) {
              ...
              if (hasResource) {
                ...
                if (isPaused) {
                  try {
                    ...
                    Prime.Logger.info(
                      `Released ${resourceType} resources from paused app ${app.id}`,
                    );
                    return true;
                  } catch (error) {
                    Prime.Logger.error(
                      `Error releasing ${resourceType} from ${app.id}`,
                      { error },
                    );
                  }
                }
            ...
            if (lowPriorityApps.length > 0) {
              ...
              try {
                ...
                Prime.Logger.info(
                  `Released ${resourceType} resources from low-priority app ${appToAdjust.id}`,
                );
                return true;
              } catch (error) {
                Prime.Logger.error(
                  `Error releasing ${resourceType} from ${appToAdjust.id}`,
                  { error },
                );
              }
            }
          ...
          Prime.Logger.warn(
            `Could not free up ${resourceType} resources, no suitable apps found`,
          );
          return false;
        }
        ...
        case ResolutionStrategy.PAUSE_CONFLICTING: {
          ...
          if (actions.pauseFirst && app1.state === "running") {
            await app1.pause();
            Prime.Logger.info(
              `Paused app ${app1Id} to resolve conflict with ${app2Id}`,
            );
            return true;
          }
          ...
          if (app2.state === "running") {
            await app2.pause();
            Prime.Logger.info(
              `Paused app ${app2Id} to resolve conflict with ${app1Id}`,
            );
            return true;
          }
        ...
        case ResolutionStrategy.LOG_ONLY:
          ...
          Prime.Logger.warn(
            `Coherence violation logged: ${violation.message}`,
            {
              type: violation.type,
              severity: violation.severity,
              source: violation.source,
  ...
  _onAppUnregistered(event) {
    ...
    for (const [violationId, violation] of this.activeViolations.entries()) {
      if (
        violation.source.appId === event.appId ||
