...
class PrimeVeneer extends EventEmitter {
  ...
  async init() {
    ...
    try {
      ...
      throw new PrimeVeneerError(
        "Failed to initialize veneer",
        {
          originalError: error,
        },
        "VENEER_INIT_FAILED",
  ...
  async registerApplication(appInstance, appId) {
    if (!this.state.initialized) {
      throw new PrimeVeneerError(
        "Veneer not initialized",
        {},
        "VENEER_NOT_INITIALIZED",
      );
    }
    ...
    if (
      appInstance.manifest &&
      appInstance.manifest.coherence &&
      appInstance.manifest.coherence.boundaries
    ) {
      this._registerAppCoherenceBoundaries(
        appId,
        appInstance.manifest.coherence.boundaries,
      );
    }
  ...
  async allocateResource(appId, resourceType, requirements) {
    ...
    Prime.Logger.debug(
      `Allocating resource: ${resourceType} for app: ${appId}`,
      requirements,
    );

    // Allocate the resource
    const resourceHandle = await this.resources[resourceType].allocate(
      appId,
      requirements,
    );
    ...
    Prime.Logger.info("Resource allocated", {
      appId,
      resourceType,
      requirements,
    });
  ...
  calculateSystemCoherence() {
    ...
    for (const app of this.applications.values()) {
      ...
      if (appCoherence === undefined || appCoherence === null) {
        if (
          app.instance &&
          typeof app.instance.calculateCoherence === "function"
        ) {
          try {
            appCoherence = app.instance.calculateCoherence();
          } catch (error) {
            Prime.Logger.warn(`Error getting coherence from app ${app.id}`, {
              error,
            });
  ...
  _registerAppCoherenceBoundaries(appId, boundaries) {
    ...
    for (const [domain, domainBoundaries] of Object.entries(boundaries)) {
      ...
      for (const [name, boundaryConfig] of Object.entries(domainBoundaries)) {
        ...
        const validator = (value, context) => {
          ...
          if (domain === "resource") {
            return this._validateResourceBoundary(
              name,
              value,
              context,
              boundaryConfig,
          ...
          if (domain === "numerical") {
            return this._validateNumericalBoundary(
              name,
              value,
              context,
              boundaryConfig,
          ...
          if (domain === "logical") {
            return this._validateLogicalBoundary(
              name,
              value,
              context,
              boundaryConfig,
        ...
        this.boundaryRegistry.registerBoundary(
          ...
          validator,
          {
            ...
            threshold:
              boundaryConfig.threshold || this.options.coherenceThreshold,
            description:
              boundaryConfig.description || `${domain} boundary for ${appId}`,
    ...
    Prime.Logger.debug(`Registered coherence boundaries for ${appId}`, {
      boundaries,
    });
  }
  ...
  _validateResourceBoundary(resourceType, value, context, config) {
    ...
    if (resourceType === "memory") {
      ...
      const maxBytes =
        typeof memoryLimit === "string"
          ? this._parseMemorySize(memoryLimit)
          : memoryLimit;
    ...
    if (resourceType === "storage") {
      ...
      const maxBytes =
        typeof quota === "string" ? this._parseMemorySize(quota) : quota;
    ...
    if (resourceType === "compute") {
      const concurrency =
        context.concurrency || context.maxConcurrency || Infinity;
  ...
  _validateNumericalBoundary(name, value, context, config) {
    if (name === "finiteness") {
      ...
      if (Array.isArray(value)) {
        const finiteCount = value.filter(
...
class VeneerResourceManager {
  ...
  async allocate(appId, requirements) {
    ...
    Prime.Logger.info(
      `Resource allocated: ${this.type} for ${appId}`,
      requirements,
    );
