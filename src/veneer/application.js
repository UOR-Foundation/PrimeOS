...
class PrimeApplication extends EventEmitter {
  ...
  constructor(manifest = {}, context = {}) {
    ...
    // Validate manifest
    if (!manifest.name) {
      throw new PrimeApplicationError('Invalid manifest: missing name', {
        manifest
      }, 'INVALID_MANIFEST');
    }
    ...
    Prime.Logger.debug('PrimeApplication created', { name: this.manifest.name });
  }
   ...
   */
  async init() {
    if (this.state !== ApplicationState.CREATED && this.state !== ApplicationState.STOPPED) {
      throw new PrimeApplicationError('Cannot initialize application in current state', {
    ...
    try {
      ...
      throw new PrimeApplicationError('Failed to initialize application', {
        originalError: error
      }, 'INIT_FAILED');
    }
  }
   ...
   */
  async start() {
    if (this.state !== ApplicationState.INITIALIZED && this.state !== ApplicationState.PAUSED) {
      throw new PrimeApplicationError('Cannot start application in current state', {
        currentState: this.state,
        expectedState: [ApplicationState.INITIALIZED, ApplicationState.PAUSED]
      }, 'INVALID_STATE_TRANSITION');
    }
    ...
    try {
      ...
      throw new PrimeApplicationError('Failed to start application', {
        originalError: error
      }, 'START_FAILED');
    }
  }
  ...
  async pause() {
    if (this.state !== ApplicationState.RUNNING) {
      throw new PrimeApplicationError('Cannot pause application in current state', {
        currentState: this.state,
    ...
    try {
      ...
      throw new PrimeApplicationError('Failed to pause application', {
        originalError: error
      }, 'PAUSE_FAILED');
    }
  }
  ...
  async resume() {
    if (this.state !== ApplicationState.PAUSED) {
      throw new PrimeApplicationError('Cannot resume application in current state', {
        currentState: this.state,
    ...
    try {
      ...
      throw new PrimeApplicationError('Failed to resume application', {
        originalError: error
      }, 'RESUME_FAILED');
    }
  }
  ...
  async stop() {
    ...
    if (this.state !== ApplicationState.RUNNING && 
        this.state !== ApplicationState.PAUSED && 
        this.state !== ApplicationState.ERROR) {
      throw new PrimeApplicationError('Cannot stop application in current state', {
        currentState: this.state,
        expectedState: [ApplicationState.RUNNING, ApplicationState.PAUSED, ApplicationState.ERROR]
      }, 'INVALID_STATE_TRANSITION');
    }
    ...
    try {
      ...
      throw new PrimeApplicationError('Failed to stop application', {
        originalError: error
      }, 'STOP_FAILED');
    }
  }
  ...
  async getResource(resourceType, requirements) {
    if (!this.context.veneer) {
      throw new PrimeApplicationError('No veneer context', {}, 'NO_VENEER_CONTEXT');
    }
  ...
  async _requestResources() {
    ...
    if (this.manifest.resources.storage && 
    ...
    if (this.manifest.resources.compute && 
    ...
    if (this.manifest.resources.memory && 
