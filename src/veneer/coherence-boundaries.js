  ...
  validate(value, context = {}) {
    ...
    try {
      ...
      if (result === true || result === false) {
        ...
        coherence = typeof result.coherence === 'number' 
...
const DefaultBoundaryValidators = {
  ...
  [DomainType.RESOURCE]: {
    ...
    compute: (value, context = {}) => {
      ...
      return Math.max(0, Math.min(1, (maxConcurrency - currentTasks) / maxConcurrency));
    }
  },
  ...
  [DomainType.LIFECYCLE]: {
    ...
    transitionValidity: (value, context = {}) => {
      ...
      for (const transition of validTransitions) {
        if ((transition.from === from || transition.from === '*') && 
...
class CoherenceBoundaryRegistry {
  ...
  registerBoundary(domain, name, validator, metadata = {}) {
    if (typeof validator !== 'function') {
      throw new CoherenceBoundaryError('Validator must be a function', { domain, name });
    }
  ...
  validateDomain(domain, value, context = {}) {
    ...
    if (results.boundaryResults.length > 0) {
      ...
      results.coherence = totalWeight > 0 ? weightedCoherence / totalWeight : 1.0;
    }
  ...
  validateMultipleDomains(domains, value, context = {}) {
    ...
    const domainCoherenceValues = Object.values(results.domainResults).map(r => r.coherence);
  ...
  _registerDefaultBoundaries() {
    ...
    for (const [name, validator] of Object.entries(DefaultBoundaryValidators[DomainType.RESOURCE])) {
    ...
    for (const [name, validator] of Object.entries(DefaultBoundaryValidators[DomainType.NUMERICAL])) {
      this.registerBoundary(
        DomainType.NUMERICAL,
        name,
        validator,
    ...
    for (const [name, validator] of Object.entries(DefaultBoundaryValidators[DomainType.LOGICAL])) {
      this.registerBoundary(
        DomainType.LOGICAL,
        name,
        validator,
    ...
    for (const [name, validator] of Object.entries(DefaultBoundaryValidators[DomainType.LIFECYCLE])) {
      this.registerBoundary(
        DomainType.LIFECYCLE,
        name,
        validator,
