...
class CoherenceMetricsManager {
  ...
  constructor(config = {}) {
    ...
    const violationTypes = Object.values(Prime.Distributed.Coherence.Violations.Types);
    violationTypes.forEach(type => {
      this.metrics.violationsByType[type] = 0;
    });
    
    // Initialize violation counters by severity
    const severityLevels = Object.values(Prime.Distributed.Coherence.Violations.Severity);
    severityLevels.forEach(severity => {
      this.metrics.violationsBySeverity[severity] = 0;
    });
    
    // Initialize recovery counters by strategy
    const recoveryStrategies = Object.values(Prime.Distributed.Coherence.Recovery.Strategies);
  ...
  updateMetrics(coherenceScore, checkResults, recoveryActions = null) {
    ...
    this.metrics.coherenceHistory.push({
      ...
      hasViolations: checkResults && checkResults.violations && checkResults.violations.length > 0
    });
    
    // Limit history size
    if (this.metrics.coherenceHistory.length > this.config.historySize) {
      this.metrics.coherenceHistory = this.metrics.coherenceHistory.slice(-this.config.historySize);
    }
    
    // Recalculate average coherence score
    const sum = this.metrics.coherenceHistory.reduce((total, entry) => total + entry.score, 0);
    this.metrics.averageCoherenceScore = sum / this.metrics.coherenceHistory.length;
    ...
    if (checkResults && checkResults.violations) {
      ...
      if (networkViolations.length > 0) {
        ...
        if (this.metrics.networkPartitions.lastPartitionTimestamp) {
          const partitionDuration = Date.now() - this.metrics.networkPartitions.lastPartitionTimestamp;
          this.metrics.networkPartitions.timeInPartitionedState += partitionDuration;
    ...
    const now = Date.now();
    if (now - this.lastReportTimestamp >= this.config.reportingInterval * 1000) {
  ...
  _generateReport() {
    ...
    const report = {
      ...
      summary: {
        ...
        violationRate: snapshot.checksPerformed > 0 ? 
          (snapshot.violationsDetected / snapshot.checksPerformed).toFixed(3) : 0,
        recoveryRate: snapshot.violationsDetected > 0 ? 
          (snapshot.recoveryActions / snapshot.violationsDetected).toFixed(3) : 0,
        mostCommonViolationType: this._findMostCommon(snapshot.violationsByType),
        mostCommonRecoveryStrategy: this._findMostCommon(snapshot.recoveryByStrategy),
