...
fn test_klein_group_homomorphism() {
    ...
    for &elem in &v4_elements {
        ...
        assert!((resonance - 1.0).abs() < 1e-10, 
            "Element {} should have resonance 1, got {}", elem, resonance);
    }
    ...
    for &a in &v4_elements {
        for &b in &v4_elements {
            let result = a ^ b;
            assert!(v4_elements.contains(&result),
                "{} XOR {} = {} should be in V4", a, b, result);
        }
    }
}
...
fn test_scale_adaptive_search() {
    ...
    let results = ccm_small.search_by_resonance(target, &alpha_small, tolerance).unwrap();
    
    // Verify all results are within tolerance
    for word in &results {
        let resonance = word.r(&alpha_small);
        assert!((resonance - target).abs() <= tolerance,
            "Result resonance {} outside tolerance", resonance);
    }
    ...
    let results_medium = ccm_medium.search_by_resonance(target, &alpha_medium, tolerance).unwrap();
    assert!(!results_medium.is_empty(), "Should find some results in medium dimension");
}
...
fn test_conservation_properties() {
    ...
    let klein_members = ccm.embedding().klein_members(&test_word);
