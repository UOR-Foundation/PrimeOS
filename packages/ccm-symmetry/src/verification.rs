...
use ccm_coherence::{CliffordElement, coherence_norm, coherence_product};
use num_traits::Zero;
...
impl<P: Float> GroupAxiomVerifier<P> {
    ...
    pub fn verify_inverses(&self) -> Result<bool, CcmError> {
        ...
        for g in self.group.generators() {
            ...
            if !self.elements_equal(&gg_inv, &identity) || 
               !self.elements_equal(&g_inv_g, &identity) {
                return Ok(false);
            }
        }
        Ok(true)
...
mod tests {
    ...
    fn test_ccm_invariance() {
        ...
        assert!(verifier.verify_all_invariances(&g, &x, &y, &action).unwrap());
    }
}