...
mod group_tests {
    ...
    fn test_group_generator_management() {
        ...
        let gen = GroupElement { params: vec![2.0, 3.0] };
        group.add_generator(gen.clone()).unwrap();
        assert_eq!(group.generators().len(), 1);
        
        // Wrong dimension should fail
        let bad_gen = GroupElement { params: vec![1.0, 2.0, 3.0] };
        ...
        for &pattern in &patterns {
            ...
            for i in 0..4 {
                for j in i+1..4 {
                    ...
                    assert_eq!(val & 0b00111111, 0, "Differences should only involve bits 6 and 7");
                }
            }
        }
    }
