...
mod tests {
    ...
    fn test_rotor_reverse() {
        ...
        let product = algebra.geometric_product(
            rotor.as_element(),
            reversed.as_element()
        ).unwrap();
        ...
        assert!((product.component(0).unwrap() - Complex::one()).norm() < tolerance,
                "Scalar component should be 1, got {:?}", product.component(0));
        for i in 1..product.num_components() {
            let comp = product.component(i).unwrap();
            assert!(comp.norm() < tolerance,
                    "Component {} should be 0, got {:?} (norm: {})", i, comp, comp.norm());
        }
    }
    ...
    fn test_rotor_composition() {
        ...
        for i in 0..composed.as_element().num_components() {
            assert!((composed.as_element().component(i).unwrap() - 
                    r2.as_element().component(i).unwrap()).norm() < 1e-6);
        }
    }
}