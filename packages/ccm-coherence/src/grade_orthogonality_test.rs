...
#[cfg(test)]
mod tests {
    use crate::{CliffordElement, coherence_product};
    ...
    fn test_grade_orthogonality() {
        ...
        assert!((prod_00.re - 4.0).abs() < 1e-10, "Grade 0 self-product should be 4");
        
        let prod_11 = coherence_product(&grade1, &grade1);
        assert!((prod_11.re - 25.0).abs() < 1e-10, "Grade 1 self-product should be 25 (3²+4²)");
        
        let prod_22 = coherence_product(&grade2, &grade2);
        assert!((prod_22.re - 61.0).abs() < 1e-10, "Grade 2 self-product should be 61 (5²+6²)");
        
        let prod_33 = coherence_product(&grade3, &grade3);
        assert!((prod_33.re - 49.0).abs() < 1e-10, "Grade 3 self-product should be 49 (7²)");
    }
    ...
    fn test_mixed_grade_orthogonality() {
        ...
        assert!((total_norm_sq - sum_of_grade_norms).abs() < 1e-10,
