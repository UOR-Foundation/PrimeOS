...
use ccm_coherence::{
    CliffordElement,
    optimization::{
        minimize_with_grades, minimize_coherence,
        MinimizationOptions, GradeConstraint,
    },
    coherence_norm,
};
...
fn main() {
    ...
    let result2 = minimize_coherence(
        &initial,
        &constraint,
        MinimizationOptions::default(),
    ).unwrap();
    ...
    let result3 = minimize_coherence(
        &initial,
        &grade_constraint,
        options,
    ).unwrap();
