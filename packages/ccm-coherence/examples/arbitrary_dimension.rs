...
fn main() {
    ...
    match ArbitraryCliffordAlgebra::<f64>::generate(20, config.clone()) {
        Ok(algebra) => {
            ...
            println!("   Can allocate full element: {}", algebra.can_allocate_element());
            println!("   Estimated memory: {} MB", ArbitraryCliffordAlgebra::<f64>::estimate_memory_mb(20));
    ...
    for n in [8, 12, 16, 20, 24, 30, 40, 50] {
        ...
        println!("   {:3} | {:10} | {:11.2} | {}",
                 n,
                 if n < 64 { components.to_string() } else { "overflow".to_string() },
                 mb,
