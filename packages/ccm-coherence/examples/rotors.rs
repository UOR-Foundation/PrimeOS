...
use ccm_coherence::{
    ...
    rotor::{Rotor, rotor_from_vectors},
    coherence_norm,
};
...
fn main() {
    ...
    bivector.set_component(3, Complex::new(angle / 2.0, 0.0)).unwrap(); // e₁e₂
    
    let rotor = Rotor::from_bivector(&bivector, &algebra).unwrap();
    println!("   Rotation angle: {:.1} degrees", angle * 180.0 / PI);
    println!("   Rotor is normalized: {}", rotor.as_element().is_unit_norm());
    ...
    // Create vector (e₁ + e₂)/√2
    to.set_component(1, Complex::new(1.0 / 2.0_f64.sqrt(), 0.0)).unwrap();
    to.set_component(2, Complex::new(1.0 / 2.0_f64.sqrt(), 0.0)).unwrap();
...
fn print_vector_components(v: &CliffordElement<f64>) {
    ...
    println!("   Components: {:.3}e₁ + {:.3}e₂ + {:.3}e₃", 
