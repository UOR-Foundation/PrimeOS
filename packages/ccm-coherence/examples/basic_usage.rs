...
fn main() {
    ...
    println!("   Number of basis elements: {}\n", algebra.num_basis_elements());
    ...
    println!("   e₁ ∧ e₂ gives bivector at index {}", find_nonzero_index(&wedge));
    println!("   e₁ · (e₁e₂) gives vector at index {}\n", find_nonzero_index(&inner));
    ...
    println!("   Grade 2 (bivector): norm = {:.2}", coherence_norm(&grade2));
}

// Helper functions
fn count_nonzero<T: ccm_core::Float>(elem: &CliffordElement<T>) -> usize {
    (0..elem.num_components())
        .filter(|&i| elem.component(i).map(|c| c.norm() > T::epsilon()).unwrap_or(false))
        .count()
}

fn find_nonzero_index<T: ccm_core::Float>(elem: &CliffordElement<T>) -> usize {
    (0..elem.num_components())
        .find(|&i| elem.component(i).map(|c| c.norm() > T::epsilon()).unwrap_or(false))
        .unwrap_or(0)
}