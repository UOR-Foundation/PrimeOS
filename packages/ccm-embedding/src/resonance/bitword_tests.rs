...
mod tests {
    ...
    fn test_bitword_resonance_computation() {
        for n in [8, 16, 32, 64, 128, 256, 512, 1024] {
            ...
            assert_eq!(zero.r(&alpha), 1.0, "Empty product should be 1.0 for n={}", n);
            ...
            for i in 0..n.min(10) {
                ...
                word.set_bit(i, true);
                let r = word.r(&alpha);
                assert_eq!(r, alpha[i], "Single bit {} should give α_{} for n={}", i, i, n);
            }
            ...
            if n >= 2 {
                ...
                assert!((r - 1.0).abs() < 1e-10, 
                    "Unity positions should give resonance 1.0 for n={}", n);
            }
        }
    }
        ...
        for n in [8, 16, 32, 64, 128] {
            ...
            for pattern in [0u64, 1, 42, 255, u64::MAX >> (64 - n.min(64))] {
                ...
                for member in &members {
                    ...
                    assert_eq!(repr, member_repr, 
                        "All Klein members should have same representative for n={}", n);
                }
                ...
                if word.is_klein_minimum(&alpha) {
                    ...
                    for member in &members {
                        assert!(member.r(&alpha) >= my_r,
                            "Klein minimum should have lowest resonance for n={}", n);
                    }
                }
            }
        }
    ...
    fn test_bitword_resonance_classes() {
        for n in [8, 10, 12, 16] {
            ...
            assert!(spectrum.len() > 0, "Should have at least one resonance value");
            
            // For small n, verify we have a reasonable number
            if n <= 12 {
                assert!(spectrum.len() >= n, 
                    "Should have at least {} unique resonances for n={}", n, n);
            }
            ...
            assert!(dist.total_classes > 0,
                "Should have at least one class for n={}", n);
            
            // Verify distribution properties
            assert!(dist.size_2_count > 0 || dist.size_4_count > 0,
                "Should have some resonance classes for n={}", n);
        }
    }
    ...
    fn test_bitword_conservation() {
        for n in [8, 10, 12] {
            ...
            assert!(sum > 0.0 && sum.is_finite(),
                "Resonance sum should be positive and finite for n={}", n);
            ...
            if test_range == (1usize << n) {
                assert!(current_sum.abs() < 1e-10,
                    "Current sum over full cycle should be zero for n={}", n);
            }
        }
    }
    ...
    fn test_bitword_homomorphic() {
        for n in [8, 10, 12, 16] {
            ...
            if n >= 2 {
                let unity_pair_found = pairs.iter().any(|&(i, j)| 
                    (i == n-2 && j == n-1) || (i == n-1 && j == n-2)
                );
                assert!(unity_pair_found,
                    "Should find unity pair at ({},{}) for n={}", n-2, n-1, n);
            }
            ...
            assert!(!subgroups.is_empty(), "Should have at least trivial subgroup");
            
            // Verify subgroup properties
            for sg in &subgroups {
                assert_eq!(sg.elements.len(), sg.order,
                    "Element count should match order for n={}", n);
                
                // Verify closure under XOR
                for a in &sg.elements {
                    for b in &sg.elements {
                        let c = a ^ b;
                        assert!(sg.elements.contains(&c),
                            "Subgroup should be closed under XOR for n={}", n);
                    }
                }
            }
        }
    ...
    fn test_bitword_unity_positions() {
        for n in [8, 10, 12, 16] {
            ...
            if n >= 2 {
                ...
                eprintln!("n={}: α[{}]={}, α[{}]={}, product={}",
                    n, n-2, alpha[n-2], n-1, alpha[n-1], unity_product);
                assert!((unity_product - 1.0).abs() < 1e-10,
                    "Unity constraint not satisfied for n={}", n);
            }
            ...
            if n >= 2 {
                for klein in 0..4u8 {
                    ...
                    eprintln!("Klein element {} (bits {},{} = {},{}) has resonance {}",
                        klein, n-2, n-1, (klein & 1) != 0, (klein & 2) != 0, r);
                }
            }
            ...
            for &pos in &positions {
                ...
                if (r - 1.0).abs() > 0.01 {
                    eprintln!("Bad unity position {} for n={}: word bits = {:?}, resonance = {}", 
                        pos, n, 
            ...
            assert!(density > 0.0 && density <= 1.0,
                "Unity density should be in (0,1] for n={}", n);
        }
    }
                ...
                if word.bit(i) {
                    ...
                    if alpha[i] > 1.0 {
                        assert!(gradients[i] < 0.0,
                            "Gradient should be negative for set bit {} with α > 1", i);
                    }
                } else {
                    // For unset bits, gradient should be positive if α_i > 1
                    if alpha[i] > 1.0 {
                        assert!(gradients[i] > 0.0,
                            "Gradient should be positive for unset bit {} with α > 1", i);
                    }
                }
            }
        ...
        for n in [8, 10, 12] {
            ...
            if let Ok(factors) = BitWord::factor_resonance(test_r, &alpha) {
                ...
                assert!(factors.iter().any(|f| f == &expected),
                    "Should find factorization [0,2] for n={}", n);
            }
        }
    }
    ...
    fn test_bitword_edge_cases() {
        ...
        assert_eq!(r_large, 1.0, "Empty large BitWord should have resonance 1.0");
    }
