...
pub mod tests {
    ...
    fn test_minimal_embedding() {
        ...
        assert!(klein_members.iter().any(|m| m == &min_embedding),
        ...
        for member in klein_members {
            assert!(member.r(&alpha) >= min_resonance - 1e-10,
        ...
        for test_byte in [0u8, 1, 42, 128, 255] {
            ...
            for expected in expected_members {
                assert!(members.contains(&expected),
                    "Klein group should contain byte {}", expected);
            }
        }
    }
    ...
    fn test_conservation_laws() {
        ...
        assert!((sum_768 - expected).abs() < 1e-10,
            "768-cycle should equal 3 × 256-cycle: {} vs {}", sum_768, expected);
