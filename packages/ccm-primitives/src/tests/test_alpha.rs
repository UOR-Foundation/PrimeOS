...
mod tests {
    ...
    fn test_resonance_with_dynamic_alpha() {
        ...
        assert_ne!(alpha8[1], alpha32[1], "Different bit lengths should produce different alphas");
    }
    ...
    fn test_mathematical_vs_calculated() {
        ...
        // Both should satisfy unity constraint
        assert!((calc[4] * calc[5] - 1.0).abs() < 1e-10);
        assert!((math[4] * math[5] - 1.0).abs() < 1e-10);
    }
    ...
    fn test_scaling_property() {
        ...
        let range = |alpha: &AlphaVec<f64>| {
            ...
            let min = alpha.values.iter().filter(|&&v| v > 0.0).fold(f64::MAX, |a, &b| a.min(b));
            max / min
        };
