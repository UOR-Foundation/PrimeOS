...
#[cfg(test)]
mod tests {
    use crate::{ResonanceConservation, ResonanceClasses, HomomorphicResonance, AlphaVec};
    ...
    fn test_homomorphic_subgroups_count() {
        ...
        for (i, sg) in subgroups.iter().enumerate() {
            println!("Subgroup {}: generator={}, order={}, elements={:?}",
    ...
    fn test_current_extrema() {
        ...
        println!("Maximum current: {} at position {}", extrema.max_value, extrema.max_position);
        println!("Minimum current: {} at position {}", extrema.min_value, extrema.min_position);
        ...
        assert!(
            ...
            extrema.max_value, extrema.min_value
        );
    }
}