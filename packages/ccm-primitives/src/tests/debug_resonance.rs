...
#[cfg(test)]
mod tests {
    use crate::{Resonance, HomomorphicResonance, AlphaVec};
    ...
    fn debug_resonance_values() {
        ...
        println!("\nUnity constraint: α[6] * α[7] = {} * {} = {}", 
                 alpha[6], alpha[7], alpha[6] * alpha[7]);
        ...
        for (i, member) in klein_0.iter().enumerate() {
            println!("Member {}: {} (binary: {:08b}) -> R = {}", 
                     i, member, member, member.r(&alpha));
        }
        ...
        for (i, j) in pairs {
            println!("Bits ({}, {}) are homomorphic: α[{}] * α[{}] = {}", 
