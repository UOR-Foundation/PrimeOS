...
pub trait UnityStructure {
    ...
    fn unity_positions<P: Float>(alpha: &AlphaVec<P>, range: usize) -> Vec<usize>;
...
pub mod analysis {
    ...
    pub fn analyze_unity_spacing<P: Float>(alpha: &AlphaVec<P>, range: usize) -> UnitySpacing {
    ...
    pub fn unity_density<P: Float>(alpha: &AlphaVec<P>, range: usize) -> f64 {
        let positions = u8::unity_positions(alpha, range);
...
pub mod klein_unity {
    ...
    pub fn unity_klein_groups<P: Float>(alpha: &AlphaVec<P>) -> Vec<Vec<u8>> {
        ...
        for value in 0u8..=255 {
            // Get the Klein representative (clear bits 6,7 for u8)
            let repr = value & 0b00111111;
