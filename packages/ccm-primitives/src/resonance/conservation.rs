...
#[cfg(feature = "alloc")]
impl<P: Float> ResonanceConservation<P> for BitWord {
    fn resonance_sum(start: usize, count: usize, alpha: &AlphaVec<P>) -> P {
        let mut sum = P::zero();
        let n = alpha.len().min(64); // Limit to practical size
        let modulus = if n >= 64 { usize::MAX } else { 1usize << n };

        for i in 0..count {
            let value = if modulus == usize::MAX {
                start.wrapping_add(i)
            } else {
                (start + i) % modulus
            };
            let word = BitWord::from_u64(value as u64, n);
    ...
    fn verify_conservation(alpha: &AlphaVec<P>) -> ConservationResult<P> {
        let n = alpha.len().min(20); // Limit to practical size for conservation
    ...
    fn resonance_current(idx: usize, alpha: &AlphaVec<P>) -> P {
        let n = alpha.len().min(64);
        let modulus = if n >= 64 { usize::MAX } else { 1usize << n };
        
        let current_val = if modulus == usize::MAX {
            idx
        } else {
            idx % modulus
        };
        
        let next_val = if modulus == usize::MAX {
            idx.wrapping_add(1)
        } else {
            (idx + 1) % modulus
        };
        
        let current = BitWord::from_u64(current_val as u64, n);
        let next = BitWord::from_u64(next_val as u64, n);
...
pub mod properties {
    ...
    pub fn verify_8k_conservation<P: Float>(alpha: &AlphaVec<P>, k: usize) -> bool {
    ...
    pub fn verify_telescoping<P: Float>(alpha: &AlphaVec<P>, range: usize) -> bool {
        let mut sum = P::zero();
