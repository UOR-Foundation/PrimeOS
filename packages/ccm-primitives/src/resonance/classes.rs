...
impl<P: Float, const N: usize> ResonanceClasses<P> for BitWord<N> {
    fn resonance_spectrum(alpha: &AlphaVec<P>) -> Vec<P> {
        let mut unique_resonances = Vec::new();
        ...
        if N <= 16 {
            ...
            for repr in 0..=klein_mask {
                ...
                let tolerance = P::epsilon() * P::from(100.0).unwrap();
                let already_exists = unique_resonances.iter().any(|&r: &P| (r - min_resonance).abs() < tolerance);
...
mod tests {
    ...
    fn test_resonance_spectrum_u8() {
        ...
        assert!(spectrum.len() > 10, "Expected at least 10 unique resonances, got {}", spectrum.len());
        assert!(spectrum.len() <= 256, "Cannot have more unique resonances than input values");
    ...
    fn test_class_distribution() {
        ...
        assert!(dist.total_classes > 0, "Should have at least one resonance class");
        ...
        assert_eq!(total_values, 256, "All 256 values should be in exactly one resonance class");
        
        // The total number of classes should match the sum of all size counts
        let total_class_count = dist.size_2_count + dist.size_4_count + 
                               dist.other_sizes.iter().map(|(_, count)| count).sum::<usize>();
